...
void create_zone(int theme, int map_room1, int map_room2, int level_range, int rooms)
{
	...
	while (rooms - i)
	{
		...
		else
		{
			...
			connect_rooms(world[parent_room].number, world[room_nr].number, direction);
...
int write_relic_highscore()
{
	...
	fprintf(f, "%d %d %d", GOODIE_RELIC_POINTS, EVIL_RELIC_POINTS, UNDEAD_RELIC_POINTS);
	fclose(f);
...
int get_relic_num(P_char ch)
{
	...
	if (EVIL_RACE(ch) && !IS_PUNDEAD(ch))
		return relic_struct_data[1].good_relic + relic_struct_data[1].undead_relic;
...
int random_quest_mob_proc(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	if (arg && cmd == CMD_ASK)
	{
		...
		{
			mobsay(ch, "I'll help ya with directions to hell, if I could see you!!");
			return 1;
		}
		...
		mobsay(ch, "Also, if you happen to find a sigil in there please bring it to me.");
		mobsay(ch, "It is something I treasure and I will reward you apon its return.");
		return 1;
	}
...
int create_lab(int type)
{
	...
	while (rooms - i > 0)
	{
		...
		if (!number(0, 4))
		{
			mob = (P_char)create_random_mob(theme_to_use, (int)(40 + number(0, 22)));
			if (mob)
			{
				char_from_room(mob);
