...
{
	if (affected_by_spell(ch, SKILL_BERSERK) || GET_CHAR_SKILL(ch, SKILL_BERSERK) < 1)
		return;
	...
	if (OBJ_WORN_BY(obj, ch))
	{
		...
		if (!number(0, 27))
		{
			...
			for (tch = world[ch->in_room].people; tch; tch = tch_next)
			{
				...
				if (IS_NPC(tch) && (!tch->following || IS_NPC(tch->following)) && (GET_OPPONENT(ch) != tch) && (GET_OPPONENT(tch) != ch))
				{
					continue;
				}
				if (IS_TRUSTED(tch))
				...
				if (((IS_FIGHTING(tch) && (GET_OPPONENT(tch) == ch)) || !IS_FIGHTING(ch)) && !IS_DRAGON(tch))
				{
		...
		if (!number(0, 27))
		{
			...
			for (tchar1 = world[ch->in_room].people; tchar1; tchar1 = tchar2)
			{
				...
				if (!IS_DRAGON(tchar1) && !IS_TRUSTED(tchar1) && (tchar1->specials.z_cord == ch->specials.z_cord))
				{
...
int orb_of_the_sea(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (OBJ_WORN_BY(obj, ch) && IS_ALIVE(victim) && !IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) && !IS_GH_GOLEM(victim) && GET_RACE(victim) != RACE_CONSTRUCT &&
	    !(strstr(victim->player.name, "_no_move_")))
	{
		// 1/30 chance.
		if (!number(0, 29))
		{
...
{
	P_char                 vict;
	struct damage_messages messages = {"Your $q &+Wslams &+L$N with a &+yhuge boulder&+L!",
	...
	{
		spell_damage(ch, vict, 406, SPLDAM_GENERIC, SPLDAM_NOSHRUG | SPLDAM_NODEFLECT, &messages);
	}
	return (FALSE);
}
	...
	if ((cmd == CMD_SAY) && (!(IS_FIGHTING(ch))))
	{
		...
		if (!strcmp(arg, "efreeti") && OBJ_WORN_BY(obj, ch))
		{
			...
			if (obj->timer[0] + 300 <= curr_time)
			{
				...
				{
					send_to_room("&+wSm&+Lok&+we r&+Lis&+Wes&n from &+rthe holy flamberge&+w and slowly forms into the &+wim&+Wa&+Lge&n of an &+ref&+Rre&+ret&+Ri&n &+LUnq&+wu&+Lenchable&n who brings "
					             "forth the &+rf&+Rir&+Re&+rs of &+Rhe&+rl&+Rl&n!.&n\n",
					             ch->in_room);
				...
				{
					send_to_room("A &+wh&+Lea&+wt h&+La&+Wze&n from &+rthe holy flamberge&+w materializes into the &+wim&+Wa&+Lge&n of an &+LOrd&+we&+Lr of &+RFi&+re&+Rry He&+ra&+Rrt&n who calls on "
					             "the &+wforces of &+wl&+Wi&+Yg&+Wh&+wt for &+wpr&+Wot&+Yec&+Wti&+won&n!.&n\n",
					             ch->in_room);
...
int unholy_avenger_bloodlust(P_obj obj, P_char ch, int cmd, char *arg)
{
	P_char                 vict;
	int                    dam;
	struct damage_messages messages = {"&+rBlood red energy travels into your hand and up your arm, seeping into your veins!",
		...
		if (!number(0, 24))
		{
			...
			if (is_char_in_room(vict, ch->in_room))
			{
				if (GET_RACE(vict) != RACE_E_ELEMENTAL)
					spell_damage(ch, vict, number(500, 600), SPLDAM_GENERIC, SPLDAM_NOSHRUG | SPLDAM_NODEFLECT, &messages);
				else
				{
					act("&+yThe earth and rock heals $N!", FALSE, 0, obj, vict, TO_CHAR);
					act("$n &+yheals you with the power of the earth and rock.", FALSE, 0, obj, vict, TO_VICT);
					act("$n &+yheals $N &+ywith the power of earth and rock.", FALSE, 0, obj, vict, TO_NOTVICT);
...
int flamberge(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (OBJ_WORN_BY(obj, ch) && vict)
	{
		if (!number(0, 24))
		{
			act("Your $q &+rsummons the power of fire and flames at at $N.", FALSE, obj->loc.wearing, obj, vict, TO_CHAR);
			act("$n's $q &+rsummons the power of fire and flames at you.", FALSE, obj->loc.wearing, obj, vict, TO_VICT);
			act("$n's $q &+rsummons the power of fire and flames at $N.", FALSE, obj->loc.wearing, obj, vict, TO_NOTVICT);
			spell_burning_hands(60, ch, 0, SPELL_TYPE_SPELL, vict, 0);
			if (char_in_list(vict))
				spell_fireball(60, ch, 0, SPELL_TYPE_SPELL, vict, 0);
			if (char_in_list(vict))
			{
				act("&+rA &+Rse&+Yari&+Rng &+Yburst &+rleaps from your $q&+r and hits $N &+Rdead on!", FALSE, obj->loc.wearing, obj, vict, TO_CHAR);
				act("&+rA &+Rse&+Yari&+Rng &+Yburst &+rleaps from $n's $q&+r and hits you &+Rdead on!", FALSE, obj->loc.wearing, obj, vict, TO_VICT);
				act("&+rA &+Rse&+Yari&+Rng &+Yburst &+rleaps from $n's $q&+r and hits $N &+Rdead on!", FALSE, obj->loc.wearing, obj, vict, TO_NOTVICT);
...
int avernus(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	*/
	...
	// 1/25 chance.
	if (!number(0, 24)) /*    && (CheckMultiProcTiming(ch) || !number(0, 2)) && !IS_UNDEADRACE(vict) && !IS_CONSTRUCT(vict))*/
	{
	...
	if (cmd == CMD_PERIODIC)
	{
		...
		for (obj = ch->carrying; obj; obj = obj->next_content)
		{
			...
			if (obj->type == ITEM_CORPSE && IS_SET(obj->value[1], PC_CORPSE) && obj->timer[0] < time(NULL) && !number(0, 9))
			{
				...
				act("$n&n &+mst&+Mop&+ms s&+Mud&+mde&+Mnl&+my&n, then begins to &+yu&+Yn&+yd&+Yu&+yl&+Ya&+yt&+Ye&n.  $o&n drops from $s behind.", FALSE, ch, obj, 0, TO_ROOM);
				writeCorpse(obj);
				break;
			}
		}
		...
		if (!number(0, 9))
		{
			...
			if (!IS_TRUSTED(vict) && !(IS_NPC(vict) && IS_ELITE(vict)) && !IS_NEXUS_GUARDIAN(vict) && !IS_GH_GOLEM(vict) && !IS_OP_GOLEM(vict))
			{
				if (vict->in_room == ch->in_room)
				{
					...
					act("$n opens $s gaping maw, and the last thing you feel is\nthe odd sensation of sliding down $s throat.", FALSE, ch, 0, vict, TO_VICT);

					if (IS_PC(vict))
					{
						statuslog(vict->player.level, "%s killed by purple worm swallow at [%d].", GET_NAME(vict), ((vict->in_room == NOWHERE) ? -1 : world[vict->in_room].number));
						logit(LOG_DEATH, "%s killed by purple worm swallow at [%d].", GET_NAME(vict), ((vict->in_room == NOWHERE) ? -1 : world[vict->in_room].number));
					}
...
int piercer(P_char ch, P_char pl, int cmd, char *arg)
{
	...
	int                    dam;
	struct damage_messages messages = {"You gently pierce $N when you fall from the ceiling.",
	 ...
	 */
	if (!GET_OPPONENT(ch) && IS_AFFECTED(ch, AFF_HIDE) && MIN_POS(ch, POS_STANDING + STAT_RESTING))
	{
		for (tmp_ch = world[ch->in_room].people; tmp_ch; tmp_ch = next)
		{
			next = tmp_ch->next_in_room;

			if (IS_PC(tmp_ch) && CAN_SEE(ch, tmp_ch) && !IS_TRUSTED(tmp_ch) && !IS_AFFECTED(tmp_ch, AFF_HIDE))
			{
				vict = tmp_ch;
			}
			if (vict && (ch->in_room == vict->in_room))
			{
				...
				/*
				   ...
				   sorts.
				 */
				if ((IS_AFFECTED(vict, AFF_AWARE) || IS_AFFECTED(vict, AFF_SKILL_AWARE)) && StatSave(vict, APPLY_AGI, -10))
				{
					/*
					   piercer missed, they dodged
					 */
					act("$n crashes to the ground, missing you by a hair!", FALSE, ch, 0, vict, TO_VICT);
					act("$n crashes to the ground, missing $N by a hair!", FALSE, ch, 0, vict, TO_NOTVICT);
					act("You crash to the ground, missing $N by a hair!", FALSE, ch, 0, vict, TO_CHAR);
				}
				else
				{
					if (!IS_FIGHTING(ch))
...
int elfgate(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (((GET_RACE(ch) != RACE_GREY) && (GET_RACE(ch) != RACE_HALFELF) && GET_RACE(ch) != RACE_CENTAUR))
	{
		send_to_char("You are not of TRUE faerie blood, you may not enter this gate.\n", ch);
		return (FALSE);
	}
	if (GET_LEVEL(ch) < 1)
	{
		send_to_char("The gate flares briefly, but refuses to transport someone of your level.\n", ch);
		return (FALSE);
	}
  ...
  teleport_to(ch, real_room(obj->value[temp]));
	...
	} while (IS_ROOM(to_room, ROOM_PRIVATE) || PRIVATE_ZONE(to_room) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN || world[to_room].number < 110000 ||
	         world[to_room].number >= 210000);
	/*         !IS_MAP_ROOM(to_room)); */
	if (IS_FIGHTING(ch))
		stop_fighting(ch);
	if (IS_DESTROYING(ch))
...
int nexus(P_obj obj, P_char ch, int cmd, char *arg)
{
  ...
  teleport_to(ch, real_room(obj->value[temp]));
	...
	{
		to_room = number(0, top_of_world);
	} while (IS_ROOM(to_room, ROOM_PRIVATE) || PRIVATE_ZONE(to_room) || IS_ROOM(to_room, ROOM_NO_TELEPORT) || world[to_room].sector_type == SECT_OCEAN || !IS_SURFACE_MAP(to_room));
	if (IS_FIGHTING(ch))
		stop_fighting(ch);
	if (IS_DESTROYING(ch))
		stop_destroying(ch);
...
int magic_map_pool(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	while (world[target_room].sector_type == SECT_MOUNTAIN || world[target_room].sector_type == SECT_INSIDE || world[target_room].sector_type == SECT_OCEAN || IS_ROOM(target_room, ROOM_NO_GATE))
	{
		target_room = real_room(random_map_room());
	}
...
int random_map_room() { return number(SURFACE_MAP_START, SURFACE_MAP_END); }
	...
	if (obj->loc.wearing == ch)
	{
			...
			if (j)
			{
				...
				if (IS_ALIVE(vict) && IS_PC(vict) && (vict->in_room != ch->in_room))
				{
					...
					if (!number(0, 6))
					{
						...
						send_to_char("&+WSuddenly, a flash of light obscures your vision.  You feel as though you are falling at an incredible rate, but yet you can hear nothing.  You find yourself "
						             "elsewhere...\n",
						             vict);
						char_from_room(vict);
		...
		else if (!number(0, 399))
		{
			...
			if (vict && (GET_LEVEL(vict) < 51) && !IS_TRUSTED(vict))
			{
				...
				{
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly flashes out at incredible speed, slashing at the head of $N&+W and instantly beheading $M!",
					    FALSE,
					    ch,
					    obj,
					    vict,
					    TO_NOTVICT);
					act("&+WIn a blinding burst of light, your $q&+W suddenly flashes out at incredible speed, slashing at the head of $N&+W and instantly beheading $M!",
					    FALSE,
					    ch,
					    obj,
					    vict,
					    TO_CHAR);
					act("&+WIn a blinding burst of light, $n&+W's $q&+W suddenly flashes out at incredible speed, slashing at your head and resulting in your instant beheading!",
					    FALSE,
					    ch,
					    obj,
					    vict,
...
{
	char                   bufs[6][128];
	struct damage_messages messages = {bufs[0], bufs[1], bufs[2], bufs[3], bufs[4], bufs[5], 0};
	char                   dam_msg[256];
	char                   colors[5][16] = {"&+rred&N", "&+Lblack&N", "&+bblue&N", "&+ggreen&N", "&+Wwhite&N"};
	P_char                 vict, next_ch;
	P_obj                  t_obj, next;
	int                    damtype, i;
	...
	if (cmd == CMD_DEATH)
	{
		act("&+WWith her very last breath, &+LTiamat &+Wcloses her eyes.\n"
		    ...
		    "&+WAs the light subsides, only a few pieces of her once great body remain.",
		    FALSE,
		    ch,
		    0,
		    0,
		...
		debug("&+LTiamat death: Heart decays in &+C%d&+L obj ticks = &+C%d&+L sec = &+C%d&+L:&+C%02d&+L:&+C%02d&+L.&n", obj->value[0], obj->value[1], obj->value[2], obj->value[3], obj->value[4]);
		logit(LOG_OBJ, "Tiamat death: Heart decays in %d obj ticks = %d sec = %d:%02d:%02d.", obj->value[0], obj->value[1], obj->value[2], obj->value[3], obj->value[4]);
	...
	if (ch->in_room == real_room(VROOM_TIAMAT_HOME))
	{
		...
		{
			act("&+LWith a mighty &+RROAR&+l, &+LTiamat shreds the blanket of silence!", 0, ch, 0, 0, TO_ROOM);
			affect_from_char(ch, SPELL_SILENCE);
		}

		if (!IS_ROOM(ch->in_room, ROOM_MAGIC_LIGHT))
		{
			act("A beam of light radiates from &+LTiamats eyes, lighting a sconce upon the wall.", FALSE, ch, NULL, NULL, TO_ROOM);
	...
	switch (number(1, 3))
	{
		...
		case 1:
			...
			for (vict = world[ch->in_room].people; vict; vict = next_ch)
			{
				...
				if (!IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) && IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch))
				{
		...
		case 2:
			...
			if (vict && !IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) && IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch))
			{
				if (!StatSave(vict, APPLY_AGI, -4))
				{
					struct damage_messages msgs = {"", "&+LHer immense tail broadsides you in the face!", "$N&+L is slapped by Tiamat's tail, and crashes to the ground!", "", msgs.victim, msgs.room};
		...
		default:
			...
			if (vict && !IS_DRAGON(vict) && !affected_by_spell(vict, SKILL_BERSERK) && IS_FIGHTING(vict) && (GET_OPPONENT(vict) == ch) && !IS_TRUSTED(vict))
			{
				struct damage_messages msgs = {
					"", "&+RHer tail lashes out, stabbing you deep in the back!", "$N makes a strange sound as &+LTiamat's tail stabs $M in the back!", msgs.attacker, msgs.victim, msgs.room};
					...
					if (!IS_DRAGON(vict))
					{
						...
						switch (i)
						{
							...
							case 1:
								...
								{
									struct damage_messages msgs = {"$N &+Rcatches on fire!",
							...
							case 3:
								...
								{
									struct damage_messages msgs = {"$N's&+B face turns a shade of blue, as $s heart stops for a moment!",
							...
							case 4:
								if (!number(0, 5))
								{
									act("&+LTiamat &+RROARS &+Lloudly, and her tailsweep sends you crashing into the wall!", FALSE, ch, 0, vict, TO_VICT);
									act("&+LTiamat &+RROARS &+Lloudly, and in a frightening display, whips her tail deftly about the room!", FALSE, ch, 0, vict, TO_NOTVICT);
									SET_POS(vict, POS_SITTING + GET_STAT(vict));
							...
							case 5:
								...
								{
									struct damage_messages msgs = {"$N &+Wscreams in pain as $S stone-like skin cracks under the intense cold!",
...
int elvenkind_cloak(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (tch)
	{
		act("$n's $q &+gflashes brilliantly and suddenly flares in front of the spell, deflecting it to YOU!", FALSE, ch, obj, tch, TO_VICT);
		act("$n's $q &+gflashes brilliantly and suddenly flares in front of the spell, deflecting it to $N!", FALSE, ch, obj, tch, TO_NOTVICT);
		act("Your $q &+gflashes brilliantly and suddenly flares in front of the spell, deflecting it to $N!", FALSE, ch, obj, tch, TO_CHAR);
...
int deflect_ioun(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (tch)
	{
		act("$n's $p flashes brilliantly and suddenly darts in front of the spell, deflecting it to YOU!", FALSE, ch, obj, tch, TO_VICT);
		act("$n's $p flashes brilliantly and suddenly darts in front of the spell, deflecting it to $N!", FALSE, ch, obj, tch, TO_NOTVICT);
		act("Your $p flashes brilliantly and suddenly darts in front of the spell, deflecting it to $N!", FALSE, ch, obj, tch, TO_CHAR);
		spell_damage(ch, tch, data->dam, data->attacktype, data->flags | SPLDAM_NODEFLECT, data->messages);
		return TRUE;
	}
...
void barb_proc_dwarven_ancestor(int level, P_char ch, P_char victim)
{
	...
	act("&+rIn a column of &+RROARING&+r fire, $N&+r makes $S entrance.", FALSE, ch, 0, mob, TO_CHAR);
	act("&+rIn a column of &+RROARING&+r fire, $N&+r makes $S entrance.", FALSE, ch, 0, mob, TO_ROOM);
	...
	{
		if (is_char_in_room(ch, ch->in_room) && is_char_in_room(victim, ch->in_room))
			switch (number(0, 2))
			{
				case 0:
					if (isMaulable(mob, victim))
						maul(mob, victim);
					else if (isKickable(mob, victim) && number(0, 1))
						do_kick(mob, 0, 0);
					else if (GET_CHAR_SKILL(mob, SKILL_INFURIATE))
						do_infuriate(mob, NULL, CMD_INFURIATE);
					break;
				case 1:
					act("$n &+rflows into battle!", FALSE, mob, 0, 0, TO_ROOM);
					do_rage(mob, NULL, CMD_RAGE);
					break;
				case 2:
					act("$n &+Rlets out an ear-shattering war cry!", FALSE, mob, 0, 0, TO_ROOM);
					do_war_cry(mob, 0, 0);
					break;
				default:
					break;
...
int barb(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	static struct _BarbProcArtiData BarbProcData =
		...
		{
			NULL, NULL, 0, 0, 0, {NULL, NULL, NULL, NULL, NULL, NULL, 0, NULL},
                 FALSE
    };

	// Deflect a spell with bad messages.
	struct damage_messages messages_deflect = {"&+bA wave of &+Bmagical &+Cenergy&+b released from your weapon strikes down at $N!",
	...
	if ((cmd == CMD_GOTNUKED) && (GET_RACE(ch) == RACE_MOUNTAIN || GET_RACE(ch) == RACE_DUERGAR) && (!number(0, 5)))
	{
		...
		if (BarbProcData.damage)
		{
			...
			result = spell_damage(
				ch, vict, BarbProcData.damage, BarbProcData.attacktype, BarbProcData.flags | SPLDAM_NOSHRUG | SPLDAM_NODEFLECT, BarbProcData.messages_set ? &BarbProcData.messages : &messages_deflect);
		...
		if (data->messages && data->messages->attacker && data->messages->victim && data->messages->room && data->messages->death_attacker && data->messages->death_victim &&
		    data->messages->death_room)
		{
	...
	if (IS_ALIVE(vict))
	{
		...
		else if ((!number(0, 49) && (GET_RACE(ch) == RACE_MOUNTAIN || GET_RACE(ch) == RACE_DUERGAR)) && CheckMultiProcTiming(ch))
		{
			...
			{
				result              = spell_damage(ch, vict, BarbProcData.damage, BarbProcData.attacktype, BarbProcData.flags, BarbProcData.messages_set ? &BarbProcData.messages : &messages_deflect);
				BarbProcData.damage = 0;
				BarbProcData.attacktype = 0;
				BarbProcData.flags      = 0;
...
int gfstone(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	char params[MAX_STRING_LENGTH], owner[MAX_STRING_LENGTH], buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH], buf3[MAX_STRING_LENGTH];
	...
	if (!IS_ALIVE(ch) || !OBJ_WORN_BY(obj, ch) || (!OBJ_WORN_POS(obj, HOLD) && !OBJ_WORN_POS(obj, SECONDARY_WEAPON)))
	{
		return FALSE;
	}
		...
		if (should_area_hit(ch, victim))
		{
			...
			else if (HAS_FOOTING(victim) && number(0, 1))
			{
				act("&+LThe ground splits apart as a &+Bmass of water&+L erupts "
				    "from below knocking $n off $s feet.",
				    FALSE,
				    victim,
				    0,
				    0,
				    TO_ROOM);
				act("&+LThe ground splits apart as a &+Bmass of water&+L erupts "
				    "from below knocking you off your feet.",
				    FALSE,
				    victim,
				    0,
				    0,
	...
	if (arg && (isname(arg, "trident") || isname(arg, "tsunami")))
	{
		...
		if (cmd == CMD_TAP)
		{
			if (obj->timer[0] + 300 <= curr_time)
			{
				...
				act("You tap $q on the ground three times.", FALSE, ch, obj, vict, TO_CHAR);
				act("$n taps $q on the ground three times.", FALSE, ch, obj, vict, TO_ROOM);
				...
				send_to_room("\n&+BT&+Csun&+Wami&+C, t&+Bhe &+btri&+Bden&+Ct o&+Wf s&+Ctor&+Bm s&+budd&+Benl&+Cy e&+Wrup&+Cts &+Bint&+bo a&+B gu&+Cshi&+Wng &+Cflo&+Bw o&+bf p&+Bure&+C wh&+Wite&+C,\n",
				             ch->in_room);
				send_to_room("&+Bfro&+Cthi&+Wng &+Cwat&+Ber,&+b co&+Bale&+Csin&+Wg i&+Cnto&+B a &+bnea&+Brly&+C pe&+Wrfe&+Cct &+Bfor&+bm r&+Bese&+Cmbl&+Wing&+C Po&+Bsei&+bdon&+B,  &+Crul&+Wer\n",
				             ch->in_room);
				send_to_room("&+bof&+B th&+Ce S&+Wea &+CKin&+Bgdo&+bm. &+B  H&+Cis &+Wmou&+Cth &+Bope&+bn i&+Bn a&+C si&+Wlen&+Ct s&+Bcre&+bam &+Bof &+Cpro&+Wtes&+Ct, &+Bthe&+b sh&+Bape&+C of\n",
				             ch->in_room);
				send_to_room("&+Bt&+bhe &+Bfal&+Clen&+W Go&+Cd e&+Bxpl&+bode&+Bs i&+Cnto&+W je&+Ct s&+Btre&+bams&+B of&+C vi&+Wtal&+Cizi&+Bng &+bwat&+Ber,&+C sh&+Woot&+Cing&+B ab&+broa&+Bd i&+Cn\n",
				             ch->in_room);
				send_to_room("&+Ball&+b di&+Brec&+Ctio&+Wns &+Cand&+B en&+bgul&+Bfin&+Cg y&+Wour&+C al&+Blie&+bs w&+Bith&+C  t&+Whe &+Clon&+Bg f&+borg&+Bott&+Cen &+Wpow&+Cers&+B of&+b ol&+Bd.\n\n",
				             ch->in_room);
				...
				for (vict = world[ch->in_room].people; vict; vict = vict->next_in_room)
				{
					if (!affected_by_spell(vict, SPELL_VITALITY))
					{
						...
						{
							act("&+W$N&+L is &+Bvit&+Cal&+Wiz&+Ced by &+WTsu&+Bna&+bmi's&+W power&+W!", FALSE, ch, obj, vict, TO_ROOM);
					...
					else
					{
						...
						if (vict == ch)
						{
							act("&+bThe &+Bwater&+b from $q&+b flows around &+Wyou&+b, refreshing your &+Wvitality&+b!", TRUE, ch, obj, NULL, TO_CHAR);
						}
						else
						{
							act("&+bThe &+Bwater &+bfrom $q&+b flows around &+W$N&+b, refreshing your &+Wvitality&+b!", TRUE, ch, obj, vict, TO_CHAR);
						}

						act("&+bThe &+Bwater &+bfrom $q &+bflows around &+W$N&+b, refreshing your &+Wvitality&+b!", TRUE, ch, obj, vict, TO_ROOM);
					}
				}
			...
			{
				act("You &+Ltap and tap and &+Wtap but no &+Bwa&+bter's comin outta &+Wthis &+Bs&+Ch&+Wa&+Cf&+Bt.", TRUE, ch, obj, NULL, TO_CHAR);
			}
			return TRUE;
		}
		...
		if (cmd == CMD_THRUST)
		{
			...
			act("You thrust $q deep into the ground invoking the name of Poseidon, King of the Sea.", FALSE, ch, obj, 0, TO_CHAR);
			act("With great strength $n thrusts $q deep into the ground as $e invokes the name of Poseidon.", FALSE, ch, obj, 0, TO_ROOM);
		}
		else // cmd == CMD_RAISE
		{
			...
			act("You raise your $q skyward calling for the aid of Poseidon, ruler of the Sea.", FALSE, ch, obj, 0, TO_CHAR);
			act("$n raises $s $q skyward while shouting a prayer to Poseidon, ruler of the Sea.", FALSE, ch, obj, 0, TO_ROOM);
		}
...
int sevenoaks_longsword(P_obj obj, P_char ch, int cmd, char *arg)
{
	P_char                 vict;
	int                    i, room;
	struct damage_messages messages = {"Your $q &+rhums briefly as it unleashes a &+Cr&+Wai&+Cn of sharp &+Wic&+Ci&+Bc&+Cl&+Wes at $N!",
	...
	// 1/33 chance.
	if (!number(0, 32) && spell_damage(ch, vict, dice(10, 24), SPLDAM_COLD, SPLDAM_NOSHRUG | SPLDAM_NODEFLECT | RAWDAM_NOKILL, &messages) != DAM_NONEDEAD)
	{
		return TRUE;
	}
...
int dagger_of_wind(P_obj obj, P_char ch, int cmd, char *arg)
{
	P_char                 vict;
	int                    i, room;
	struct damage_messages messages = {"Your $q &+ccalls forth the &+Cessen&+Wce of the Wind &+cas you strike at $N...",
...
int sevenoaks_mace(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (OBJ_WORN_BY(obj, ch) && vict)
	{
		if (!number(0, 30))
		{
			act("Your $q &+Bvibrates then becomes very &+Cchilly!", FALSE, obj->loc.wearing, obj, vict, TO_CHAR);
			act("$n's $q &+Bvibrates violently, directing &+Ccold&+B energy at you!", FALSE, obj->loc.wearing, obj, vict, TO_VICT);
			act("$n's $q &+Bvibrates violently then explodes in &+Cchilly &+Bforce!", FALSE, obj->loc.wearing, obj, vict, TO_NOTVICT);
...
int tendrils(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	*/
	...
	act("$n sidesteps $N's lunge only to slam $s face with an elbow!", TRUE, ch, 0, vict, TO_NOTVICT);
	act("$n completely sidesteps your lunge only to slam $s elbow into your face!", TRUE, ch, 0, vict, TO_VICT);
	act("You completely sidestep $N's lunge, only to slam your elbow into $s face!", TRUE, ch, 0, vict, TO_CHAR);
	/*
	...
	*/
	if (!number(0, 7))
	{
		act("Before $N can recover $n grabs $M by the throat and utters a single &+Wword.\n"
		    "$N spasms in &+Rpain as tiny &+Bbolts of &+Blight surge through $S body!",
		    FALSE,
		    ch,
		    0,
		    vict,
		...
		act("Before your can recover $n grabs your throat and utters a single &+Wword.\n"
		    "You spasm in &+Rpain as tiny &+Bbolts of &+Blight surge through your body!",
		    TRUE,
		    ch,
		    0,
		    vict,
...
int Einjar(P_obj obj, P_char ch, int cmd, char *arg)
{
	...
	if (!number(0, 14) && GET_POS(vict) == POS_STANDING && !IS_TRUSTED(ch))
	{
		...
		act("You are knocked to the ground by $n's mighty bash!", FALSE, ch, 0, vict, TO_VICT);
		act("$N is knocked to the ground by $n's mighty bash!", FALSE, ch, 0, vict, TO_NOTVICT);
		SET_POS(vict, POS_SITTING + GET_STAT(vict));
		CharWait(vict, PULSE_VIOLENCE * 2);
	}
