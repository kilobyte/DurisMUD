...
void assign_innates()
{
	...
	// ADD_RACIAL_INNATE(INNATE_CALMING, RACE_HALFLING, 1);
	...
	ADD_RACIAL_INNATE(INNATE_GROUNDFIGHTING, RACE_DRIDER, 21);
	...
	ADD_RACIAL_INNATE(INNATE_GROUNDFIGHTING, RACE_TROLL, 21);
	...
	ADD_CLASS_INNATE(INNATE_OPHIDIAN_EYES, CLASS_DRAGOON, 1, 0);
}
...
void do_disappear(P_char ch, char *arg, int cmd)
{
	...
	{
		act("&+Gho ho ho, &+Rfun fun fun&+Y and still _not_&n", FALSE, ch, 0, 0, TO_CHAR);
	}
}
...
void do_throw_lightning(P_char ch, char *argument, int cmd)
{
	...
	act("&+BYou begin charging for an electric discharge!&N", FALSE, ch, 0, 0, TO_CHAR);
	act("&+BSparks begin to arc between $n and $N!&N", TRUE, ch, 0, GET_OPPONENT(ch), TO_ROOM);
...
void do_blast(P_char ch, char *arg, int cmd)
{
	...
	send_to_char("&+rYou feel something attempting to grab hold of your insides..\n", victim);
...
void do_reduce(P_char ch, char *arg, int cmd)
{
	...
	if (!affected_by_spell(ch, SPELL_REDUCE))
	{
		...
		act("You shrink to about half your normal size!", FALSE, ch, 0, NULL, TO_CHAR);
		act("$n shrinks to about half $s normal size!", TRUE, ch, 0, NULL, TO_ROOM);
	}
	CharWait(ch, PULSE_VIOLENCE);
}
...
void event_tempus(P_char ch, P_char victim, P_obj obj, void *args)
{
	...
	if (GET_CLASS(ch, CLASS_AVENGER) && !IS_SET(ch->specials.affected_by4, AFF4_HOLY_SACRIFICE))
	{
...
void do_doorkick(P_char ch, char *arg, int cmd)
{
	...
	{
		send_to_char("Um.. sorry, your rear legs just couldn't take the strain.\n", ch);
		return;
	}
	if (!ch->desc)
		return;
...
void do_stomp(P_char ch, char *arg, int cmd)
{
	...
	if (IS_PC(ch))
	{
		send_to_char("You stomp on the ground like a sumo wrestler..\n", ch);
		return;
	}
	else if (GET_RACE(ch) == RACE_TITAN || GET_RACE(ch) == RACE_AVATAR)
	{
		StompAttack(ch);
		if (IS_PC_PET(ch))
			CharWait(ch, PULSE_VIOLENCE);
...
void do_fade(P_char ch, char *argument, int cmd)
{
	...
	{
		send_to_char("You click your shoes together 3 times, but nothing happens.\n", ch);
		return;
	}

	if (IS_ROOM(ch->in_room, ROOM_NO_RECALL) || (world[ch->in_room].sector_type == SECT_OCEAN))
	{
