...
int list_weapons(P_char ch, P_ship ship, int owned)
{
	...
	for (int i = 0; i < MAXWEAPON; i++)
	{
		...
		send_to_char_f(ch,
		               " %-2d) &+%c%-20s &+Y%2d  %5s  %7s   %2d    %3d %3d%% %3d%% %3d%%    %3d  &n%10s&n\n",
		               i + 1,
...
int list_equipment(P_char ch, P_ship ship, int owned)
{
	...
	for (int i = 0; i < MAXEQUIPMENT; i++)
	{
		...
		send_to_char_f(ch,
		               ...
		               weight,
		               (cost == 0) ? "  &+WFREE" : coin_stringv(cost, 6));
	}
...
int list_hulls(P_char ch, P_ship ship, int owned)
{
	if (owned)
	{
		...
		for (int i = 0; i < MAXSHIPCLASS; i++)
		{
			...
			send_to_char_f(ch,
			               " %-2d) %-11s  &+Y%4d  &+Y%4d       &+Y%3d  &+Y%4d  &+Y%4d  &+Y%1d/&+Y%1d/&+Y%1d/&+Y%1d &+R%s&n %s\n",
			               i + 1,
			               SHIPTYPE_NAME(i),
			               ...
			               ship_arc_properties[i].max_weapon_slots[SIDE_FORE],
			               ship_arc_properties[i].max_weapon_slots[SIDE_STAR],
			               ship_arc_properties[i].max_weapon_slots[SIDE_REAR],
			               ship_arc_properties[i].max_weapon_slots[SIDE_PORT],
			               epic_cost_string(i),
			               coin_stringv(SHIPTYPE_COST(i), 6));
		}
...
int sell_cargo_slot(P_char ch, P_ship ship, int slot, int rroom)
{
	...
	sprintf(buf,
	        "CARGO: %s sold &+W%d&n %s&n at %s&n [%d] for %s&n (%d percent profit)",
	        GET_NAME(ch),
	        crates,
	        cargo_type_name(type),
	        ports[rroom].loc_name,
...
int sell_contra_slot(P_char ch, P_ship ship, int slot, int rroom)
{
	...
	sprintf(buf,
	        "CONTRABAND: %s sold &+W%d&n %s&n at %s&n [%d] for %s&n (%d percent profit)",
	        GET_NAME(ch),
	        crates,
	        contra_type_name(type),
	        ports[rroom].loc_name,
...
int sell_slot(P_char ch, P_ship ship, int slot)
{
	...
	{
		send_to_char_f(ch,
		               "Invalid slot number (%d is not between 1 and %d).\n"
	...
	else if (is_diplomat_slot(ship, slot))
	{
		...
		send_to_char_f(ch, "&+gYou get &n%s &+gfor the &n%s&+g.&n\n", (value == 0) ? "nothing" : coin_stringv(value), ship->slot[slot].get_description());
		ship->slot[slot].clear();
		ADD_MONEY(ch, value);
		update_ship_status(ship);
		write_ship(ship);
	...
	for (int slot = 0; slot < MAXSLOTS; slot++)
	{
		if (ship->slot[slot].type == SLOT_CONTRABAND)
		{
			...
			if (type < 0 || type >= NUM_PORTS)
				return;
			if (type == rroom)
				continue; // port does not confiscate its own contraband
			...
			else*/
			{
				...
				if (conf_chance > 100.0)
					conf_chance = 100.0;
				if (conf_chance < 0)
					conf_chance = 5.0; // always a small chance of being confiscated
			}
			...
			{
				sprintf(
					buf, "CONTRABAND: %s had &+W%d&n/&+W%d&n %s&n confiscated in %s [%d]", SHIP_OWNER(ship), confiscated, crates, contra_type_name(type), ports[rroom].loc_name, ports[rroom].loc_room);
				statuslog(56, buf);
				logit(LOG_SHIP, strip_ansi(buf).c_str());

				act_to_all_in_ship_f(ship, "&+L...&+Band find &+Y%d &+Bcrates of &n%s&+B, which they confiscate.&n", confiscated, contra_type_name(type));
...
int reload_ammo(P_char ch, P_ship ship, char *arg)
{
	...
	if (is_number(arg))
	{
		...
		{
			send_to_char_f(ch,
			               "Slot %d does not house a weapon.\n"
			               "&+YSyntax: '&+greload <&+Gall&+g|weapon slot>&+Y'.&n\n",
			               slot);
			return TRUE;
		}
...
int buy_weapon(P_char ch, P_ship ship, char *arg1, char *arg2)
{
	...
	{
		send_to_char("&+YSyntax: '&+gbuy &+Gw&+geapon <number> <&+Gf&+gore|&+Gr&+gear|&+Gs&+gtarboard|&+Gp&+gort>&+Y'.&n\n", ch);
		return TRUE;
	}
	 ...
	 */
	if (ship->frags < weapon_data[w].min_frags && ship->crew.guns_skill < weapon_data[w].min_crewexp)
	{
...
int ship_shop_proc(int room, P_char ch, int cmd, char *arg)
{
	...
	if ((cmd != CMD_SUMMON) && (cmd != CMD_LIST) && (cmd != CMD_BUY) && (cmd != CMD_RELOAD) && (cmd != CMD_REPAIR) && (cmd != CMD_SELL))
	{
		return FALSE;
	}
...
int crew_shop_proc(int room, P_char ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_LIST)
	{
		...
		for (int i = n = 0; i < MAXCREWS; i++)
		{
			...
			{
				send_to_char_f(ch,
				               "&+Y%2d)&n %s &+C%2d&n &+W%4d&n/&+W%4d&n/&+W%4d&n   &+G%5d&n    &+W%c%-2d&n/ &+W%c%-2d&n/ &+W%c%-2d   &+r%7d &n%s\n",
				               ++n,
				               pad_ansi(ship_crew_data[i].name, 30).c_str(),
				               ship_crew_data[i].level + 1,
				               ship_crew_data[i].base_sail_skill,
				               ship_crew_data[i].base_guns_skill,
				               ship_crew_data[i].base_rpar_skill,
				               ship_crew_data[i].base_stamina,
				               (ship_crew_data[i].sail_mod > 0) ? '+' : ' ',
				               ship_crew_data[i].sail_mod,
				               (ship_crew_data[i].guns_mod > 0) ? '+' : ' ',
				               ship_crew_data[i].guns_mod,
				               (ship_crew_data[i].rpar_mod > 0) ? '+' : ' ',
				               ship_crew_data[i].rpar_mod,
				               ship_crew_data[i].hire_frags,
	...
	if (cmd == CMD_HIRE)
	{
		...
		for (int i = 0; i < MAXCREWS; i++)
		{
			if (ship_crew_data[i].hire_room(world[room].number) && ++c == n)
			{
				...
				if (ship->frags < ship_crew_data[i].hire_frags && (ship->crew.sail_skill < ship_crew_data[i].base_sail_skill || ship->crew.guns_skill < ship_crew_data[i].base_guns_skill ||
		...
		for (int i = 0; i < MAXCHIEFS; i++)
		{
			if (ship_chief_data[i].hire_room(world[room].number) && ++c == n)
			{
				...
				if (ship->frags < ship_chief_data[i].hire_frags && ((ship_chief_data[i].type == SAIL_CHIEF && ship->crew.sail_skill < ship_chief_data[i].min_skill) ||
