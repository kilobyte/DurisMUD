...
char *where_obj(P_obj w_obj, int flag)
{
	...
	if (OBJ_ROOM(w_obj))
	{
		snprintf(GS_buf1 + strlen(GS_buf1),
		         MAX_STRING_LENGTH - strlen(GS_buf1),
		         "in [&+R%4d&+W:&+C%6d&n] &n%s",
		         ROOM_ZONE_NUMBER(w_obj->loc.room),
		         world[w_obj->loc.room].number,
		         world[w_obj->loc.room].name);
		return (GS_buf1);
	}
	...
	{
		snprintf(GS_buf1 + strlen(GS_buf1), MAX_STRING_LENGTH - strlen(GS_buf1), "&+Yinside &n%s&+Y, ", w_obj->loc.inside->short_description);
 ...
 */
void sa_intCopy(P_char ch, unsigned long offset, int value) { bcopy((char *)&value, (char *)ch + offset, sizeof(int)); }
...
void do_reboot_restore(P_char ch, P_char victim)
{
	...
	if (affected_by_spell(victim, SPELL_DISEASE) || affected_by_spell(victim, SPELL_PLAGUE) || affected_by_spell(victim, SPELL_BMANTLE) || affected_by_spell(victim, SPELL_FLAMESTRIKE))
	{
...
void do_read_player(P_char ch, char *arg, int cmd)
{
  ...
  if((tmp = restorePasswdOnly(vict, arg)) < 0)
  {
	...
	logit(LOG_WIZ, "%s loaded %s's char into the game [%d]", GET_NAME(ch), GET_NAME(vict), world[ch->in_room].number);
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if (d->descriptor == sdesc)
		{
			if (!d->character || CAN_SEE(ch, d->character))
			{
				...
				if (GET_LEVEL(ch) < 62)
				{
					wizlog(GET_LEVEL(ch), "%s just released socket %d.", GET_NAME(ch), sdesc);
...
void do_trans(P_char ch, char *argument, int cmd)
{
	...
	if (str_cmp("all", buf))
	{
		...
		logit(LOG_WIZ, "%s transferred %s from %d to %d", ch->player.name, victim->player.name, world[old_room].number, world[victim->in_room].number);
	...
	{ /*
	   ...
	   */
		...
		for (i = descriptor_list; i; i = i->next)
		{
			...
			if (i->character != ch && !i->connected && (GET_LEVEL(i->character) <= level))
			{
		...
		logit(LOG_WIZ, "%s transferred all to %d", ch->player.name, world[ch->in_room].number);
...
void do_at(P_char ch, char *argument, int cmd)
{
	...
	else if ((target_mob = get_char_vis(ch, loc_str)))
	{
		...
		{
			logit(LOG_DEBUG, "%s in NOWHERE!!", (IS_PC(target_mob) ? GET_NAME(target_mob) : target_mob->player.short_descr));
...
void do_affectpurge(P_char ch, char *argument, int cmd)
{
	...
	else
	{
		...
		spell = old_search_block(rest, 1, ((uint)(MAX(0, qend - 1))), (const char **)spells, 0);
		if (spell != -1)
			spell--;
		if (spell == -1)
		{
	...
	if (victim && spell != 0)
	{
		act("&+W$n&+W waves $s mighty hand over your body...", FALSE, ch, 0, victim, TO_VICT);
		act("&+WYou waves your mighty hand over $N&+W's body...", FALSE, ch, 0, victim, TO_CHAR);
		act("&+W$n&+W waves $s mighty hand over $N&+W's body...", FALSE, ch, 0, victim, TO_NOTVICT);
...
void stat_dam(P_char ch, char *arg)
{
	...
	for (race = 1; race <= LAST_RACE; race++)
	{
		...
		snprintf(tmplate, 512, buf, race_name, (int)pulse, multiplier, mult_mod, (damcap > 1) ? 'C' : 'c', damcap, (int)(damcap * damroll_cap));
...
void stat_spldam(P_char ch, char *arg)
{
	...
	for (race = 1; race <= LAST_RACE; race++)
	{
		...
		{
			snprintf(buf, 512, "damage.spellTypeMod.offensive.racial.%s.%s", race_names_table[race].no_spaces, spldam_types[type]);
		...
		for (type = 0; type < LAST_SPLDAM_TYPE; type++)
		{
			snprintf(buf, 512, "damage.spellTypeMod.defensive.racial.%s.%s", race_names_table[race].no_spaces, spldam_types[type]);
...
void stat_game(P_char ch)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		{
			snprintf(buf + strlen(buf), MAX_STRING_LENGTH - strlen(buf), "%2d%%/%3d  %s", (int)((race[i + 1] / x) * 100 + .5), (int)race[i + 1], pad_ansi(race_names_table[i + 1].ansi, 15).c_str());
		}
		else if (i < (LAST_RACE - 1))
		{
			snprintf(buf + strlen(buf), MAX_STRING_LENGTH - strlen(buf), " 0%%/  0  %s", pad_ansi(race_names_table[i + 1].ansi, 15).c_str());
		}
		else
			strcat(buf + strlen(buf), "               ");
		...
		{
			snprintf(buf + strlen(buf), MAX_STRING_LENGTH - strlen(buf), "      %10d%%/%3d  %s\n", (int)((m_class[i + 1] / x) * 100 + .5), (int)m_class[i + 1], class_names_table[i + 1].ansi);
		}
...
void do_stat(P_char ch, char *argument, int cmd)
{
	...
	if ((*arg1 == 'r') || (*arg1 == 'R'))
	{
		...
		else
		{
			...
			if (!is_number(arg2) || ((i = real_room(atoi(arg2))) < 0) || (i > top_of_world))
			{
		...
		snprintf(o_buf, MAX_STRING_LENGTH, "&+YRoom: [&N%d&+Y](&N%d&+Y)  Zone: &N%d&+Y  Sector type: &N%s\n", rm->number, i, zone_table[rm->zone].number, buf2);
		...
		snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "&+YSpecial procedure:&N %s\n", (rm->funct) ? get_function_name((void *)rm->funct) : "None");
		...
		snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "&+YSection: &N%d  &+YX = &N%d  &+YY = &N%d  &+YZ = &N%d&N\n", rm->map_section, rm->x_coord, rm->y_coord, rm->z_coord);
		...
		for (i = 0; i <= (NUM_EXITS - 1); i++)
		{
			if (rm->dir_option[i])
			{
				...
				snprintf(buf,
				         ...
				         rm->dir_option[i]->key,
				         buf2,
				         (rm->dir_option[i]->to_room != NOWHERE) ? world[rm->dir_option[i]->to_room].number : -1,
	...
	else if ((*arg1 == 'o') || (*arg1 == 'O') || (*arg1 == 'i') || (*arg1 == 'I'))
	{
		...
		*/
		...
		snprintf(
			o_buf, MAX_STRING_LENGTH, "&+YObject:\n&+YNumber: [&N%d&+Y](&N%d&+Y)  Type: &N%s  &+YName: &N%s\n", m_virtual, j->R_num, buf2, ((j->short_description) ? j->short_description : "None"));

		snprintf(
			o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "&+YKeywords: &N%s\n&+YLong description:\n%s\n", ((j->name) ? j->name : "None"), ((j->description) ? j->description : "None"));
		...
		{
			case ITEM_LIGHT:
				snprintf(buf, MAX_STRING_LENGTH, "&+YColor: [&N%d&+Y]  Type: [&N%d&+Y]  Hours: [&N%d&+Y]", j->value[0], j->value[1], j->value[2]);
				break;
			case ITEM_POTION:
			case ITEM_SCROLL:
				...
				for (i = 1; (i < 4) && (j->value[i] > 0); i++)
				{
					...
					snprintf(buf, MAX_STRING_LENGTH, "%s%d) &+C%s [%d]&+Y, ", buf, j->value[i], buf2, GetCircle(j->value[i]));
				}
			...
			case ITEM_WAND:
				if (j->value[3] > 0)
					...
					strcpy(buf2, "&+RBUGGED!&");
				snprintf(buf, MAX_STRING_LENGTH, "%d(%d)&+Y charges, Level &N%d&+Y spell: %d) &+C%s [%d]&N", j->value[1], j->value[2], j->value[0], j->value[3], buf2, GetCircle(j->value[3]));
				break;
			case ITEM_FIREWEAPON:
				...
				snprintf(buf, MAX_STRING_LENGTH, "&+YRange: &N%d  &+YRate of fire: &N%d  &+YMissile type: &N%d", j->value[1], j->value[0], j->value[3]);
				break;
			case ITEM_WEAPON:
			{
				int  spells[3];
				...
				if (obj_index[j->R_num].func.obj == NULL && j->value[5])
				{
					...
					if (spells[1] && skills[spells[1]].name)
						snprintf(spell_list + strlen(spell_list), MAX_STRING_LENGTH - strlen(spell_list), "&n, &+W%s", skills[spells[1]].name);
					if (spells[2] && skills[spells[2]].name)
						snprintf(spell_list + strlen(spell_list), MAX_STRING_LENGTH - strlen(spell_list), "&n, &+W%s", skills[spells[2]].name);

					if (j->value[5] / 1000000000)
						snprintf(buf1, MAX_STRING_LENGTH, "&+YProcs one of &+W%d&+Y level &+W%s &+Yat &+W1/%d&+Y chance\n", j->value[6], spell_list, j->value[7]);
					else
						snprintf(buf1, MAX_STRING_LENGTH, "&+YProcs all of &+W%d&+Y level &+W%s &+Yat &+W1/%d&+Y chance\n", j->value[6], spell_list, j->value[7]);
				}
				else
					*buf1 = 0;
				snprintf(buf, MAX_STRING_LENGTH, "%s&+YType: &n%s &+Ydice: &N%dD%d&N %s", buf1, buf2, j->value[1], j->value[2], j->value[4] ? "&+g(Poisoned)&n" : "");
				break;
			}
			case ITEM_QUIVER:
				snprintf(
					buf, MAX_STRING_LENGTH, "&+YMax Capacity: &N%d  &+YCurrent No. Arrows. &N%d  &+YContainer Flags: &N%d  &+YMissile type: &N%d", j->value[0], j->value[3], j->value[1], j->value[2]);
				break;
			case ITEM_MISSILE:
				if ((j->value[3] < 1) || (j->value[3] > 6))
					strcpy(buf2, "&+RBUGGED!&N");
					...
					sprinttype(j->value[3] - 1, shot_types, buf2);
				snprintf(buf, MAX_STRING_LENGTH, "&+YDamage: &N%dd%d&N &+YMissile Type: &n%s", j->value[1], j->value[2], missile_types[j->value[3] - 1].flagLong);
				break;
			case ITEM_ARMOR:
			...
			case ITEM_CONTAINER:
			case ITEM_STORAGE:
				snprintf(buf, MAX_STRING_LENGTH, "&+YHolds: &N%d  &+YLocktype: &N%d  &+YKey: &N%d  &+YSize hold: &N%d", j->value[0], j->value[1], j->value[2], j->value[3]);
				break;
			case ITEM_CORPSE:
				if (IS_SET(j->value[1], PC_CORPSE))
					snprintf(buf, MAX_STRING_LENGTH, "&+mPlayer Corpse&n &+YHolding:&n %d &+Ylbs&N", j->value[0]);
				else
					snprintf(buf, MAX_STRING_LENGTH, "&+bNPC Corpse&n&+Y (&n%d&+Y) Holding:&n %d &+Ylbs&n", j->value[3], j->value[0]);
				break;
			case ITEM_DRINKCON:
				sprinttype(j->value[2], drinks, buf2);
				snprintf(buf, MAX_STRING_LENGTH, "&+YHolds: &N%d  &+YContains:&N %d  &+YPoisoned:&N %d  &+YLiquid:&N %s", j->value[0], j->value[1], j->value[3], buf2);
				break;
			case ITEM_NOTE:
			...
			case ITEM_KEY:
				...
				break;
			case ITEM_FOOD:
				snprintf(buf, MAX_STRING_LENGTH, "&+YMakes full:&N %d  &+YPoisoned:&N %d", j->value[0], j->value[3]);
				break;
			case ITEM_MONEY:
				snprintf(buf, MAX_STRING_LENGTH, "&+YCopper:&N %d  &+YSilver:&N %d  &+YGold:&N %d  &+YPlatinum:&N %d", j->value[0], j->value[1], j->value[2], j->value[3]);
				break;
			case ITEM_WORN:
				snprintf(buf, MAX_STRING_LENGTH, "&+rWarmth:&N %d  &+YPrestige:&N %d  &+YMaterial:&n %d", j->value[1], j->value[2], j->value[3]);
				break;
			case ITEM_TELEPORT:
				...
				snprintf(buf,
				         ...
				         j->value[0],
				         ((i > 1) && (i <= top_of_world)) ? world[real_room(j->value[0])].name : "",
				         j->value[1],
				         j->value[2],
				         j->value[3]);
				break;
		...
		*/
		...
		if (j->affects)
		{
			...
			for (o_af = j->affects; o_af; o_af = o_af->next)
			{
				if (o_af->extra2)
				{
					snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "   &n%s &+Yfor&n %d &+Ygranting:&n ", skills[o_af->type].name, (int)o_af->data);
					sprintbitde(o_af->extra2, extra2_bits, o_buf + strlen(o_buf));
				}
				else
					snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "   &n%s &+Yfor&n %d&n", skills[o_af->type].name, (int)o_af->data);
				strcat(o_buf, "\n");
			}
		}
		if (j->events || j->nevents)
		{
			...
			LOOP_EVENTS_OBJ(ne, j->nevents)
			{
				...
				{
					snprintf(
						o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "  &+YOffending mob: &n%s&N %d&+Y.\n", (ne->ch) ? J_NAME(ne->ch) : "NULL", IS_ALIVE(ne->ch) ? GET_ID(ne->ch) : -1);
				}
			}
			strcat(o_buf, "\n");
		}
	...
	else if ((*arg1 == 'c') || (*arg1 == 'C') || (*arg1 == 'm') || (*arg1 == 'M'))
	{
		...
		if (is_number(arg2))
		{
			...
			if (!t_mob)
			{
				logit(LOG_DEBUG, "do_stat(): mob %d [%d] not loadable", i, mob_index[i].virtual_number);
		...
		snprintf(buf1,
		         ...
		         zone_table[world[k->in_room].zone].number,
		         world[k->in_room].zone,
		         zone_table[world[k->in_room].zone].name);
		snprintf(buf2, MAX_STRING_LENGTH, "%s %s%s  ", buf, (IS_PC(k) ? "&+YPC" : (IS_PC(k) ? "&+RNPC" : "&+GMOB")), (t_mob) ? "" : buf1);
		if (IS_NPC(k))
		{
			snprintf(buf,
			         MAX_STRING_LENGTH,
			         "Numbers: &N%d&+Y-V &N%d&+Y-R &N%d&+Y-I   # in game: &n%d\n",
			         mob_index[GET_RNUM(k)].virtual_number,
			         GET_RNUM(k),
			         GET_IDNUM(k),
		...
		else
		{
			...
			snprintf(buf2, MAX_STRING_LENGTH, "&+Y Exp to Level: &N%s", IS_TRUSTED(k) ? "Unknown" : buf1);
		}
		...
		snprintf(buf, MAX_STRING_LENGTH, "&+YRace: &N%s  &+YClass: &N", race_names_table[k->player.race].ansi);
		...
		snprintf(buf,
		         ...
		         (int)k->specials.base_combat_round,
		...
		if (IS_PC(k))
		{
			...
			snprintf(buf, MAX_STRING_LENGTH, "&+YAge: &N%4d &+Yyears  &N%2d &+Ymonths  &N%2d &+Ydays  &N%2d &+YHours\n", age(k).year, age(k).month, age(k).day, age(k).hour);
			strcat(o_buf, buf);
		...
		snprintf(buf, MAX_STRING_LENGTH, "&+YAgi: &n%3d&+Y (&n%3d&+Y)    Wis: &n%3d&+Y (&n%3d&+Y)    Size: &n%s&+Y\n", GET_C_AGI(k), k->base_stats.Agi, GET_C_WIS(k), k->base_stats.Wis, buf2);
		strcat(o_buf, buf);
		...
		i = GET_C_STR(k) + GET_C_DEX(k) + GET_C_AGI(k) + GET_C_CON(k) + GET_C_POW(k) + GET_C_INT(k) + GET_C_WIS(k) + GET_C_CHA(k);

		i2 = k->base_stats.Str + k->base_stats.Dex + k->base_stats.Agi + k->base_stats.Con + k->base_stats.Pow + k->base_stats.Int + k->base_stats.Wis + k->base_stats.Cha;

		snprintf(buf, MAX_STRING_LENGTH, "&+YAvg: &n%3d&+Y (&n%3d&+Y)  Total mod: (&n%3d&+Y)              Load modifer: &n%3d\n\n", (int)(i / 8), (int)(i2 / 8), (i - i2), load_modifier(k));
		strcat(o_buf, buf);
		...
		{
			snprintf(buf, MAX_STRING_LENGTH, "&+mSpells left in circles:  (%d to regain)\n&+m", k->specials.undead_spell_slots[0]);
			strcat(o_buf, buf);

			for (i4 = 1; i4 < MAX_CIRCLE + 1; i4++)
			{
				snprintf(buf, MAX_STRING_LENGTH, "%d:%d/%d", i4, k->specials.undead_spell_slots[i4], spl_table[GET_LEVEL(k)][i4 - 1]);
				snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "%-8s", buf);
			}
			strcat(o_buf, "\n\n");
		}
		snprintf(buf, MAX_STRING_LENGTH, "&+YHits: [&N%5d&+Y/&N%5d&+Y/&N%5d&+Y+&N%3d&+Y]&+W   Pcoins: &N%5d", GET_HIT(k), GET_MAX_HIT(k), k->points.base_hit, hit_regen(k, TRUE), GET_PLATINUM(k));
		if (IS_PC(k))
		...
		snprintf(buf, MAX_STRING_LENGTH, "&+YMana: [&N%5d&+Y/&N%5d&+Y/&N%5d&+Y+&N%3d&+Y]   Gcoins: &N%5d", GET_MANA(k), GET_MAX_MANA(k), k->points.base_mana, mana_regen(k, TRUE), GET_GOLD(k));

		if (IS_PC(k))
			snprintf(buf, MAX_STRING_LENGTH, "%s  &+YGbank: &N%5d\n", buf, GET_BALANCE_GOLD(k));
		else
			snprintf(buf, MAX_STRING_LENGTH, "%-50s  &+YSpecial: &N%s\n", buf, (mob_index[GET_RNUM(k)].func.mob ? get_function_name((void *)mob_index[GET_RNUM(k)].func.mob) : "None"));
		strcat(o_buf, buf);

		snprintf(buf, MAX_STRING_LENGTH, "&+YMove: [&N%5d&+Y/&N%5d&+Y/&N%5d&+Y+&N%3d&+Y]&n   Scoins: %5d", GET_VITALITY(k), GET_MAX_VITALITY(k), vitality_limit(k), move_regen(k, TRUE), GET_SILVER(k));
		if (IS_PC(k))
		...
		else if ((qi = find_quester_id(GET_RNUM(k))) >= 0)
		{
			snprintf(buf,
			         ...
			         "%-52s  &+YQuest: &N%s\n",
			         buf,
			         mob_index[GET_RNUM(k)].qst_func ? (has_quest_complete(qi) ? "&+BComplete&N" : (has_quest_ask(qi) ? "&+RAsk&N" : "&+RRoomMsg&n")) : "None");
		}
		strcat(o_buf, buf);

		snprintf(buf, MAX_STRING_LENGTH, "                                &+yCcoins: &N%5d", GET_COPPER(k));
		if (IS_PC(k))
		...
		snprintf(buf, MAX_STRING_LENGTH, "&+Y thAC0: &N%d &+Y  +Hit: &N%d", i2, GET_HITROLL(k) + str_app[STAT_INDEX(GET_C_STR(k))].tohit);
		if (IS_NPC(k) || (GET_CLASS(k, CLASS_MONK) && !k->equipment[WIELD] && !k->equipment[WEAR_SHIELD] && !k->equipment[HOLD] && !k->equipment[SECONDARY_WEAPON]))
		{
		...
		{
			snprintf(buf, MAX_STRING_LENGTH, "%s  &+Y+Dam: &N%d+%d = %d\n", buf, GET_DAMROLL(k), str_app[STAT_INDEX(GET_C_STR(k))].todam, TRUE_DAMROLL(k));
		}
		strcat(o_buf, buf);
		...
		snprintf(buf1, MAX_STRING_LENGTH, "%s  &+YFighting:&n %s", buf, ((GET_OPPONENT(k)) ? GET_NAME(k->specials.fighting) : "---"));
		if (IS_NPC(k))
		{
			strcat(buf1, "\n");
			strcpy(buf, buf1);
		...
		{
			snprintf(buf, MAX_STRING_LENGTH, "  %s\n", IS_NPC(fol->follower) ? fol->follower->player.short_descr : GET_NAME(fol->follower));
			strcat(o_buf, buf);
		}
		...
		if (IS_PC(k))
		{
			snprintf(buf,
			         ...
			         k->only.pc->pc_timer[0],
			         k->only.pc->pc_timer[1],
			         k->only.pc->pc_timer[2],
			         k->only.pc->pc_timer[3],
			         k->only.pc->pc_timer[4],
			         k->only.pc->pc_timer[5],
			         k->only.pc->pc_timer[6],
			         k->only.pc->pc_timer[7],
			         k->only.pc->pc_timer[8],
		...
		if (k->affected)
		{
			...
			for (aff = k->affected; aff; aff = aff->next)
			{
				...
				snprintf(buf,
				         ...
				         IS_SET(aff->flags, AFFTYPE_NOAPPLY) ? "NONE" : apply_types[(int)aff->location],
				         aff->modifier,
				         aff->duration,
			...
			LOOP_EVENTS_CH(ne, k->nevents)
			{
				...
				if (ne->func == event_short_affect)
					snprintf(o_buf + strlen(o_buf),
					         MAX_STRING_LENGTH - strlen(o_buf),
					         " - %s&+Y.\n",
					         (ne->data == NULL || ((event_short_affect_data *)ne->data)->af == NULL) ? "No affect" : skills[((event_short_affect_data *)ne->data)->af->type].name);
				else
		...
		if (k->linking || k->linked || k->obj_linked)
		{
			...
			{
				snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "%s (%s): &+Ylinked to&n %s.\n", link_types[link->type].name, "master", link->linked->player.name);
			}
			for (link = k->linked; link; link = link->next_linked)
			{
				snprintf(o_buf + strlen(o_buf), MAX_STRING_LENGTH - strlen(o_buf), "%s (%s): &+Ylinked to&n %s.\n", link_types[link->type].name, "slave", link->linking->player.name);
			}
			for (olink = k->obj_linked; olink; olink = olink->next)
			{
				snprintf(o_buf + strlen(o_buf),
				         ...
				         OBJ_SHORT(olink->obj),
				         (olink->affect == NULL) ? "no affect" : ((skills[olink->affect->type].name) ? skills[olink->affect->type].name : "Nameless Type"));
			}
			strcat(o_buf, "\n");
		}
		if (IS_PC(k))
	...
	else if ((*arg1 == 's') || (*arg1 == 'S'))
	{
		...
		else
		{
			...
			if (((mob_index[GET_RNUM(k)].func.mob != shop_keeper) && (mob_index[GET_RNUM(k)].qst_func != shop_keeper)) || (i >= number_of_shops))
			{
		...
		snprintf(o_buf,
		         ...
		         shop_index[i].shop_is_roaming ? "Roaming " : "",
		         i,
		         mob_index[GET_RNUM(k)].virtual_number,
		         GET_RNUM(k),
		         k->player.short_descr ? k->player.short_descr : "&+rNone");
		snprintf(o_buf + strlen(o_buf),
		         ...
		         shop_index[i].open2,
		         shop_index[i].close2,
		         shop_index[i].shop_killable ? 'Y' : 'N',
		...
		snprintf(o_buf + strlen(o_buf),
		         ...
		         (int)(shop_index[i].sell_percent * 100),
		         num_pr,
		         num_tr);
		...
		****/
		...
		for (i2 = 0; i2 < shop_index[i].number_items_produced; i2++)
		{
			if (shop_index[i].producing[i2] != -1)
			{
				...
				{
					logit(LOG_DEBUG, "do_stat(): obj %d [%d] not loadable (shop stat)", shop_index[i].producing[i2], obj_index[shop_index[i].producing[i2]].virtual_number);
...
void do_echoe(P_char ch, char *arg, int cmd)
{
	...
	else
	{
		...
		for (d = descriptor_list; d; d = d->next)
		{
			if (d->connected == CON_PLAYING)
			{
				if ((EVIL_RACE(d->character) || IS_TRUSTED(d->character)) && (!(IS_RACEWAR_UNDEAD(d->character))))
				{
					if (GET_LEVEL(d->character) >= level)
					{
...
void do_shutdow(P_char ch, char *argument, int cmd) { send_to_char("If you want to shut something down - say so!\n", ch); }
...
void do_wizlock(P_char ch, char *arg, int cmd)
{
	...
	if (!*arg || !str_cmp(arg, "?"))
	{
		send_to_char_f(ch,
		               ...
		               YESNO(IS_SET(game_locked, LOCK_MAX_PLAYERS)),
		               number_of_players(),
		               game_locked_players,
		               YESNO(IS_SET(game_locked, LOCK_LEVEL)),
		               game_locked_level);
	...
	if (ch->desc)
	{
		if (IS_TRUSTED(ch))
		{
			if (((*argument == 'g') || (*argument == 'G')) && (*(argument + 1) == ' '))
				good = TRUE;
			else if (((*argument == 'e') || (*argument == 'E')) && (*(argument + 1) == ' '))
				evil = TRUE;
			else if (((*argument == 'u') || (*argument == 'U')) && (*(argument + 1) == ' '))
				undead = TRUE;
			else if (((*argument == 'n') || (*argument == 'N')) && (*(argument + 1) == ' '))
				neutral = TRUE;
			else if (((*argument == 'a') || (*argument == 'A')) && (*(argument + 1) == ' '))
			{
...
void do_jestros(P_char ch, char *argument, int cmd)
{
	...
	for (i = descriptor_list; i; i = i->next)
	{
		...
		gmcp_comm_channel_ex(to, "jchat", PERS(ch, to, FALSE), argument, good ? "good" : evil ? "evil" : undead ? "undead" : "neutral");
	}

	gmcp_comm_channel_ex(ch, "jchat", GET_NAME(ch), argument, good ? "good" : evil ? "evil" : undead ? "undead" : "neutral");
...
void do_wizmsg(P_char ch, char *arg, int cmd)
{
	...
	for (d = descriptor_list; d; d = d->next)
	{
		...
		if ((d->connected == CON_PLAYING) && (GET_LEVEL(toChar) >= min_level) && !PLR_FLAGGED(toChar, PLR_WIZMUFFED))
		{
	...
	else
	{
		...
		else
		{
			...
			if (IS_TRUSTED(ch) && !IS_FIGHTING(ch) && !IS_DESTROYING(ch))
			{
				...
				{
					act("$n's &+Wyeyes&n slowly &+wglaze&n over, and then $n slowly fades out &+wof view&+L...&n", FALSE, ch, 0, 0, TO_ROOM);
...
void do_force(P_char ch, char *argument, int cmd)
{
	...
	else if (str_cmp("all", name))
	{
		...
		{
			send_to_room("&+yThe ground begins to shake...&n &=LBLightning&n&+B streaks towards your head...&n\n", ch->in_room);
			act("And Fafhrd turns to $n with a wicked grin... 'I don't think so'", FALSE, ch, 0, 0, TO_ROOM);
			send_to_char("And Fafhrd turns to you with a wicked grin... 'I don't think so'\n", ch);
			return;
		}
		else
		{
			...
			else
			{
				...
				logit(LOG_FORCE, "%s has forced %s to '%s' [%d/%d]", GET_NAME(ch), GET_NAME(vict), to_force, world[ch->in_room].number, world[vict->in_room].number);
...
void do_load(P_char ch, char *argument, int cmd)
{
	...
	if (is_abbrev(type, "char") || is_abbrev(type, "mobile"))
	{
		...
		if (!mob)
		{
			logit(LOG_DEBUG, "do_load(): mob %d [%d] not loadable", r_num, mob_index[r_num].virtual_number);
			return;
		}
		...
		if (GET_LEVEL(ch) < OVERLORD)
			wizlog(GET_LEVEL(ch), "%s loaded mob %s '%s' in [%d]", GET_NAME(ch), num, GET_NAME(mob), world[ch->in_room].number);
		logit(LOG_WIZLOAD, "%s loaded mob %s '%s' in [%d]", GET_NAME(ch), num, GET_NAME(mob), world[ch->in_room].number);
	...
	else if (is_abbrev(type, "object") || is_abbrev(type, "item"))
	{
		...
		if (!obj)
		{
			logit(LOG_DEBUG, "do_load(): obj %d [%d] not loadable", r_num, obj_index[r_num].virtual_number);
			return;
		}
		if (GET_LEVEL(ch) < OVERLORD)
			wizlog(GET_LEVEL(ch), "%s loaded obj %s '%s' in [%d]", GET_NAME(ch), num, obj->short_description, world[ch->in_room].number);
		logit(LOG_WIZLOAD, "%s loaded obj %s '%s' [%d]", GET_NAME(ch), num, obj->short_description, world[ch->in_room].number);
...
void do_purge(P_char ch, char *argument, int cmd)
{
	...
	if (*name)
	{
		if ((vict = get_char_room_vis(ch, name)))
		{
			...
			wizlog(GET_LEVEL(ch), "%s has purged %s [%d]", GET_NAME(ch), GET_NAME(vict), world[ch->in_room].number);
		...
		else if ((obj = get_obj_in_list_vis(ch, name, world[ch->in_room].contents)))
		{
			...
			wizlog(GET_LEVEL(ch), "%s has purged %s [%d]", GET_NAME(ch), obj->short_description, world[ch->in_room].number);
			logit(LOG_WIZ, "%s has purged %s [%d]", GET_NAME(ch), obj->short_description, world[ch->in_room].number);
		...
		else if (!str_cmp("pfiles", name))
		{
			...
			while (fscanf(flist, " %s \n", buf) != EOF)
			{
				...
				if (((timegone > 1440) && ((timegone / 1440) > 60)) && (GET_LEVEL(vict) <= 56))
				{
	...
	{ /* no argument. clean out the room */
		...
		for (obj = world[ch->in_room].contents; obj; obj = next_o)
		{
			...
			if (IS_ARTIFACT(obj))
			{
				act("Skipping artifact $p.  You must purge that item by name if you want it gone.", FALSE, ch, obj, 0, TO_ROOM);
				continue;
			}
...
void NewbySkillSet(P_char ch, bool fullReset)
{
	...
	for (i = FIRST_SKILL; i <= LAST_SKILL; i++)
	{
		...
		if (SKILL_DATA_ALL(ch, i).rlevel[0] > 0 && SKILL_DATA_ALL(ch, i).rlevel[0] <= GET_LEVEL(ch) && !IS_SPELL(i))
		{
...
void do_start(P_char ch, int nomsg)
{
	...
	if (!(GET_CLASS(ch, CLASS_AVENGER | CLASS_DREADLORD)) && !((GET_RACE(ch) == RACE_DUERGAR || GET_RACE(ch) == RACE_MOUNTAIN) && GET_CLASS(ch, CLASS_BERSERKER)))
	{
	...
	if (nomsg != CMD_MULTICLASS)
	{
		...
		ch->only.pc->prompt = (PROMPT_HIT | PROMPT_MAX_HIT | PROMPT_MOVE | PROMPT_MAX_MOVE | PROMPT_TANK_NAME | PROMPT_TANK_COND | PROMPT_ENEMY | PROMPT_ENEMY_COND | PROMPT_TWOLINE | PROMPT_STATUS);
	}
...
void do_advance(P_char ch, char *argument, int cmd)
{
	...
	if (newlevel >= MINLVLIMMORTAL && oldlevel < MINLVLIMMORTAL)
	{
		send_to_char("A sense of timelessness overwhelms you.  "
		             ...
		             victim);
		victim->specials.act |= (PLR_PETITION | PLR_AGGIMMUNE | PLR_WIZLOG | PLR_STATUS | PLR_VNUM | PLR_NAMES);
		REMOVE_BIT(victim->specials.act, PLR_ANONYMOUS);
		victim->only.pc->prompt |= (PROMPT_VIS);
	}

	wizlog(GET_LEVEL(ch), " %s has advanced %s to level %d", GET_NAME(ch), GET_NAME(victim), GET_LEVEL(victim));
	logit(LOG_WIZ, "%s advanced %s to level %d", GET_NAME(ch), GET_NAME(victim), GET_LEVEL(victim));
...
void do_reroll(P_char ch, char *argument, int cmd)
{
	...
	else
	{
		snprintf(buf,
		         ...
		         GET_NAME(victim),
		         (GET_C_STR(victim) + GET_C_DEX(victim) + GET_C_AGI(victim) + GET_C_CON(victim) + GET_C_POW(victim) + GET_C_INT(victim) + GET_C_WIS(victim) + GET_C_CHA(victim)) / 8,
		         GET_C_STR(victim),
		         GET_C_DEX(victim),
		         GET_C_AGI(victim),
		         GET_C_CON(victim),
...
void do_zreset(P_char ch, char *argument, int cmd)
{
	...
	else if (!str_cmp(arg, "ok"))
	{
		...
		wizlog((GET_LEVEL(ch) > MAXLVLMORTAL) ? GET_LEVEL(ch) : MINLVLIMMORTAL, "%s just reset the zone %s.", GET_NAME(ch), zone_struct->name);
...
void do_reinitphys(P_char ch, char *arg, int cmd)
{
	...
	else if (!str_cmp("all", buf))
	{
		for (p = descriptor_list; p; p = p->next)
			if (!p->connected)
			{
				...
				send_to_char("&+LA haze of powdery dust falls from the heavens, clogging your breathing, choking your lungs, blurring your vision. As it begins to subside, you realize all your "
	...
	act("Your height and weight have been randomly rolled again by $N!", TRUE, vict, 0, ch, TO_CHAR);
...
void do_silence(P_char ch, char *argument, int cmd)
{
	...
	{
		send_to_char("You can use communications channels again...but don't piss off the gods again!\n", vict);
	...
	{
		send_to_char("&+WThe gods take away your ability to use ANY communications channel!\n", vict);
...
void do_demote(P_char ch, char *argument, int cmd)
{
	...
	snprintf(buf, MAX_STRING_LENGTH, "You have just demoted %s to level 1!  Ain't they hating..\n", GET_NAME(victim));
	send_to_char(buf, ch);
...
void do_teleport(P_char ch, char *argument, int cmd)
{
	...
	if ((GET_LEVEL(victim) > MIN(62, GET_LEVEL(ch))) && (!IS_NPC(victim) || (GET_LEVEL(ch) < 60)))
	{
	...
	logit(LOG_WIZ, "%s teleported %s from %d to %d", ch->player.name, victim->player.name, world[old_room].number, world[victim->in_room].number);
	sql_log(ch, WIZLOG, "Teleported %s from %d to %d", victim->player.name, world[old_room].number, world[victim->in_room].number);
}
...
void do_lookup(P_char ch, char *argument, int cmd)
{
	...
	if (!*arg || !*pattern)
	{
		send_to_char("Syntax : lookup <pfile | room | zone | mob | obj | random> <pattern>\n", ch);
		return;
	}
	/*
			...
			if (restoreCharOnly(owner, skip_spaces(Gbuf2)) >= 0)
			{
				...
				{ // LEVEL
					...
					{ // CLASS
						...
						{ // RACE
							how_many++;
							snprintf(buf, MAX_STRING_LENGTH, "%-12s %d\t %-10s\t %-10s\n", GET_NAME(owner), GET_LEVEL(owner), get_class_string(owner, buffer), race_to_string(owner));
							send_to_char(buf, ch);
							if (how_many > 20)
							{
...
void do_ptell(P_char ch, char *arg, int cmd)
{
	...
	*/
	...
	for (d = descriptor_list; d; d = d->next)
	{
		if ((STATE(d) == CON_PLAYING) && IS_TRUSTED(d->character) && IS_SET(d->character->specials.act, PLR_PETITION) && (d->character != vict) && (d->character != ch))
		{
			snprintf(Gbuf1, MAX_STRING_LENGTH, "&+rPetition: %s responds to %s with '&+R%s&n&+r'.\n", CAN_SEE(d->character, ch) ? GET_NAME(ch) : "Someone", GET_NAME(vict), msg);
			send_to_char(Gbuf1, d->character, LOG_PRIVATE);

			/* Send GMCP to other gods monitoring petitions */
			{
				char sender_label[MAX_NAME_LENGTH + MAX_NAME_LENGTH + 8];
				snprintf(sender_label, sizeof(sender_label), "%s -> %s", CAN_SEE(d->character, ch) ? GET_NAME(ch) : "Someone", GET_NAME(vict));
...
void GetMIA(char *playerName, char *returned)
{
	...
	if (minutesgone > 0)
	{
		...
		if (minutesgone > 1440)
		{
			snprintf(returned + strlen(returned), MAX_STRING_LENGTH - strlen(returned), "%ld day%s%s", (minutesgone / 1440), ((minutesgone / 1440) > 1) ? "s" : "", (minutesgone % 1440) ? ", " : "");
		}
		// % 1440 -> removes days.  .. / 60 -> hours MIA.
		if ((minutesgone % 1440) / 60 > 0)
		{
			snprintf(returned + strlen(returned),
			         MAX_STRING_LENGTH - strlen(returned),
			         "%ld hour%s%s",
			         (minutesgone % 1440) / 60,
...
void GetMIA2(char *playerName, char *returned)
{
	...
	snprintf(returned + strlen(returned), MAX_STRING_LENGTH - strlen(returned), " - %ld mud hour%s.", timegone / SECS_PER_MUD_HOUR, (timegone / SECS_PER_MUD_HOUR) > 1 ? "s" : "");
}
...
void do_finger(P_char ch, char *arg, int cmd)
{
	...
	snprintf(Gbuf1,
	         ...
	         GET_NAME(finger_foo),
	         GET_TITLE(finger_foo),
	         GET_LEVEL(finger_foo),
	...
	if (in_game)
	{
		...
		int hours = (timegone / 3600), minutes = (timegone % 3600) / 60, seconds = (timegone % 60);
	...
	snprintf(Gbuf1,
	         ...
	         world[real_room0(GET_HOME(finger_foo))].name,
	         GET_HOME(finger_foo),
	...
	for (d = descriptor_list; d; d = d->next)
		if (STATE(d) == CON_ACCEPTWAIT && d->character && !str_cmp(GET_NAME(d->character), f_a))
		{
			if (!arg || !*arg)
			{
				...
				SEND_TO_Q("that either your name does not suit fantasy theme, or something else is\n"
...
void do_approve(P_char ch, char *arg, int cmd)
{
	...
	switch (search_block(arg, approve_modes, FALSE))
	{
		case 0:
			...
			for (d1 = descriptor_list; d1; d1 = d1->next)
			{
				if ((d1->connected == CON_PLAYING) && d1->character && IS_SET(d1->character->specials.act, PLR_PETITION) && IS_TRUSTED(d1->character))
				{
		...
		default:
			for (d1 = descriptor_list; d1; d1 = d1->next)
			{
				if (STATE(d1) == CON_ACCEPTWAIT && d1->character && !str_cmp(GET_NAME(d1->character), arg))
				{
					...
					{
						if ((d2->connected == CON_PLAYING) && d2->character && IS_SET(d2->character->specials.act, PLR_PETITION) && IS_TRUSTED(d2->character))
						{
...
void do_clone(P_char ch, char *argument, int cmd)
{
	...
	if (is_abbrev(type, "mobile") || is_abbrev(type, "character"))
	{
		...
		{
			wizlog(GET_LEVEL(ch), "%s just cloned %s %d times [&+C%d&N]", GET_NAME(ch), mob->player.short_descr, count, world[ch->in_room].number);
			logit(LOG_WIZ, "%s cloned %s %d times [&+C%d&N]", GET_NAME(ch), mob->player.short_descr, count, world[ch->in_room].number);
	...
	else if (is_abbrev(type, "object"))
	{
		...
		if (IS_TRUSTED(ch) && GET_LEVEL(ch) < OVERLORD)
		{
			wizlog(GET_LEVEL(ch), "%s just cloned %s %d times [&+C%d&N]", GET_NAME(ch), obj->short_description, count, world[ch->in_room].number);
			logit(LOG_WIZ, "%s cloned %s %d times [&+C%d&N]", GET_NAME(ch), obj->short_description, count, world[ch->in_room].number);
...
void do_inroom(P_char ch, char *args, int cmd)
{
	...
	*p = 's';
	...
	for (v = world[ch->in_room].people; v; v = v_next)
	{
		...
		if ((!CAN_SEE(ch, v)) || (IS_PC(v) && (target == 2)) || (IS_NPC(v) && (target == 1)) || (ch == v))
			continue;
...
bool check_flagsde(int *value, const flagDef flagNames[], const char *flagName)
{
	...
	for (i = 0; (flagNames[i].flagShort != NULL) && str_cmp(flagNames[i].flagShort, flagName); i++)
		;
...
void do_which(P_char ch, char *args, int cmd)
{
	...
	else if ((*arg1 == 'o') || (*arg1 == 'O') || (*arg1 == 'i') || (*arg1 == 'I'))
	{
		...
		for (t_obj = object_list; t_obj; t_obj = t_obj->next)
		{
			...
			if (found && !stat_check)
			{
				snprintf(buf1, MAX_STRING_LENGTH, "[%5d] %-30s - %s\n", obj_index[t_obj->R_num].virtual_number, t_obj->short_description, where_obj(t_obj, FALSE));
			...
			else if (found && stat_check)
			{
				if (sc_min <= t_obj->affected[j].modifier && sc_max >= t_obj->affected[j].modifier)
				{
					...
					snprintf(buf1, MAX_STRING_LENGTH, "[%5d] %-30s - %s\n%s\n", obj_index[t_obj->R_num].virtual_number, t_obj->short_description, where_obj(t_obj, FALSE), temp);
					o_len += strlen(buf1);
					if (o_len > MAX_STRING_LENGTH)
					{
						strcat(o_buf, "And so on, and so forth...\n");
	...
	else if ((*arg1 == 'c') || (*arg1 == 'C') || (*arg1 == 'm') || (*arg1 == 'M'))
	{
		...
		for (t_ch = character_list; t_ch; t_ch = t_ch->next)
		{
			...
			{
				if (IS_NPC(t_ch))
					snprintf(buf1, MAX_STRING_LENGTH, "%-30s- &+Y[&n%5d&+Y]&n %s\n", t_ch->player.short_descr, world[t_ch->in_room].number, world[t_ch->in_room].name);
				else
...
{
	char   victname[MAX_INPUT_LENGTH], cmdname[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH];
	int   *new_arr, i, s;
	P_char vict;
	...
	if (!cmdname[0])
	{
		...
		else
		{
			...
			for (i = 0; i < s; i++)
			{
				snprintf(buf2, MAX_STRING_LENGTH, "[&+Y%d&n] &+c%-20s&n", cmd_info[vict->only.pc->gcmd_arr[i]].minimum_level, command[vict->only.pc->gcmd_arr[i] - 1]);
	...
	logit(LOG_WIZ, "%s granted %s the '%s' (%d) command.", GET_NAME(ch), GET_NAME(vict), command[cmd - 1], cmd_info[cmd].minimum_level);
	wizlog(GET_LEVEL(ch), "%s granted %s the '%s' (%d) command.", GET_NAME(ch), GET_NAME(vict), command[cmd - 1], cmd_info[cmd].minimum_level);
	sql_log(ch, WIZLOG, "Granted %s the '%s' (%d) command.", GET_NAME(vict), command[cmd - 1], cmd_info[cmd].minimum_level);
}
...
void do_revoke(P_char ch, char *args, int cmd)
{
	...
	for (i = 0; i < s; i++)
	{
		if (vict->only.pc->gcmd_arr[i] == cmd)
		{
			...
			logit(LOG_WIZ, "%s revoked '%s' (%d) command from %s.", GET_NAME(ch), command[cmd - 1], cmd_info[cmd].minimum_level, GET_NAME(vict));
			wizlog(GET_LEVEL(ch), "%s revoked '%s' (%d) command from %s.", GET_NAME(ch), command[cmd - 1], cmd_info[cmd].minimum_level, GET_NAME(vict));
			sql_log(ch, WIZLOG, "Revoked '%s' (%d) command from %s.", command[cmd - 1], cmd_info[cmd].minimum_level, GET_NAME(vict));
...
void do_terminate(P_char ch, char *argument, int cmd)
{
	...
	act("&+rYou smile as $N will never again challenge your greatness!", FALSE, ch, 0, victim, TO_CHAR);

	act("&+r$n is surrounded by a red aura, and $s &+Weyes&n &+rglow.", FALSE, ch, 0, victim, TO_NOTVICT);
	act("&+r$n raises $s hands and points them at $N and a&n &+Lblack ", FALSE, ch, 0, victim, TO_NOTVICT);
	act("&+Lbeam&n &+r shoots out to strike at $N!", FALSE, ch, 0, victim, TO_NOTVICT);
	act("&+r$N cries out and $E shatters into a million bits!", FALSE, ch, 0, victim, TO_NOTVICT);

	act("&+r$n is surrounded by a red aura, and $s &+Weyes&n &+rglow.", FALSE, ch, 0, victim, TO_VICT);
	act("&+r$n raises $s hands and points them at YOU! and a&n &+Lblack ", FALSE, ch, 0, victim, TO_VICT);
	act("&+Lbeam&n &+r shoots out to strike at YOU!", FALSE, ch, 0, victim, TO_VICT);
	act("&+rYou feel your soul being invaded by $n as $e begins to", FALSE, ch, 0, victim, TO_VICT);
	act("&+runravel the mysteries of your existance! You feel every part", FALSE, ch, 0, victim, TO_VICT);
	act("&+rof your body being torn to shreds, and you suddenly cry out in", FALSE, ch, 0, victim, TO_VICT);
	act("&+rutter terror as $n stands over the remains of you in righteous", FALSE, ch, 0, victim, TO_VICT);
	act("&+rjustice!  You fall away into complete&n &+Lblackness.......", FALSE, ch, 0, victim, TO_VICT);

	statuslog(ch->player.level, "%s's existence on Duris was just terminated by %s!", GET_NAME(victim), GET_NAME(ch));

	logit(LOG_WIZ, "%s's existence on Duris was just terminated by %s!", GET_NAME(victim), GET_NAME(ch));
...
void do_sacrifice(P_char ch, char *argument, int cmd)
{
	...
	act("$n whips out a huge dagger out of nowhere and stabs you in the heart!", FALSE, ch, 0, victim, TO_VICT);
	act("You conjure a dagger out of nowhere and stab $N in the heart!", FALSE, ch, 0, victim, TO_CHAR);
	act("$n whips out a huge dagger out of nowhere and stabs $N in the heart!", FALSE, ch, 0, victim, TO_NOTVICT);
	statuslog(ch->player.level, "%s was just sacrificed by %s.", GET_NAME(victim), GET_NAME(ch));
...
int vnum_mobile(char *searchname, struct char_data *ch)
{
	...
	for (i = 0; i <= top_of_mobt; i++)
		if ((t_mob = read_mobile(mob_index[i].virtual_number, VIRTUAL)))
		{
			...
			snprintf(buf, MAX_STRING_LENGTH, "%6d  %5d  %-s\n", mob_index[i].virtual_number, mob_index[i].number - 1, (t_mob->player.short_descr) ? t_mob->player.short_descr : "None");
			count++;
			if (t_mob)
			{
				extract_char(t_mob);
			...
			logit(LOG_DEBUG, "vnum_mobile(): mob %d not loadable", mob_index[i].virtual_number);
	strcat(buff, "\n");
	snprintf(buff + strlen(buff), MAX_STRING_LENGTH - strlen(buff), "&+LTotal mobs of this name in database:&n %d\n", count);
...
void do_storage(P_char ch, char *arg, int cmd)
{
	...
	if (!strcmp(subcmd, "new"))
	{
		...
		if (s_obj->type == ITEM_STORAGE)
		{
			...
			wizlog(GET_LEVEL(ch), "%s loads %s as a storage unit in room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			logit(LOG_WIZ, "%s loads %s as a storage unit in room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			return;
		}
		else
		{
	...
	{
		if ((s_obj = get_obj_in_list(objarg, world[ch->in_room].contents)) && (s_obj->type == ITEM_STORAGE))
		{
			extract_obj(s_obj);
			PurgeSavedItemFile(s_obj);
			wizlog(GET_LEVEL(ch), "%s deletes storage item %s from room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			logit(LOG_WIZ, "%s deletes storage item %s from room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			return;
		}
		else
		{
	...
	else if (!strcmp(subcmd, "remove"))
	{
		if ((s_obj = get_obj_in_list(objarg, world[ch->in_room].contents)) && (s_obj->type == ITEM_STORAGE))
		{
			...
			extract_obj(s_obj, FALSE);
			PurgeSavedItemFile(s_obj);
			wizlog(GET_LEVEL(ch), "%s removes storage item %s from room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			logit(LOG_WIZ, "%s remove storage item %s from room %d.", J_NAME(ch), s_obj->short_description, world[ch->in_room].number);
			return;
		}
		else
		{
...
void do_givepet(P_char ch, char *arg, int cmd)
{
	...
	if (*buf && *pet)
	{
		...
		else if (isdigit(*pet))
		{
			...
			logit(LOG_WIZ, "(%s) has loaded pet (%s)(Level: %d) for (%s).", GET_NAME(ch), mob->player.short_descr, GET_LEVEL(mob), GET_NAME(victim));
		}
		else
		{
...
void do_petition_block(P_char ch, char *argument, int cmd)
{
	...
	else if (IS_SET(vict->specials.act2, PLR2_B_PETITION))
	{
		...
		wizlog(GET_LEVEL(ch), "%s just removed the petition block on %s.", GET_NAME(ch), GET_NAME(vict));
		logit(LOG_WIZ, "%s just removed the petition block on %s.", GET_NAME(ch), GET_NAME(vict));
	...
	else
	{
		...
		wizlog(GET_LEVEL(ch), "%s was just PETITION BLOCKED by %s.", GET_NAME(vict), GET_NAME(ch));
...
void do_questwhere(P_char ch, char *arg, int cmd)
{
	...
	for (obj = object_list; obj && !stop; obj = obj->next)
	{
		...
		{
			snprintf(buf2, MAX_STRING_LENGTH, "%3d. [%6d] %s - ", ++count, obj_index[obj->R_num].virtual_number, pad_ansi(obj->short_description, 40).c_str());
...
bool is_quested_item(P_obj obj)
{
	...
	for (qcp = quest_index[0].quest_complete; count < number_of_quests; qcp = quest_index[++count].quest_complete)
	{
	...
	for (i = 0; i < top_of_mobt; i++)
	{
		...
		if (mobRace == raceIndex)
		{
			...
			snprintf(buf,
			         ...
			         ++count,
			         (mobIndex->func.mob == NULL) ? ' ' : '*',
			         mobVnum,
			         pad_ansi((mobIndex->desc2 == NULL) ? "(NULL)" : mobIndex->desc2, 30, TRUE).c_str(),
			         (mobIndex->qst_func == NULL) ? ' ' : 'Q',
			         mobIndex->number,
			         mobIndex->limit,
			         zone_table[mobZone].name);
...
void stat_single_race(P_char ch, int race)
{
	...
	snprintf(buf,
	         ...
	         pad_ansi(race_names_table[race].ansi, 15).c_str(),
	         race,
	         race_names_table[race].normal,
	...
	snprintf(buf,
	         ...
	         stat_factor[race].Str,
	         stat_factor[race].Pow,
	         stat_factor[race].Dex,
	         stat_factor[race].Int,
	         stat_factor[race].Agi,
	         stat_factor[race].Wis,
	         stat_factor[race].Con,
	         stat_factor[race].Cha,
	         stat_factor[race].Kar,
	         stat_factor[race].Luk,
	         combat_by_race[race][0],
	         spell_pulse_data[race],
	         combat_by_race[race][1],
	         combat_by_race[race][2]);
	send_to_char(buf, ch);
	...
	snprintf(buf,
	         ...
	         racial_data[race].base_age,
	         racial_data[race].max_age,
	         racial_data[race].hp_bonus,
	         racial_data[race].base_vitality,
	         racial_data[race].base_mana,
	         racial_data[race].max_mana,
	         racial_values[race][0],
	         racial_values[race][1]);
	send_to_char(buf, ch);

	snprintf(buf, MAX_STRING_LENGTH, "&+MShrug&n: &+c%2d&n, ExpFactor: &+c%1.3f&n, VictimExpFactor: &+c%1.3f&n\n\r", racial_shrug_data[race], racial_exp_mods[race], racial_exp_mod_victims[race]);
	send_to_char(buf, ch);
...
void stat_zone(P_char ch, char *arg)
{
	...
	if (get_zone_info(zone->number, &zinfo))
	{
		...
		snprintf(o_buf + strlen(o_buf),
		         ...
		         YESNO(zinfo.task_zone),
		         YESNO(zinfo.quest_zone),
		         YESNO(zinfo.trophy_zone));
		...
		for (int rw = 1; rw <= MAX_RACEWAR; rw++)
		{
			snprintf(o_buf + strlen(o_buf),
			         ...
			         racewar_color[rw].color,
			         racewar_color[rw].name,
			         zdata->players[rw],
			         YESNO(zdata->misfiring[rw]));
		}
	}
...
void do_offlinemsg(P_char ch, char *arg, int cmd)
{
	...
	for (d = descriptor_list; d; d = d->next)
	{
		...
		if (IS_NPC(recipient))
		{
			debug("do_offlinemsg: NPC char '%s' %d on descriptor list??", GET_NAME(recipient), (recipient->only.npc != NULL) ? GET_VNUM(recipient) : -1);
			continue;
		}
...
#define OBJ_COLOR(rnum) ((obj_index[rnum].number <= 1) ? "+L" : (obj_index[rnum].number - 1 > obj_index[rnum].limit) ? "+W" : (obj_index[rnum].number - 1 == obj_index[rnum].limit) ? "n" : "+w")
...
void which_stat(P_char ch, char *argument)
{
	...
	if (greater)
	{
		...
		for (int r_num = 0; r_num <= top_of_objt; r_num++)
		{
			...
			for (i = 0; i < MAX_OBJ_AFFECT; i++)
			{
				if (obj->affected[i].location == flags && obj->affected[i].modifier >= amount)
				{
...
void where_nowhere(P_char ch, char *args)
{
	...
	for (count = 0, obj = object_list; obj; obj = obj->next)
	{
		...
		if (OBJ_NOWHERE(obj) || (OBJ_ROOM(obj) && (ROOM_VNUM(obj->loc.room) == NOWHERE)) || (OBJ_WORN(obj) && (obj->loc.wearing == NULL)) || (OBJ_CARRIED(obj) && (obj->loc.carrying == NULL)) ||
...
int where_compare(const void *line1, const void *line2) { return ((line_info *)line1)->room_number > ((line_info *)line2)->room_number; }
...
void do_where(P_char ch, char *argument, int cmd)
{
	...
	if (!*argument)
	{
		...
		for (d = descriptor_list; d; d = d->next)
		{
			...
			if (t_ch && IS_PC(t_ch) && (d->connected == CON_PLAYING) && (d->character->in_room != NOWHERE) && CAN_SEE(ch, t_ch))
			{
				if (d->original) /* If switched */
				{
					...
					snprintf(lines[line_count].line,
					         ...
					         IS_TRUSTED(t_ch) ? 'w' : racewar_color[GET_RACEWAR(t_ch)].color,
					         t_ch->player.name,
	...
	if (isname(argument, "evils") || isname(argument, "goods") || isname(argument, "undeads") || isname(argument, "neutrals"))
	{
		int racewar = 0;
		...
		for (d = descriptor_list; d; d = d->next)
		{
			...
			if (t_ch && IS_PC(t_ch) && (d->connected == CON_PLAYING) && (d->character->in_room != NOWHERE) && CAN_SEE(ch, t_ch) && t_ch->player.racewar == racewar)
			{
				if (d->original) /* If switched */
				{
					snprintf(lines[line_count].line,
	...
	if (is_number(argument))
	{
		...
		for (k = object_list; k && !flag; k = k->next)
		{
			if (v_num == OBJ_VNUM(k))
			{
				...
				if ((OBJ_WORN(tobj) && WIZ_INVIS(ch, tobj->loc.wearing)) || (OBJ_CARRIED(tobj) && WIZ_INVIS(ch, tobj->loc.carrying)))
				{
					continue;
				}
	...
	if (is_abbrev(argument, "zone"))
	{
		...
		for (d = descriptor_list; d; d = d->next)
		{
			if (d->character && IS_PC(d->character) && (d->connected == CON_PLAYING) && (d->character->in_room != NOWHERE) && CAN_SEE(ch, d->character))
			{
				if (world[d->character->in_room].zone == world[ch->in_room].zone)
				{
					...
					{
						snprintf(buf2, MAX_STRING_LENGTH, "%-20s &+Y- &n[&+C%6d&n] %s&n\n", d->character->player.name, world[d->character->in_room].number, world[d->character->in_room].name);
					}
					if (strlen(buf2) + length + 35 > MAX_STRING_LENGTH)
					{
	...
	for (i = character_list; i && !flag; i = i->next)
	{
		if ((isname(argument, GET_NAME(i)) || (IS_NPC(i) && isname(argument, i->player.short_descr))) && CAN_SEE(ch, i))
		{
			if ((i->in_room != NOWHERE) && (IS_TRUSTED(ch) || (world[i->in_room].zone == world[ch->in_room].zone)))
			{
	...
	for (k = object_list; k && !flag; k = k->next)
	{
		if ((isname(argument, k->name) || isname(argument, k->short_description)) && CAN_SEE_OBJ(ch, k))
		{
			...
			if (OBJ_CARRIED(tobj) && WIZ_INVIS(ch, tobj->loc.carrying))
				continue;
			if ((k->affected[0].location == APPLY_LEVEL || k->affected[1].location == APPLY_LEVEL) && GET_LEVEL(ch) <= 60)
				continue;
...
void which_weapon(P_char ch, char *argument)
{
	...
	if (is_abbrev(argument, "twohanded") || is_abbrev(argument, "two-handed") || is_abbrev(argument, "twohands") || is_abbrev(argument, "two-hands"))
	{
		type = -1;
	}
	...
	for (int r_num = 0; r_num <= top_of_objt; r_num++)
	{
		...
		if ((obj->type == ITEM_WEAPON) && ((type == -1 && IS_SET(obj->extra_flags, ITEM_TWOHANDS)) || (obj->value[0] == type)))
		{
			snprintf(buf,
			         MAX_STRING_LENGTH,
			         "%3d) &%s%4d/%4d %6d&n %s&n - %s.\n",
...
void stat_skill(P_char ch, char *arg)
{
	...
	for (int cls = 1; class_names_table[cls].code != NULL; cls++)
	{
		...
		{
			send_to_char_f(ch, "Class: %s : @lvl %d Max %d.\n", class_names_table[cls].code, skills[skl].m_class[cls - 1].rlevel[0], skills[skl].m_class[cls - 1].maxlearn[0]);
		}
...
void remove_account_char(P_char ch, P_char target) { send_to_char("Not supported yet.\n", ch); }

void add_account_char(P_char ch, P_char target) { send_to_char("Not supported yet.\n", ch); }

void change_account_email(P_char ch, P_char target, const char *newEmail) { send_to_char("Not supported yet.\n", ch); }

void lookup_account(P_char ch, const char *arguments) { send_to_char("Not supported yet.\n", ch); }
