...
struct melee_death_messages
{
	...
	// HIT
	{					   "Your fearsome punch hits $N, smashing $M to death.","$n's final punch sends you to meet your maker.",                                                    "$n punches $N to death."                                                                                 },
	{					 "You hit $N in the throat, $E chokes, gasps and dies.", "$n smashes your throat.  Gasping and choking, you descend into darkness.",                    "$n punches $N in the throat, $E chokes, gasps and dies."},
	// BLUDGEON
	{								   "You bludgeon $N, killing $M instantly.",                                       "You are bludgeoned to death by $n.",                                       "$n ruthlessly bludgeons $N to death."},
	{								"Your wicked bludgeon crushed $N to death.",             "A wicked bludgeon from $n sends you spiraling into darkness.",           "A blood spray erupts from $N as $E is bludgeoned to death by $n."},
	// PIERCE
	{			 "You successfully stab $N in the heart, killing $M instantly.",
     "$n stabs you directly in the heart, killing you instantly.",   "$n stabs the heart of $N who falls to the ground clutching at the wound."																						   },
	{    "Your stab cuts a vital artery and causes $N to fall before your feet.",
     "$n's stab causes massive blood-loss and you fall to the ground, lifeless.",         "$n stabs viciously at $N, whose lifeless body falls to the ground."																			},
	// SLASH
	{					 "You beautifully slash $N into two parts - both dead.",        "Your upper body is disconnected from your legs as $n slashes you.",                 "$N is slashed into two by a master stroke performed by $n."},
	{			  "Your final slash sends $N's head bouncing along the ground.",   "Your vision spins into darkness as your head falls towards the ground.",                         "$n neatly performs a coup de grace and beheads $N."},
	// WHIP
	{		   "Your whip opens a fatal wound and $N falls to the ground dead.",    "$n's lashing whip causes immense blood-loss, effectively killing you.",                             "With a final stunning whip, $n ends $N's life."},
	{		  "The crack of your whip is the last thing $N hears before death.",            "A stinging whip from $n spills your entrails onto the ground.",                         "The gash opened up by $n's whip fatally wounds $N."},
	// CLAW
	{	   "Your claw opens a fatal wound, and $N gives up the fight for good.",
     "$n's claw finds your vital organs and rips them out, killing you instantly.",                  "With a final swipe of a claw, $n ends $N's life for good."																		  },
	{					"You swipe a vicious claw at $N, causing a fatal gash.",                  "A fearsome swipe of a claw from $n ends your existence.",                 "A final fearsome swipe of a claw from $n kills $N quickly."},
	// BITE
	{	   "Your bite opens a fatal wound, and $N gives up the fight for good.",                                     "$n's final bite crushes your throat.",                                     "With a final chomp, $n ends $N's life."},
	{	  "You bite down hard upon $N and have the satisfaction of killing $M!",
     "A final bite from $n chews through you and your life slips away.",     "$n nearly chomps $N in half with a wicked bite, gruesomely killing $M."																					 },
	// STING
	{		   "Your sting finally reached a vital organ killing $N instantly.",
     "You begin to blackout as overwhelming pain expands outward from $n's sting.",                        "$n's sting reached $N's heart killing $M instantly."																		  },
	{																		  0,																		  0,																			0},
	// CRUSH
	{"Your final crushing blow caves in $N's chest, and $E dies rather quickly.",
     "$n crushes your chest, and all your hopes of escaping this battle alive.", "Snapping ribs can be heard as $n caves in $N's chest with a fearsome blow."                                                                             },
	{							"You crushed $N's skull, I'm afraid $E's dead.",       "$n's crushing blow to your head makes you see stars, then nothing.",                  "$n crushes $N's skull.  $N instantly collapses, lifeless."},
	// MAUL
	{				   "Your maul hits $N squarely, smashing the life from $M.",                "$n's final devastating maul sends you to meet your maker.",                                "$n smashes $N to death with a vicious maul."},
	{																		  0,																		  0,																			0},
	// THRASH
	{				  "$N's chest falls into itself as your hooves land on it!",                                 "$n's hooves crush your chest completely.",                    "$N's chest falls into itself as $n's hooves land on it!"},
	{																		  0,																		  0,																			0},
	// TOUCH
	{					"$N's life is drained away by your supernatural touch!",                    "$n's touch drains the last life force from your body!",              "$N's touch leaves behind a lifeless husk of what once was $n!"},
};
...
int proccing_slots[] = {PRIMARY_WEAPON, SECONDARY_WEAPON, THIRD_WEAPON, FOURTH_WEAPON, WEAR_SHIELD,    WEAR_HANDS,     WEAR_HANDS_2,  WEAR_BODY,     WEAR_ABOUT,    WEAR_IOUN, WEAR_HEAD,
                        WEAR_HORN,      WEAR_NECK_1,      WEAR_NECK_2,  WEAR_ARMS,     WEAR_ARMS_2,    WEAR_WRIST_LR,  WEAR_WRIST_LL, WEAR_FINGER_R, WEAR_FINGER_L, WEAR_NOSE, WEAR_TAIL,
                        WEAR_LEGS,      WEAR_FEET,        WEAR_EYES,    WEAR_FACE,     WEAR_EARRING_R, WEAR_EARRING_L, WEAR_WAIST,    WEAR_QUIVER,   GUILD_INSIGNIA};

int on_front_line(P_char);

// LAST_SPLDAM_TYPE == number of types, range 0 .. LAST_SPLDAM_TYPE-1
const char *spldam_types[LAST_SPLDAM_TYPE] = {"generic", "fire", "cold", "lightning", "gas", "acid", "negative", "holy", "psionic", "spiritual", "sound", "earth"};
...
int check_damage_ward(P_char attacker, P_char ch, int dam)
{
	...
	while (paf && absorbed < dam)
	{
		...
		act("&+CThe ward around you flashes briefly as it absorbs $n&+C's assault!&n", FALSE, attacker, 0, ch, TO_VICT | ACT_NOTTERSE);
		act("&+CThe ward around&n $N&+C flashes briefly as it absorbs your assault!&n", FALSE, attacker, 0, ch, TO_CHAR | ACT_NOTTERSE);
		act("&+CThe ward around&n $N&+C flashes briefly as it absorbs &n$n&+C's assault!&n", FALSE, attacker, 0, ch, TO_NOTVICT | ACT_NOTTERSE);
	}
...
bool rapier_dirk(P_char victim, P_char attacker)
{
	...
	if (rapier_dirk_check(victim) && chance > number(1, 1000) && MIN_POS(victim, POS_STANDING + STAT_NORMAL) && !IS_DRAGON(attacker) && GET_OPPONENT(victim) == attacker)
	{
		if (number(0, 1))
		{
			if (GET_CLASS(victim, CLASS_BARD) || GET_SPEC(victim, CLASS_ROGUE, SPEC_THIEF))
			{
				act("$n's $q &=LCflashes&n into the path of $N's attack, then $n delivers a graceful counter-attack!", TRUE, victim, wep1, attacker, TO_NOTVICT);
				act("With &=LWunbelievable speed&n, $n's $q intercepts your attack, and then $n steps wide to deliver a graceful counter-attack!", TRUE, victim, wep1, attacker, TO_VICT);
				act("With superb grace and ease, you intercept $N's attack with $q and counter-attack!", TRUE, victim, wep1, attacker, TO_CHAR);
			}
			else
			{
				act("$n's $q &=LCflashes&n into the path of $N's attack, then $n delivers a graceful counter-attack!", TRUE, victim, wep2, attacker, TO_NOTVICT);
				act("With &=LWunbelievable speed&n, $n's $q intercepts your attack, and then $n steps wide to deliver a graceful counter-attack!", TRUE, victim, wep2, attacker, TO_VICT);
				act("With superb grace and ease, you intercept $N's attack with $q and counter-attack!", TRUE, victim, wep2, attacker, TO_CHAR);
			}
		}
		else
		{
			act("$n's intercepts $N's attack with $q with eloquent ease.", TRUE, victim, wep1, attacker, TO_NOTVICT);
			act("$n's skillfully shifts $s stance and $q &+Wglistens&n with insane speed to thwart your attack!", TRUE, victim, wep1, attacker, TO_VICT);
			act("You play with $N before lunging into an offensive routine!", TRUE, victim, wep1, attacker, TO_CHAR);
		}
		...
		{
			act("$p slices through the air with incredible ease!", TRUE, victim, wep1, attacker, TO_CHAR);
			act("$p slices through the air with incredible ease!", TRUE, victim, wep1, attacker, TO_ROOM);
...
bool opposite_racewar(P_char ch, P_char victim) { return IS_PC(ch) && IS_PC(victim) && GET_RACEWAR(ch) != GET_RACEWAR(victim); }
...
int vamp(P_char ch, double fhits, double fcap)
{
	...
	else if ((af = get_spell_from_char(ch, SPELL_BMANTLE)) != NULL || (af = get_spell_from_char(ch, SPELL_FLAMESTRIKE)) != NULL)
	{
	...
	if (IS_PC(ch) && IS_AFFECTED3(ch, AFF3_PALADIN_AURA) && has_aura(ch, AURA_HEALING))
	{
		...
		if (!IS_SET(ch->specials.affected_by4, AFF4_REGENERATION) && !affected_by_spell(ch, SPELL_ACCEL_HEALING) && !affected_by_skill(ch, SKILL_REGENERATE))
		{
...
bool soul_trap(P_char ch, P_char victim)
{
	...
	{
		if (!notch_skill(ch, SKILL_SOUL_TRAP, get_property("skill.notch.soulTrap", 1)) && number(1, 100) >= GET_CHAR_SKILL(ch, SKILL_SOUL_TRAP) / 3)
			return false;
		himself = true;
	}
	else
	{
		for (gl = ch->group; gl; gl = gl->next)
		{
			if (GET_CHAR_SKILL(gl->ch, SKILL_SOUL_TRAP) &&
			    (notch_skill(gl->ch, SKILL_SOUL_TRAP, get_property("skill.notch.soulTrap", 100) * 3) || GET_CHAR_SKILL(gl->ch, SKILL_SOUL_TRAP) / 5 > number(1, 100)))
			{
				ch = gl->ch;
				break;
			}
	...
	if (himself)
	{
		act("&+LThe room seems to darken as&n $n &+Rslams&n $p &+Lthrough&n $N's &+Lchest!&n", FALSE, ch, 0, victim, TO_ROOM);
		act("&+LAs $E slumps to the ground, bleeding, screaming, begging for mercy,&n $n &+Ltwist the weapon and draws the soul from the dying body.", FALSE, ch, 0, victim, TO_ROOM);
		act("&+LThe air seems to vibrate with &+Wenergy &+Lwhich causes&n $n &+Lto throw back $s head and let out a mocking laughter that freezes your &+Rblood.&n", FALSE, ch, 0, victim, TO_ROOM);

		act("&+LThe room seems to darken as you slam&n $p &+Lthrough&n $N's &+Lchest!&n", FALSE, ch, 0, victim, TO_CHAR);
		act("&+LAs $E slumps to the ground, bleeding, screaming, begging for mercy, you twist the weapon and draw the soul from the dying body.", FALSE, ch, 0, victim, TO_CHAR);
		act("&+LA sweetness fills your being and you cannot help but to let out a &=LWmocking laughter.&n&n", FALSE, ch, 0, victim, TO_CHAR);
	}
	else
	{
		act("With a &+mmocking smile&n lacking any &+ywarmth&n $n takes a step towards $s &+yhelpless victim&n \n"
		    ...
		    victim,
		    TO_ROOM);
		act("$s &+msmile deepens&n as $e watches the life quickly seep out of $N's body.", FALSE, ch, 0, victim, TO_ROOM);
		act("&+LStrengthened by the suffering around $m&n $n &+Ljerks $s weapon free causing the now lifeless husk to crumble in a heap.&n", FALSE, ch, 0, victim, TO_ROOM);

		act("&+YAmused, &+yyou take a step forward and drive&n $p &+ythrough&n $N's &+ychest.&n", FALSE, ch, 0, victim, TO_CHAR);
		act("&+yAs $E slumps to the ground screaming for &+Wmercy&+y, you twist the blade causing $S &+Llifeblood &+yto pour from $S body.&n", FALSE, ch, 0, victim, TO_CHAR);
		act("&+yStrengthened by the suffering you jerk your weapon free causing the now lifeless husk to crumble in a heap.&n", FALSE, ch, 0, victim, TO_CHAR);
	}
...
void setHeavenTime(P_char victim)
{
	...
	debug("%s time in heaven: until %s = %d seconds, recent deaths = %d.", J_NAME(victim), asctime(localtime(&(victim->only.pc->pc_timer[PC_TIMER_HEAVEN]))), time_in_heaven, counter);
}
...
unsigned int calculate_ch_state(P_char ch)
{
	if (ch)
	{
		if (GET_HIT(ch) < -10)
			...
			return STAT_INCAP;
		else if (((GET_STAT(ch) == STAT_SLEEPING) || (GET_STAT(ch) == STAT_RESTING)) && (IS_FIGHTING(ch) || NumAttackers(ch) || IS_DESTROYING(ch)))
			return STAT_NORMAL;
...
void update_pos(P_char ch)
{
	 ...
	 */
	if (IS_NPC(ch) && (stat > STAT_SLEEPING) && !IS_FIGHTING(ch) && CAN_ACT(ch) && ((ch->only.npc->default_pos & STAT_MASK) >= STAT_SLEEPING) && (!HAS_MEMORY(ch) || !GET_MEMORY(ch)) &&
	    !IS_DESTROYING(ch))
...
bool AdjacentInRoom(P_char ch, P_char ch2)
{
	...
	if (IS_TRUSTED(ch) || IS_TRUSTED(ch2) || IS_AFFECTED(ch, AFF_WRAITHFORM) || IS_AFFECTED(ch2, AFF_WRAITHFORM))
		return TRUE;
...
P_obj make_corpse(P_char ch, int loss)
{
	...
	if (IS_PC(ch))
	{
		snprintf(buf2, MAX_STRING_LENGTH, "%s %s", index("AEIOU", race_names_table[ch->player.race].normal[0]) == NULL ? "a" : "an", race_names_table[ch->player.race].normal);
	}
	...
	if (was_in != NOWHERE)
		for (door = 0; door <= (NUM_EXITS - 1); door++)
		{
			if (VIRTUAL_CAN_GO(was_in, door))
			{
				...
				{
					case 1:
						snprintf(buf, MAX_INPUT_LENGTH, "&+rThe unmistakable sound of something dying reverberates from nearby.\r\n");
						break;
					case 2:
						snprintf(buf, MAX_INPUT_LENGTH, "&+rYour spine tingles as a rattling death cry reaches your senses from nearby.\r\n");
						break;
					case 3:
						snprintf(buf, MAX_INPUT_LENGTH, "&+rA nearby death cry rings out loudly, heightening your bloodlust.\r\n");
						break;
				}
...
P_char ForceReturn(P_char ch)
{
	...
	if (IS_MORPH(t_ch))
	{
		...
		else
		{
			...
			send_to_char("As the last of life leaves your form, you return to your normal body.\r\n", t_ch);
		}
	...
	if (ch && IS_PC(ch) && ch->only.pc->switched && IS_MORPH(ch->only.pc->switched))
	{
		...
		send_to_char("&+RYour mind can no longer hold this form as your true body has been destroyed!!&n\r\n", t_ch);
		act("&+BThe soul holding $n &+Bto this realm has perished!\r\n$n&+B vanishes.", FALSE, t_ch, 0, 0, TO_ROOM);
		if (GET_OPPONENT(t_ch))
			stop_fighting(t_ch);
		if (IS_DESTROYING(t_ch))
			stop_destroying(t_ch);
	...
	if (t_ch->desc && t_ch->desc->original && IS_PC(t_ch->desc->original) && t_ch->desc->original->only.pc->switched)
	{
...
bool in_their_zone(P_char mob)
{
	...
	if (vnum < zone_table[world[mob->in_room].zone].number * 100 || (world[mob->in_room].zone < top_of_zone_table && vnum > zone_table[world[mob->in_room].zone + 1].number * 100))
		return FALSE;
...
void die(P_char ch, P_char killer)
{
	...
	if (IS_PC(ch) || !IS_SET(ch->specials.act, ACT_SPEC_DIE))
	{
		...
		else if (GET_LEVEL(ch) > 44 && get_property("thanksgiving", 0.000) && (number(0, 100) < get_property("thanksgiving.turkey.chance", 5.000)))
		{
			thanksgiving_proc(ch);
		}
		// Then Christmas.
		else if (GET_LEVEL(ch) > 35 && get_property("christmas", 0.000) && (number(0, 100) < get_property("christmas.elf.chance", 5.000)))
		{
			christmas_proc(ch);
		}
	...
	{
		logit(LOG_DEATH, "%s killed by %s at %s", GET_NAME(ch), (IS_NPC(killer) ? killer->player.short_descr : GET_NAME(killer)), world[ch->in_room].name);
		statuslog(ch->player.level, "%s killed by %s at [%d] %s", GET_NAME(ch), (IS_NPC(killer) ? killer->player.short_descr : GET_NAME(killer)), world[ch->in_room].number, world[ch->in_room].name);

		// If killer is a PC, or a pet with master group and in room.. then we have PvP
		if ((IS_PC(killer) || (IS_PC_PET(killer) && (GET_MASTER(killer)->in_room == killer->in_room) && killer->group && killer->group == GET_MASTER(killer)->group)) && (killer != ch))
		{
	...
	*/
	...
	if (IS_NPC(ch) && (GET_LEVEL(ch) > 51) && !IS_PC_PET(ch) && !affected_by_spell(ch, TAG_CONJURED_PET)) // soul shard - Drannak
	{
		...
		if (number(1, 250) < dchance)
		{
			...
			act("&+LAs &+R$n &+Lfalls to the ground, a small shard of their &+rlifeforce&n manifests&+L.&N", FALSE, ch, 0, 0, TO_ROOM);
		}
	}

	// possibility to find a recipe for the items in the zone.
	//  Only find recipes from mobs inside their own zone.
	if (IS_PC(killer) && !IS_PC_PET(ch) && in_their_zone(ch) && !affected_by_spell(ch, TAG_CONJURED_PET))
	...
	if (!CHAR_IN_ARENA(ch) || IS_NPC(ch))
	{
		...
		if (corpse && killer != ch && (affected_by_spell(killer, SPELL_SPAWN) || has_innate(killer, INNATE_SPAWN) || has_innate(killer, INNATE_ALLY)))
		{
	...
	else
	{
		...
		{
			snprintf(strn, MAX_STRING_LENGTH, "&+R%s was %s by %s!&N\r\n", GET_NAME(ch), arena_death_msg(killer->equipment[WIELD]), GET_NAME(killer));
			send_to_arena(strn, -1);
			...
			if (arena.type != TYPE_DEATHMATCH || arena.type != TYPE_KING_OF_THE_HILL)
			{
				for (tmp_ch = world[killer->in_room].people; tmp_ch; tmp_ch = tmp_ch->next_in_room)
				{
					if (killer->group)
					{
						if (killer->group == tmp_ch->group && tmp_ch != ch && killer != tmp_ch)
						{
		...
		switch (arena.deathmode)
		{
			...
			case DEATH_FREE:
				...
				if (ARENA_PLAYER(ch).lives > 0)
				{
					...
					arena_char_spawn(ch);
					snprintf(strn, MAX_STRING_LENGTH, "&+WYou breathe new air as you respawn.\r\nLives remaining: %d\r\n", ARENA_PLAYER(ch).lives);
					send_to_char(strn, ch);
				}
				else
				{
					char_to_room(ch, real_room(arena_hometown_location[arena_team(ch)]), -1);
...
void kill_gain(P_char ch, P_char victim)
{
	...
	for (gl = ch->group; gl; gl = gl->next)
	{
		...
		if (IS_PC(gl->ch) && !IS_TRUSTED(gl->ch) && (ch->in_room == gl->ch->in_room) && GET_LEVEL(gl->ch) > highest_level)
		{
	...
	for (gl = ch->group; gl; gl = gl->next)
	{
		if (IS_PC(gl->ch) && !IS_TRUSTED(gl->ch) && (gl->ch->in_room == ch->in_room))
		{
			...
			if (XP && IS_PC(gl->ch))
			{
				logit(LOG_EXP, "%s: %d, group kill of: %s [%d]", GET_NAME(gl->ch), XP, GET_NAME(victim), (IS_NPC(victim) ? GET_VNUM(victim) : 0));
			}
...
void dam_message(double fdam, P_char ch, P_char victim, struct damage_messages *messages)
{
	...
	*/
	...
	{
		snprintf(buf_char, 160, messages->attacker, weapon_damage[w_loop], victim_damage[h_loop]);
		snprintf(buf_vict, 160, messages->victim, weapon_damage[w_loop], victim_damage[h_loop]);
		snprintf(buf_notvict, 160, messages->room, weapon_damage[w_loop], victim_damage[h_loop]);
	}
	...
	if (IS_PC(ch) && !IS_SET(ch->specials.act2, PLR2_BATTLEALERT) && !IS_SET(ch->specials.act2, PLR2_TERSE))
	{
		strcat(buf_char, "&+G]=-&N");
		send_to_char("&+G-=[&N", ch);
	}
	...
	if (IS_PC(victim) && !IS_SET(victim->specials.act2, PLR2_BATTLEALERT) && !IS_SET(victim->specials.act2, PLR2_TERSE))
	{
		strcat(buf_vict, "&+R]=-&N");
...
int try_riposte(P_char ch, P_char victim, P_obj wpn)
{
	...
	{
		act("$n flourishes $s dagger, intercepting $N's attack, and gracefully counters!", TRUE, ch, 0, victim, TO_NOTVICT);
		act("$n thrusts forth $s dagger, intercepting your attack, and counters it gracefully!", TRUE, ch, 0, victim, TO_VICT);
		act("You brandish your offhand dagger, intercepting $N's blow, and countering his attack!", TRUE, ch, 0, victim, TO_CHAR);
	...
	if (IS_ELITE(ch))
	{
		if (GET_CLASS(ch, CLASS_WARRIOR) || GET_CLASS(ch, CLASS_ANTIPALADIN) || GET_CLASS(ch, CLASS_DREADLORD) || GET_CLASS(ch, CLASS_AVENGER))
		{
			skl += 100;
			npcepicriposte = TRUE;
		}
...
int attack_back(P_char ch, P_char victim, int physical)
{
	...
	else if (!physical && (IS_PC(ch) || IS_PC_PET(ch)) && IS_PC(victim) && (!IS_SET(ch->specials.act, PLR_VICIOUS) || IS_BACKRANKED(ch) || IS_BACKRANKED(victim)))
	{
		return DAM_NONEDEAD;
	}
...
bool can_hit_target(P_char ch, P_char vict)
{
	...
	for (opponent = combat_list; opponent; opponent = next_ch)
	{
		...
		if ((GET_OPPONENT(opponent) == vict) && (opponent != ch) && IS_PC(ch) && IS_PC(opponent))
		{
			num_opponents++;
...
bool damage(P_char ch, P_char victim, double dam, int attacktype)
{
	...
	{
		if (attacktype == SPELL_HOLY_WORD || attacktype == SPELL_UNHOLY_WORD || attacktype == SPELL_VOICE_OF_CREATION)
			type = SPLDAM_HOLY;
		else
			type = SPLDAM_GENERIC;
		...
		if ((attacktype >= SPELL_FIRE_BREATH && attacktype <= SPELL_LIGHTNING_BREATH) || attacktype == SPELL_SHADOW_BREATH_1 || attacktype == SPELL_SHADOW_BREATH_2)
			flags |= SPLDAM_BREATH;
...
static dam_mod_predicate spell_damage_modifiers[] = {{MAKE_DAM_MOD_PRED(){if (get_linked_char(victim, LNK_PET) && IS_PC(caster)){dam_mod->mod += get_property("damage.pcs.vs.pets", 2.000) - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(victim, SKILL_BERSERK)){dam_mod->mod = dam_factor[DF_BERSERKSPELL] - 1.0;
...
, {MAKE_DAM_MOD_PRED(){if (ELEMENTAL_DAM(damageType) && affected_by_spell(victim, SPELL_ENERGY_CONTAINMENT)){dam_mod->mod += dam_factor[DF_ENERGY_CONTAINMENT] - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (ELEMENTAL_DAM(damageType) && has_innate(caster, INNATE_ELEMENTAL_POWER) && GET_LEVEL(caster) >= 35){dam_mod->mod += dam_factor[DF_ELEMENTALIST] - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (GET_CHAR_SKILL(victim, SKILL_ARCANE_BLOCK) > 0 && !IS_TRUSTED(victim) && !IS_STUNNED(victim) &&
                           !IS_IMMOBILE(victim)){if (damage > 15 && (notch_skill(victim, SKILL_ARCANE_BLOCK, get_property("skill.notch.arcane", 10)) ||
...
act("$N raises hands performing an &+Marcane gesture&n and some of $n's &+mspell energy&n is dispersed.", TRUE, caster, 0, victim, TO_NOTVICT);
act("$N raises hands performing an &+Marcane gesture&n and some of your &+mspell's energy&n is dispersed.", TRUE, caster, 0, victim, TO_CHAR);
act("You perform an &+Marcane gesture&n dispersing some of $n's &+mspell energy.&n", TRUE, caster, 0, victim, TO_VICT);
}
}
}
}
, {MAKE_DAM_MOD_PRED(){switch (damageType){case SPLDAM_GENERIC : if (has_innate(victim, MAGICAL_REDUCTION)){dam_mod->mod += -0.2;
dam_mod->type = dam_mod_type::More;
}
break;
}
...
, {MAKE_DAM_MOD_PRED(){if (damageType == SPLDAM_FIRE && IS_AFFECTED4(victim, AFF4_ICE_AURA)){dam_mod->mod += dam_factor[DF_VULNFIRE] - 1.0;
dam_mod->type = dam_mod_type::More;
act("&+rYour fiery spell causes&n $N to &+rsmolder and spasm in pain!&n", TRUE, caster, 0, victim, TO_CHAR);
act("$n's &+fiery spell causes you smolder and spasm in pain!&n", TRUE, caster, 0, victim, TO_VICT);
act("$n's &+rfiery spell causes&n $N &n&+rto smolder and spasm in pain!&n", TRUE, caster, 0, victim, TO_NOTVICT);
}
}
}
, {MAKE_DAM_MOD_PRED(){
	  if (damageType == SPLDAM_FIRE && caster && affected_by_spell(caster, TAG_BLOODLUST) && !IS_PC_PET(victim) && IS_NPC(victim) && !CHAR_IN_JUSTICE_AREA(caster)){struct affected_type * paf;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_FIRE) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_COLD) return;
...
if (GET_RACE(victim) == RACE_F_ELEMENTAL || IS_EFREET(victim))
{
	...
	act("$n's &+Bicy spell causes&n $N to writhe in agony!&n", TRUE, caster, 0, victim, TO_NOTVICT);
}
...
if (IS_AFFECTED2(victim, AFF2_FIRESHIELD))
{
	if (GET_CHAR_SKILL(victim, SKILL_FLAME_MASTERY) && GET_CHAR_SKILL(victim, SKILL_FLAME_MASTERY) > number(1, 101) && !IS_STUNNED(victim) && CAN_SEE(victim, caster))
	{
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_GAS) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_ACID) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_LIGHTNING) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_HOLY) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_PSI) return;
...
, {MAKE_DAM_MOD_PRED(){if (damageType != SPLDAM_NEGATIVE) return;
...
, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED5(victim, AFF5_JUDICIUM_FIDEI)){dam_mod->mod += dam_factor[DF_JUDICIUM_FIDEI] - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (!has_innate(victim, MAGIC_VULNERABILITY)) return;
...
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(caster, ACH_DRAGONSLAYER) && (GET_RACE(victim) == RACE_DRAGON)){dam_mod->mod += 0.1;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(caster, ACH_DEMONSLAYER) && (GET_RACE(victim) == RACE_DEMON)){dam_mod->mod += 0.1;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(victim, SPELL_SOULSHIELD) && (GET_CLASS(victim, CLASS_PALADIN) || GET_CLASS(victim, CLASS_ANTIPALADIN))){dam_mod->mod += -0.15;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){double modifier = MAGICRES(victim);
...
, {MAKE_DAM_MOD_PRED(){dam_mod->mod += ((MAGICDAMBONUS(caster) <= 100 ? 100 : MAGICDAMBONUS(caster) - 10 + number(1, 10)) / 100.0) - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
...
#define HOA_AWE_VNUM 77746
	  ...
	  bool awe = false;
if (victim->equipment[WEAR_BODY] && (victim->equipment[WEAR_BODY]->R_num == real_object(HOA_AWE_VNUM)))
{
	awe = TRUE;
}
...
, {MAKE_DAM_MOD_PRED(){if (has_innate(victim, INNATE_VULN_COLD) && damageType == SPLDAM_COLD){dam_mod->mod += dam_factor[DF_VULNCOLD] - 1.0;
dam_mod->type = dam_mod_type::Increased;
...
, {MAKE_DAM_MOD_PRED(){if (parse_chaos_shield(caster, victim)){dam_mod->mod += dam_factor[DF_CHAOSSHIELD] - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
}
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(victim, SKILL_SPELL_PENETRATION)){int damageReductionMod = number(20, 70);
dam_mod->type = dam_mod_type::More;
...
, {MAKE_DAM_MOD_PRED(){if (GET_RACE(caster) > RACE_NONE && GET_RACE(caster) <= LAST_RACE && damageType >= 0 && damageType < LAST_SPLDAM_TYPE){dam_mod->type = dam_mod_type::More;
...
, {MAKE_DAM_MOD_PRED(){if (IS_NPC(caster) && !affected_by_spell(caster, TAG_CONJURED_PET)){int zone_difficulty = BOUNDED(1, zone_table[world[real_room0(GET_BIRTHPLACE(caster))].zone].difficulty, 10);

if (zone_difficulty > 1)
{
	dam_mod->type = dam_mod_type::More;
	dam_mod->mod += (get_property("damage.zoneDifficulty.spells.factor", 0.05) * zone_difficulty);
}
}
}
}
, {MAKE_DAM_MOD_PRED(){int necropets[] = {3, 4, 5, 6, 7, 8, 9, 10, 78, 79, 80, 81, 82, 83, 84, 85};
if (IS_NPC(caster) && IS_PC(victim))
{
	for (int r = 0; r < 16; r++)
	{
		if (mob_index[GET_RNUM(caster)].virtual_number == NECROPET || mob_index[GET_RNUM(caster)].virtual_number == necropets[r])
		{
			dam_mod->type = dam_mod_type::More;
			dam_mod->mod += -0.5;
			break;
...
, {MAKE_DAM_MOD_PRED(){dam_mod->mod += get_property("damage.spell.multiplier", 1.0) - 1.0;
dam_mod->type = dam_mod_type::More;
}
}
, {MAKE_DAM_MOD_PRED(){if (get_linked_char(victim, LNK_ETHEREAL) || get_linking_char(victim, LNK_ETHEREAL)){P_char eth_ch = NULL;
...
, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(victim, SKILL_DREADNAUGHT)){struct affected_type *paf = get_spell_from_char(victim, SKILL_DREADNAUGHT);
...
, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED3(victim, AFF3_PALADIN_AURA) && has_aura(victim, AURA_SPELL_PROTECTION)){dam_mod->type = dam_mod_type::More;
...
int spell_damage(P_char ch, P_char victim, double dam, int type, uint flags, struct damage_messages *messages, int *damAccumulator)
{
	...
	if (type == SPLDAM_FIRE && ch != victim && ((IS_AFFECTED2(victim, AFF2_FIRE_AURA) && IS_AFFECTED2(victim, AFF2_FIRESHIELD)) || GET_RACE(victim) == RACE_F_ELEMENTAL))
	{
	...
	if (ch != victim)
	{
		if ((IS_AFFECTED3(victim, AFF3_SPIRIT_WARD) && (flags & SPLDAM_SPIRITWARD)) || (IS_AFFECTED3(victim, AFF3_GR_SPIRIT_WARD) && (flags & SPLDAM_GRSPIRIT)))
		{
			act("&+WThe globe around your body flares as it bears the brunt of&n $n&+W's assault!", FALSE, ch, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("&+WThe globe around&n $N&+W's body flares as it bears the brunt of your assault!", FALSE, ch, 0, victim, TO_CHAR | ACT_NOTTERSE);
			attack_back(ch, victim, FALSE);
			return DAM_NONEDEAD;
		}
		if (((IS_AFFECTED(victim, AFF_MINOR_GLOBE)) && (flags & SPLDAM_MINORGLOBE)) || (IS_AFFECTED2(victim, AFF2_GLOBE) && (flags & SPLDAM_GLOBE)))
		{
			act("&+RThe globe around your body flares as it bears the brunt of&n $n&+R's assault!", FALSE, ch, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("&+RThe globe around&n $N&+R's body flares as it bears the brunt of your assault!", FALSE, ch, 0, victim, TO_CHAR | ACT_NOTTERSE);
			attack_back(ch, victim, FALSE);
			return DAM_NONEDEAD;
		}
	}
	...
	if ((ch != victim) && !IS_SET(flags, SPLDAM_NODEFLECT))
	{
		...
		{
			act("&+cA &+Ctranslucent&n&+c field &+Wflashes&n&+c around your body upon contact with&n $n&n&+c's assault, deflecting it back at $m!", FALSE, ch, 0, victim, TO_VICT);
			act("&+cA &+Ctranslucent&n&+c field &+Wflashes&n&+c around&n $N&n&+c's body upon contact with&n $n&n&+c's assault, deflecting it back at $m!", FALSE, ch, 0, victim, TO_NOTVICT);
			act("&+cA &+Ctranslucent&n&+c field &+Wflashes&n&+c around&n $N&n&+c's body upon contact with your assault, deflecting it back at YOU!", FALSE, ch, 0, victim, TO_CHAR);
		...
		if (victim && IS_NPC(victim) && mob_index[GET_RNUM(victim)].func.mob && !affected_by_spell(victim, TAG_CONJURED_PET))
		{
			data.victim     = ch;
			data.dam        = (int)dam;
			data.attacktype = type;
		...
		{
			act("&+RThe sphere circling you darts in front of&n $n&+R's assault, absorbing its magic and leaving you unharmed!", FALSE, ch, 0, victim, TO_VICT);
			act("&+RThe sphere circling&n $N&+R's body darts in front of&n $n&+R's assault!", FALSE, ch, 0, victim, TO_NOTVICT);
			act("&+RThe sphere circling&n $N&+R's body darts in front of your assault!", FALSE, ch, 0, victim, TO_CHAR);
		...
		if (GET_CHAR_SKILL(victim, SKILL_ARCANE_RIPOSTE) > 0 && !IS_TRUSTED(victim))
		{
			...
			if (!IS_STUNNED(victim) && (dam > 10 && notch_skill(victim, SKILL_ARCANE_RIPOSTE, get_property("skill.notch.arcane", 10))) || (number(1, 100) < skill_lvl / 4))
			{
				act("$N frowns in &+cconcentration&n as $E intercepts $n's spell and &+Churls it back at $m!&n", TRUE, ch, 0, victim, TO_NOTVICT);
				act("$N frowns in &+cconcentration&n as $E intercepts your spell and &+Churls it back at you!&n", TRUE, ch, 0, victim, TO_CHAR);
				act("With &+cgreat mastery&n you intercept $n's spell and &+Churl it back at $m!&n", TRUE, ch, 0, victim, TO_VICT);
		...
		if (GET_CHAR_SKILL(victim, SKILL_DISPERSE_FLAMES) && type == SPLDAM_FIRE && GET_CHAR_SKILL(victim, SKILL_DISPERSE_FLAMES) > number(0, 100) && !IS_TRUSTED(victim))
		{
			if ((dam > 10 && notch_skill(victim, SKILL_DISPERSE_FLAMES, get_property("skill.notch.pyrokinetics", 2))) || (!number(0, 2) && number(1, 56) <= GET_LEVEL(victim)))
			{
				act("$N &+rsmiles slightly as the &+Yflames &+rdwindle and die before reaching $M.&n", TRUE, ch, 0, victim, TO_NOTVICT);
				act("$N &+rsmiles slightly as the &+Yflames &+rdwindle and die before reaching $M.&n", TRUE, ch, 0, victim, TO_CHAR);
				act("&+rYou take control over the &+Yflames &+rand will them to dwindle and die.&n", TRUE, ch, 0, victim, TO_VICT);
				return DAM_NONEDEAD;
			}
		}

		if (GET_CHAR_SKILL(victim, SKILL_FLAME_MASTERY) && type == SPLDAM_FIRE && GET_CHAR_SKILL(victim, SKILL_FLAME_MASTERY) > number(0, 100) && !IS_TRUSTED(victim))
		{
			if ((dam > 10 && notch_skill(victim, SKILL_FLAME_MASTERY, get_property("skill.notch.pyrokinetics", 2))) || (!number(0, 10) && number(1, 56) <= GET_LEVEL(ch)))
			{
				act("$N &+rsmiles slightly as $E stops the &+Yflames &+rsummoned by&n $n &+rand hurls them back at $m!&n", TRUE, ch, 0, victim, TO_NOTVICT);
				act("&+rTo your horror you see&n $N &+rsmile slightly as $E halts the &+Yflames &+rand hurls them back at you!&n", TRUE, ch, 0, victim, TO_CHAR);
				act("&+rYou laugh as you send &+Yflames &+rand burning &+Wectoplasm &+rback towards&n $n.&n", TRUE, ch, 0, victim, TO_VICT);
	...
	{
		act("&+C$N&+C's &+WI&+Cc&+wy &+BShield &+Rmelts &+Caround $M from $n's &+Rf&+ri&+Yer&+Ry &+rassault, &+Cbut leaves $M unharmed!", TRUE, ch, 0, victim, TO_NOTVICT);
		act("&+C$N&+C's &+WI&+Cc&+wy &+BShield &+Rmelts &+Caround $M from your &+Rf&+ri&+Yer&+Ry &+rassault, &+Cbut leaves $M unharmed!", TRUE, ch, 0, victim, TO_CHAR);
		act("&+CYour &+Wi&+Cc&+wy &+Bshield &+Rmelts &+Caround you from the &+Rf&+ri&+Yer&+Ry &+rassault, &+Cbut leaves you unharmed!&n", TRUE, ch, 0, victim, TO_VICT);
	...
	{
		act("$n's &+WH&+wo&+Ll&+Wy &+wspell &+Ldisperses the negative shield surrounding $N&+L, but leaves $M unharmed!&n", TRUE, ch, 0, victim, TO_NOTVICT);
		act("&+LThe &+WH&+wo&+Ll&+Wy &+wspell &+Ldisperses the negative shield surrounding $N&+L, but leaves $M unharmed!&n", TRUE, ch, 0, victim, TO_CHAR);
		act("&+LThe barrier of negative energy &=LWFLASHES&n &+Las the &+WH&+wo&+Ll&+Wy &+wspell &+Ldisperses your shield but leaves you unharmed!&n", TRUE, ch, 0, victim, TO_VICT);
	...
	if (!IS_SET(flags, SPLDAM_NOSHRUG))
	{
		...
		if ((ch != victim) && (dam / 50) > number(0, 9) && has_innate(victim, INNATE_SPELL_ABSORB) && number(1, 100) <= (GET_LEVEL(victim) / 2) && USES_SPELL_SLOTS(victim))
		{
	...
	*/
	...
	if (result == DAM_NONEDEAD)
	{
		...
		if (IS_AFFECTED5(victim, AFF5_WET) && type == SPLDAM_LIGHTNING && ilogb(dam) > number(0, 10))
		{
			...
			act("The &+Celectricity&n surges through $n's wet clothes immobilizing $m momentarily!", FALSE, victim, 0, 0, TO_ROOM);
			act("The &+Celectricity&n surges through your wet clothes paralyzing you in a shock!", FALSE, victim, 0, 0, TO_CHAR);
...
int check_shields(P_char ch, P_char victim, int dam, int flags)
{
	...
	struct damage_messages coldshield      = {"$N &+Bshivers from the unnatural cold as $E hits you!&n",
	...
	struct damage_messages fireshield      = {"$N &+ris burned severely as $E hits you!",
	...
	struct damage_messages negshield       = {"$N&+r ignites into &+Lblack flames&n&+r as $E hits you!",
	...
	struct damage_messages soulshield      = {"$N &+wsuffers from contact with the aura about you.",
	...
	struct damage_messages soulshield_spec = {"$N suffers from contact with the &+Wholy&n aura about you.",
	...
	struct damage_messages acid_blood      = {"$N &+Lwrithes in agony the black blood greedily eats into $S &+rflesh.&n",
	...
	struct damage_messages infernalfury = {"&+yThe &+Rinf&+rer&+Lnal e&+rner&+Rgies &+ysurrounding you &+Wsear &+yinto&n $N &+yas $E hits you!&n",
	...
	{
		result = spell_damage(victim, ch, (int)(dam * negshielddam), SPLDAM_NEGATIVE, sflags | SPLDAM_GRSPIRIT, &negshield);
	}
	else if (IS_AFFECTED2(victim, AFF2_SOULSHIELD) && ((IS_EVIL(ch) && IS_GOOD(victim)) || (IS_GOOD(ch) && IS_EVIL(victim)) || opposite_racewar(ch, victim)))
	{
		if (GET_SPEC(victim, CLASS_CLERIC, SPEC_HOLYMAN))
		{
			result = spell_damage(victim, ch, (int)(dam * soulshielddam), SPLDAM_HOLY, SPLDAM_NODEFLECT | RAWDAM_TRANCEVAMP | SPLDAM_GRSPIRIT, &soulshield_spec);
			...
			if (IS_NPC(ch) && !affected_by_spell(ch, TAG_CONJURED_PET) && GET_LEVEL(ch) >= 51 && (IS_ELITE(ch) || IS_GREATER_RACE(ch)))
			{
				rnumber = 6;
			}
			                                                     ...
			                                                     !IS_AFFECTED3(ch, AFF3_GR_SPIRIT_WARD))*/
			{
				...
				{
					case 1:
						if (!IS_ELITE(ch) && !IS_GREATER_RACE(ch) && (GET_LEVEL(ch) + 5) > GET_LEVEL(victim))
						{
					...
					case 3:
						if (!affected_by_spell(ch, SPELL_CURSE))
						{
							...
							affect_to_char(ch, &af);
							act("&+wYour strength dwindles as&n $N &+wgrabs you by your forehead and calls down the wraith of $S deity.&n", FALSE, ch, 0, victim, TO_CHAR);
					...
					case 7:
						...
						if (!IS_AFFECTED4(ch, AFF4_NOFEAR) && !IS_ELITE(ch) && !IS_GREATER_RACE(ch))
						{
	...
	if ((result == DAM_NONEDEAD) && has_innate(victim, INNATE_ACID_BLOOD) && (dam > ((flags & PHSDAM_TOUCH) ? 9 : 5)) && !number(0, 9))
	{
...
int melee_damage(P_char ch, P_char victim, double dam, int flags, struct damage_messages *messages, int *damAccumulator)
{
	...
	if (!IS_FIGHTING(ch) && !IS_DESTROYING(ch) && !(flags & PHSDAM_NOENGAGE) && ch->in_room == victim->in_room)
	{
		set_fighting(ch, victim);
	}
	...
	if (!(flags & PHSDAM_NOREDUCE))
	{
		...
		*/
		...
		if (IS_RIDING(ch) && GET_SPEC(ch, CLASS_ANTIPALADIN, SPEC_DEMONIC) || IS_RIDING(ch) && GET_SPEC(ch, CLASS_PALADIN, SPEC_CAVALIER))
		{
			dam *= 1.20;
		}

		if (IS_AFFECTED2(victim, AFF2_SOULSHIELD) && (((IS_EVIL(ch) && IS_GOOD(victim)) || (IS_GOOD(ch) && IS_EVIL(victim)) || opposite_racewar(ch, victim))))
		{
	...
	if (GET_CLASS(ch, CLASS_BERSERKER) && (GET_HIT(ch) > 0) && affected_by_spell(ch, SKILL_BERSERK))
	{
		if (IS_NPC(ch))
		{
	...
	if (IS_AFFECTED2(victim, AFF2_EARTH_AURA) && !(flags & PHSDAM_NOREDUCE) && !number(0, 5) && GET_RACE(ch) != RACE_E_ELEMENTAL)
	{
		dam = 0;
		act("$n's &+yattack glances off of your stone hide!", TRUE, ch, NULL, victim, TO_VICT);
		act("$n's &+yattack glances off of&n $N's &+ystone hide!", TRUE, ch, NULL, victim, TO_NOTVICT);
		act("&+yYour attack glances off of&n $N's &+ystone hide!", TRUE, ch, NULL, victim, TO_CHAR);
	}

	// Combat mind bypasses displacement.
	if (affected_by_spell(victim, SPELL_DISPLACEMENT) && (!number(0, 5)) && !(flags & PHSDAM_NOREDUCE) && !affected_by_spell(ch, SPELL_COMBAT_MIND))
	{
		dam = 0;
	...
	{
		struct damage_messages wz_messages = {"Your strike $N with &+Wholy &+rfe&+Rrv&+ror&n!",
...
void check_vamp(P_char ch, P_char victim, double fdam, uint flags)
{
	...
	if ((flags & PHSDAM_ARROW) && IS_AFFECTED2(ch, AFF2_VAMPIRIC_TOUCH) && IS_PC(ch) && !IS_AFFECTED4(ch, AFF4_BATTLE_ECSTASY) && dam >= 4)
	{
	...
	// PC vamp touch.
	if ((flags & PHSDAM_TOUCH) && !vamped && IS_AFFECTED2(ch, AFF2_VAMPIRIC_TOUCH) && IS_PC(ch) && !IS_AFFECTED4(ch, AFF4_VAMPIRE_FORM)) // && !IS_AFFECTED4(ch, AFF4_BATTLE_ECSTASY) )
	{
	...
	if ((flags & PHSDAM_TOUCH) && IS_AFFECTED2(ch, AFF2_VAMPIRIC_TOUCH) && IS_NPC(ch) && !IS_AFFECTED4(ch, AFF4_VAMPIRE_FORM))
	{
		vamped = vamp(ch, dam * dam_factor[DF_TOUCHVAMP], GET_MAX_HIT(ch) * VAMPPERCENT(ch));
	}
	// end TOUCHVAMP
	...
	if (!vamped && ch != victim && !IS_AFFECTED4(victim, AFF4_HOLY_SACRIFICE) && ((flags & RAWDAM_BTXVAMP) || (flags & RAWDAM_SHRTVMP)) &&
	...
	if (dam >= 10 && ((flags & RAWDAM_BTXVAMP) || (flags & PHSDAM_ARROW)))
	{
		...
		if (IS_NPC(ch) && !IS_PC_PET(ch))
		{
			...
			for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
			{
				...
				if (IS_NPC(tch) && !IS_PC_PET(tch) && IS_SET((tch)->specials.affected_by4, AFF4_BATTLE_ECSTASY) && tch->in_room == ch->in_room && tch != ch)
				{
	...
	if (!vamped && IS_AFFECTED2(ch, AFF2_VAMPIRIC_TOUCH) && (flags & RAWDAM_TRANCEVAMP) && (IS_AFFECTED4(ch, AFF4_VAMPIRE_FORM)))
	{
	...
	// BATTLETIDE VAMP
	if (!vamped && (flags & PHSDAM_BATTLETIDE) && IS_AFFECTED4(ch, AFF4_BATTLETIDE) && !affected_by_spell(ch, SPELL_PLAGUE))
	{
	...
	// Tranced vamping
	if (!vamped && (flags & RAWDAM_TRANCEVAMP) && (IS_UNDEADRACE(ch) || IS_ANGEL(ch)) && !IS_AFFECTED4(ch, AFF4_BATTLE_ECSTASY))
	{
		if (IS_DRACOLICH(ch))
		{
...
#define HOA_ILLESARUS_VNUM 77738
	if (!vamped && ch->equipment[WIELD] && (obj_index[ch->equipment[WIELD]->R_num].virtual_number == HOA_ILLESARUS_VNUM))
	{
		vamped = vamp(ch, MIN(dam, number(2, 7)), GET_MAX_HIT(ch) * VAMPPERCENT(ch));
	}

	if ((dam >= 2 && !IS_AFFECTED4(ch, AFF4_BATTLE_ECSTASY) && IS_AFFECTED4(victim, AFF4_HOLY_SACRIFICE) && (flags & RAWDAM_HOLYSAC) && !affected_by_spell(victim, SPELL_BMANTLE) &&
	     !affected_by_spell(victim, SPELL_PLAGUE) && !affected_by_spell(victim, SPELL_FLAMESTRIKE)))
	...
	*/
	{
		...
		for (group = victim->group; group; group = group->next)
		{
			...
			if (tch->in_room == victim->in_room && tch != victim && !IS_AFFECTED4(tch, AFF4_HOLY_SACRIFICE) && !affected_by_spell(tch, SPELL_PLAGUE))
			{
...
static dam_mod_predicate raw_damage_modifiers[] = {{MAKE_DAM_MOD_PRED(){switch (GET_RACEWAR(caster)){case RACEWAR_GOOD : dam_mod->mod += dam_factor[DF_GOOD_MODIFIER] - 1.0;
break;
case RACEWAR_EVIL:
...
case RACEWAR_NEUTRAL:
	...
	, {MAKE_DAM_MOD_PRED(){if (IS_NPC(caster) && !IS_PC_PET(caster)){dam_mod->mod += get_property("damage.mob.bonus", 1.0) - 1.0;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (affected_by_spell(caster, TAG_BLOODLUST) && !IS_PC_PET(victim) && IS_NPC(victim)){int dammod;
	...
	, {MAKE_DAM_MOD_PRED(){if ((IS_GOOD(caster) && affected_by_spell(caster, SPELL_HOLY_SWORD) && IS_EVIL(victim)) ||
	                           (IS_EVIL(caster) && affected_by_spell(caster, SPELL_HOLY_SWORD) && IS_GOOD(victim))){dam_mod->mod += dice(2, 6);
	dam_mod->type = dam_mod_type::Added;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED4(victim, AFF4_SANCTUARY) && (flags & RAWDAM_SANCTUARY) && (GET_CLASS(victim, CLASS_PALADIN))){dam_mod->mod += dam_factor[DF_SANC] - 1.0;
	dam_mod->type     = dam_mod_type::More;
	...
	, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED4(victim, AFF4_SANCTUARY) && (flags & RAWDAM_SANCTUARY) && GET_CLASS(victim, CLASS_CLERIC)){dam_mod->mod += dam_factor[DF_SANC] - 1.0;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){
		  if (IS_AFFECTED4(victim, AFF4_SANCTUARY) && (flags & RAWDAM_SANCTUARY) && (!GET_CLASS(victim, CLASS_CLERIC) && !GET_CLASS(victim, CLASS_PALADIN))){dam_mod->mod += dam_factor[DF_SANC] - 1.0;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){
		  if (IS_AFFECTED(victim, AFF_SANCTUM_DRACONIS) && (flags & RAWDAM_SANCTUARY) && (GET_SPEC(victim, CLASS_DRAGOON, SPEC_DRAGON_LANCER))){dam_mod->mod += dam_factor[DF_SANC] - 1.0;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){struct affected_type *af = NULL;
	if (((IS_EVIL(caster) && !IS_EVIL(victim)) || (IS_GOOD(caster) && !IS_GOOD(victim))) && !(flags & PHSDAM_NOREDUCE) && (af = get_spell_from_char(victim, SPELL_VIRTUE)))
	{
	...
	, {MAKE_DAM_MOD_PRED(){if (get_spell_from_room(&world[caster->in_room], SPELL_CONSECRATE_LAND) && !(flags & PHSDAM_NOREDUCE)){dam_mod->mod += -0.5;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (get_spell_from_room(&world[caster->in_room], SPELL_BINDING_WIND) && !(flags & PHSDAM_NOREDUCE)){dam_mod->mod += -0.2;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED3(victim, AFF3_PROT_ANIMAL) && IS_ANIMAL(caster) && !(flags & PHSDAM_NOREDUCE)){dam_mod->mod += dam_factor[DF_PROTANIMAL] - 1.0;
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (IS_AFFECTED3(caster, AFF3_PALADIN_AURA) && has_aura(caster, AURA_BATTLELUST) &&
	                           !(flags & PHSDAM_NOREDUCE)){dam_mod->mod += ((get_property("innate.paladin_aura.battlelust_mod", 0.2) * aura_mod(caster, AURA_BATTLELUST)) / 100.0);
	dam_mod->type = dam_mod_type::More;
	}
	}
	}
	, {MAKE_DAM_MOD_PRED(){if (has_innate(caster, INNATE_WARCALLERS_FURY)){int group_size = num_group_members_in_room(victim);
	...
	if (caster->group)
	{
		...
		{
			if (caster != gl->ch && IS_PC(gl->ch) && caster->in_room == gl->ch->in_room && has_innate(gl->ch, INNATE_WARCALLERS_FURY))
			{
				count++;
			}
		}
	...
	, {MAKE_DAM_MOD_PRED(){if (!(flags & PHSDAM_NOREDUCE)){// global mod of 75% reduced damage
	                                                       dam_mod->mod += -0.75;
	dam_mod->type = dam_mod_type::Increased;
	...
	, {MAKE_DAM_MOD_PRED(){if (IS_NPC(caster) && !IS_PC_PET(caster) && !IS_MORPH(caster) && (IS_PC(victim) || IS_PC_PET(victim) || IS_MORPH(victim))){dam_mod->type = dam_mod_type::More;
	...
	int raw_damage(P_char ch, P_char victim, double dam, uint flags, struct damage_messages *messages, int *damAccumulator)
	{
		...
		if (ch && victim) // Just making sure.
		{
			...
			if (IS_AFFECTED5(victim, AFF5_IMPRISON) && (flags & RAWDAM_IMPRISON) && handle_imprison_damage(ch, victim, (int)dam))
			{
				return DAM_NONEDEAD;
			}

			if (IS_AFFECTED5(victim, AFF5_VINES) && (flags & RAWDAM_VINES) && (af = get_spell_from_char(victim, SPELL_VINES)))
			{
			...
			if (IS_PC(victim) && victim->desc && (IS_SET(victim->specials.act, PLR_SMARTPROMPT) || IS_SET(victim->specials.act, PLR_OLDSMARTP)))
			{
			...
			{
				if (IS_SET(orig->specials.act, PLR_SMARTPROMPT) || IS_SET(orig->specials.act, PLR_OLDSMARTP))
				{
			...
			*/
			...
			/* make mirror images disappear */
			if (victim && IS_NPC(victim) && !(flags & RAWDAM_NOKILL) && GET_VNUM(victim) == 250 && (GET_HIT(victim) < 15))
			{
			...
			if (new_stat == STAT_DEAD)
			{
				...
				if (victim && killer && IS_PC(victim) && opposite_racewar(killer, victim) && !IS_TRUSTED(killer) && !IS_TRUSTED(victim) && (messages->type & 0xff000000))
				{
			...
			if (IS_MINOTAUR(victim) && !number(0, 5) && (GET_HIT(victim) < GET_MAX_HIT(victim) / 6) && !affected_by_spell(victim, SKILL_BERSERK))
			{
				berserk(victim, 6 * PULSE_VIOLENCE);
			}

			if (GET_CLASS(victim, CLASS_BERSERKER) && (GET_HIT(victim) < GET_MAX_HIT(victim) / 4) && !affected_by_spell(victim, SKILL_BERSERK))
			{
	...
	int calculate_thac_zero(P_char ch, int skill)
	{
		...
		else if (GET_CLASS(ch, CLASS_WARRIOR) || GET_CLASS(ch, CLASS_DREADLORD) || GET_CLASS(ch, CLASS_AVENGER) || GET_CLASS(ch, CLASS_PALADIN) || GET_CLASS(ch, CLASS_ANTIPALADIN) ||
		         affected_by_spell(ch, SPELL_COMBAT_MIND) || is_wearing_necroplasm(ch) || GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_LANCER))
		{
			to_hit = get_property("to.hit.WarriorTypes", 10);
		}
		else if (GET_CLASS(ch, CLASS_MERCENARY) || GET_CLASS(ch, CLASS_REAVER) || GET_CLASS(ch, CLASS_RANGER) || GET_CLASS(ch, CLASS_BERSERKER) || GET_SPEC(ch, CLASS_NECROMANCER, SPEC_REAPER) ||
		         GET_SPEC(ch, CLASS_THEURGIST, SPEC_THAUMATURGE) || GET_SPEC(ch, CLASS_CLERIC, SPEC_ZEALOT) || GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_HUNTER))
		{
			to_hit = get_property("to.hit.HitterTankTypes", 8);
		}
		else if (GET_CLASS(ch, CLASS_THIEF) || GET_CLASS(ch, CLASS_BARD) || GET_CLASS(ch, CLASS_ROGUE) || GET_CLASS(ch, CLASS_ASSASSIN) || GET_CLASS(ch, CLASS_MONK))
		{
			to_hit = get_property("to.hit.RogueTypes", 7);
		}
		else if (GET_CLASS(ch, CLASS_CLERIC) || GET_CLASS(ch, CLASS_DRUID) || GET_CLASS(ch, CLASS_BLIGHTER) || GET_CLASS(ch, CLASS_SHAMAN) || GET_CLASS(ch, CLASS_WARLOCK) ||
		         GET_CLASS(ch, CLASS_ETHERMANCER) || GET_CLASS(ch, CLASS_ALCHEMIST) || GET_CLASS(ch, CLASS_PSIONICIST) || GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_PRIEST))
		{
			to_hit = get_property("to.hit.ClericTypes", 6);
		}
		else if (GET_CLASS(ch, CLASS_SORCERER) || GET_CLASS(ch, CLASS_CONJURER) || GET_CLASS(ch, CLASS_NECROMANCER) || GET_CLASS(ch, CLASS_THEURGIST) || GET_CLASS(ch, CLASS_ILLUSIONIST) ||
		         GET_CLASS(ch, CLASS_MINDFLAYER) || GET_CLASS(ch, CLASS_SUMMONER) || GET_CLASS(ch, CLASS_DRAGOON))
		{
		...
		if (ch->equipment[PRIMARY_WEAPON] && IS_SET(ch->equipment[PRIMARY_WEAPON]->extra2_flags, ITEM2_BLESS))
		{
	...
	int chance_to_hit(P_char ch, P_char victim, int skill, P_obj weapon)
	{
		...
		if (((IS_EVIL(ch) && !IS_EVIL(victim) && IS_AFFECTED(victim, AFF_PROTECT_EVIL)) || (IS_GOOD(ch) && !IS_GOOD(victim) && IS_AFFECTED(victim, AFF_PROTECT_GOOD))))
		{
		...
		{
			victim_ac += (int)(40 * (120 - GET_CHAR_SKILL_P(victim, SKILL_BLINDFIGHTING)) / 100);
		}
	...
	bool monk_critic(P_char ch, P_char victim, int *damAccumulator)
	{
		...
		if (GET_SPEC(ch, CLASS_MONK, SPEC_WAYOFSNAKE) || (GET_CLASS(ch, CLASS_MONK) && IS_NPC(ch) && GET_LEVEL(ch) > 50))
		{
	...
	void event_tainted_blade(P_char ch, P_char victim, P_obj obj, void *data)
	{
		...
		struct affected_type   *af;
		struct damage_messages  tainted_messages = {"$N &+Lstruggles against the &+wtaint &+Lcoursing through $S body.&n",
		...
		struct damage_messages  holy_messages    = {"&+rThe wound burns with a &+wsoft white glow.",
		...
		if (af->modifier-- > 0)
		{
			add_event(event_tainted_blade, (int)(IS_AFFECTED(victim, AFF_SLOW_POISON) ? 1.5 : 1) * PULSE_VIOLENCE, ch, victim, 0, 0, 0, 0);
		}
		else
		{
			affect_remove(victim, af);
	...
	bool tainted_blade(P_char ch, P_char victim)
	{
		...
		struct damage_messages  tainted_messages = {"$N &+wpales &+Las your &+wtainted &+Lweapon strikes&n $M.",
		                                            ...
		                                            0,
		                                            ch->equipment[WIELD]};
		struct damage_messages  holy_messages    = {"&+WThe holy aura surrounding $q burns into&n $N.",
	...
	bool frightening_presence(P_char ch, P_char victim)
	{
		...
		{
			act("&+rTerror&+L unlike anything you have ever felt overwhelms you.", TRUE, ch, 0, victim, TO_CHAR);
			act("Unable to stand the aura of fear surrounding $N $n turns to flee.", TRUE, ch, 0, victim, TO_ROOM);
	...
	int anatomy_strike(P_char ch, P_char victim, int msg, struct damage_messages *messages, int dam)
	{
		...
		switch (number(0, 6))
		{
			case 0:
				...
				{
					snprintf(messages->attacker, MAX_STRING_LENGTH, "Your%%s %s hits $N on the torso making $M grimace in pain.", attack_hit_text[msg].singular);
					snprintf(messages->victim, MAX_STRING_LENGTH, "$n's%%s %s hits $N on the torso making $M grimace in pain.", attack_hit_text[msg].singular);
					snprintf(messages->room, MAX_STRING_LENGTH, "$n's%%s %s hits $N on the torso making $M grimace in pain.", attack_hit_text[msg].singular);
			...
			case 1:
				if (!LEGLESS(victim))
				{
					snprintf(messages->attacker, MAX_STRING_LENGTH, "Your%%s %s hits $N across the leg, resulting in a limp stride.", attack_hit_text[msg].singular);
					snprintf(messages->victim, MAX_STRING_LENGTH, "$n's%%s %s hits $N across the leg, resulting in a limp stride.", attack_hit_text[msg].singular);
					snprintf(messages->room, MAX_STRING_LENGTH, "$n's%%s %s hits $N across the leg, resulting in a limp stride.", attack_hit_text[msg].singular);
					messages->type = DAMMSG_HIT_EFFECT;
					GET_VITALITY(victim) -= 5;
				}
				return dam;
			case 2:
				act("You surprise $N with a blow to the back, leaving $M momentarily confused.", FALSE, ch, 0, victim, TO_CHAR);
				act("$n surprises you with a blow to the back, leaving you momentarily confused.", FALSE, ch, 0, victim, TO_VICT);
				act("$n surprises $N with a blow to $S back, leaving $M momentarily confused.", FALSE, ch, 0, victim, TO_NOTVICT);
				victim->specials.combat_tics = (int)victim->specials.base_combat_round;
				goto regular;
			case 3:
				if (IS_HUMANOID(victim))
				{
					snprintf(messages->attacker, MAX_STRING_LENGTH, "Your%%s %s reached $N's arm severing tendons and muscles.", attack_hit_text[msg].singular);
					snprintf(messages->victim, MAX_STRING_LENGTH, "$n's%%s %s reached your arm severing tendons and muscles.", attack_hit_text[msg].singular);
					snprintf(messages->room, MAX_STRING_LENGTH, "$n's%%s %s reached $N's arm severing tendons and muscles.", attack_hit_text[msg].singular);
					messages->type = DAMMSG_HIT_EFFECT;
			...
			case 4:
				if (IS_HUMANOID(ch))
				{
					act("You strike viciously at $N's wrist causing $M to swing about madly.", FALSE, ch, 0, victim, TO_CHAR);
					act("$n strikes viciously at your wrist causing you to swing about madly.", FALSE, ch, 0, victim, TO_VICT);
					act("$n strikes viciously at $N's wrist causing $M to swing about madly.", FALSE, ch, 0, victim, TO_NOTVICT);
					af.duration = victim->specials.combat_tics + 1;
			...
			case 5:
				...
				{
					act("You viciously strike at $N's face, causing them to lose their concentration!", FALSE, ch, 0, victim, TO_CHAR);
					act("$n viciously strikes at your face, causing you to lose your concentration!", FALSE, ch, 0, victim, TO_VICT);
					act("$n viciously strikes at $N's face, causing them to lose their concentration!", FALSE, ch, 0, victim, TO_NOTVICT);
					StopCasting(victim);
				}
				goto regular;
			case 6:
				if (IS_HUMANOID(victim))
				{
					snprintf(messages->attacker, MAX_STRING_LENGTH, "Your%%s %s reached $N's ear causing a gush of blood.", attack_hit_text[msg].singular);
					snprintf(messages->victim, MAX_STRING_LENGTH, "$n's%%s %s reached your ear causing a gush of blood.", attack_hit_text[msg].singular);
					snprintf(messages->room, MAX_STRING_LENGTH, "$n's%%s %s reached $N's ear causing a gush of blood.", attack_hit_text[msg].singular);
					messages->type = DAMMSG_HIT_EFFECT;
	...
	bool hit(P_char ch, P_char victim, P_obj weapon, int *damAccumulator)
	{
		...
		if (mount)
		{
			if (GET_CHAR_SKILL(victim, SKILL_MOUNTED_COMBAT) &&
			    (notch_skill(victim, SKILL_MOUNTED_COMBAT, get_property("skill.notch.defensive", 17)) || GET_CHAR_SKILL_P(victim, SKILL_MOUNTED_COMBAT) * 0.3 > number(0, 100)))
			{
		...
		wpn_skill     = (IS_PC(ch) || IS_AFFECTED(ch, AFF_CHARM)) ? GET_CHAR_SKILL(ch, wpn_skill_num) : MIN(100, GET_LEVEL(ch) * 2);
		// Thieves only get 1h slash for shortswords.
		if (wpn_skill_num == SKILL_1H_SLASHING && GET_SPEC(ch, CLASS_ROGUE, SPEC_THIEF) && weapon->value[0] != WEAPON_SHORTSWORD)
		{
			wpn_skill = 0;
		}
		...
		*/
		...
		{
			act("You try to find a vital spot on your enemy, but $S body is too amorphous!", FALSE, ch, NULL, victim, TO_CHAR);
			act("$e tried to find a vital spot on you, but your body is too amorphous for that!", FALSE, ch, NULL, victim, TO_VICT);
			sic = 0;
		}
		...
		if (sic == -1 && !IS_AFFECTED2(victim, AFF2_STUNNED) && !IS_IMMOBILE(victim) &&
		    (notch_skill(victim, SKILL_QUICK_STEP, get_property("skill.notch.criticalAttack", 10)) || GET_CHAR_SKILL(victim, SKILL_QUICK_STEP) > number(1, 100)))
		{
			bool qs = FALSE;
			if (GET_POS(victim) == POS_STANDING)
			{
		...
		if (IS_AFFECTED4(victim, AFF4_DAZZLER) && !IS_AFFECTED5(ch, AFF5_DAZZLEE) && !NewSaves(ch, SAVING_SPELL, 5) && !has_innate(ch, INNATE_EYELESS) && !IS_AFFECTED(ch, AFF_BLIND) &&
		...
		if (!weapon && affected_by_spell(ch, SPELL_VAMPIRIC_TOUCH) && !IS_UNDEADRACE(victim) && !IS_CONSTRUCT(victim) && !NewSaves(victim, SAVING_PARA, 0))
		{
		...
		{
			if (!IS_HUMANOID(victim) || IS_UNDEADRACE(victim) || IS_ANGEL(victim) || IS_GREATER_RACE(victim) || IS_ELITE(victim))
			{
				send_to_char("&=LWYou score a CRITICAL HIT!!!!!&N\r\n", ch);
				if (!number(0, 9))
					make_bloodstain(victim);
		...
		if (sic == -1 && (notch_skill(ch, SKILL_CRITICAL_ATTACK, get_property("skill.notch.criticalAttack", 10)) || (1 * GET_CHAR_SKILL(ch, SKILL_CRITICAL_ATTACK)) > number(1, 100)))
		{
			critical_attack(ch, victim, msg);
		}

		if (has_innate(ch, INNATE_BATTLE_FRENZY) && !number(0, 20) && IS_HUMANOID(victim) && (battle_frenzy(ch, victim) != DAM_NONEDEAD))
		{
			return FALSE;
		}

		if (GET_CHAR_SKILL(ch, SKILL_VICIOUS_ATTACK) > 0 && !affected_by_spell(ch, SKILL_WHIRLWIND) && !vicious_hit && GET_POS(ch) == POS_STANDING && GET_RACE(victim) != RACE_CONSTRUCT)
		{
			if (notch_skill(ch, SKILL_VICIOUS_ATTACK, get_property("skill.notch.offensive.auto", 4)) || 0.1 * GET_CHAR_SKILL(ch, SKILL_VICIOUS_ATTACK) > number(0, 100))
			{
		...
		*/
		...
		if (vs_skill > 0 && (notch_skill(ch, SKILL_VICIOUS_STRIKE, get_property("skill.notch.offensive.vicious.strike", 17)) || vs_skill > number(1, 1000)))
		{
			if (IS_PC(ch))
			{
		 ...
		 */
		// 5% for elite mobs.
		else if (IS_NPC(ch) && IS_ELITE(ch) && get_property("skill.anatomy.NPC", 5.000) <= number(1, 100) && IS_HUMANOID(victim) && IS_HUMANOID(ch))
		{
			dam = anatomy_strike(ch, victim, msg, &messages, (int)dam);
		}
		else if (GET_CHAR_SKILL(ch, SKILL_ANATOMY) && IS_HUMANOID(victim) && IS_HUMANOID(ch) && GET_CHAR_SKILL(ch, SKILL_ANATOMY) / 25 >= (number(1, 100)))
		{
			dam = anatomy_strike(ch, victim, msg, &messages, (int)dam);
		}
		else
		...
		if (has_innate(victim, INNATE_WEAPON_IMMUNITY))
		{
			...
			else if (GET_LEVEL(ch) < 51 && (!ch->equipment[WEAR_HANDS] || !IS_SET(ch->equipment[WEAR_HANDS]->extra2_flags, ITEM2_MAGIC)))
			{
				dam = 1;
			}
		}
		...
		if (affected_by_spell(ch, ACH_YOUSTRAHDME) && IS_UNDEADRACE(victim) && lightbringer_proc(ch, victim, TRUE))
		{
			return TRUE;
		}

		blade_skill = GET_CLASS(ch, CLASS_AVENGER) ? SKILL_HOLY_BLADE : SKILL_TAINTED_BLADE;
		if (notch_skill(ch, blade_skill, get_property("skill.notch.offensive.auto", 4)) || (GET_CHAR_SKILL(ch, blade_skill) >= number(1, 100) && !number(0, 19)))
		{
		...
		if (weapon && is_char_in_room(ch, room) && is_char_in_room(victim, room) && IS_ALIVE(victim) && (!IS_PC_PET(ch) || OBJ_VNUM(weapon) != 1251))
		{
	...
	void StopMercifulAttackers(P_char ch)
	{
		...
		{
			hold = t_ch->specials.next_fighting;
			if ((GET_OPPONENT(t_ch) == ch) && !affected_by_spell(t_ch, SKILL_BERSERK) && ((IS_PC(t_ch) && !IS_SET(t_ch->specials.act, PLR_VICIOUS)) || (IS_NPC(t_ch) && !is_aggr_to(t_ch, ch))))
			{
	...
	void set_fighting(P_char ch, P_char vict)
	{
		...
		if ((ch == victim) || !SanityCheck(ch, "set_fighting - ch") || !SanityCheck(victim, "set_fighting - victim"))
		{
			return;
		}
		...
		if ((IS_PC(ch) || IS_PC_PET(ch)) && // IS_PC(vict) &&
		    (!on_front_line(ch) || !on_front_line(vict)) && !(ch->equipment[PRIMARY_WEAPON] && IS_REACH_WEAPON(ch->equipment[PRIMARY_WEAPON])))
			return;
		...
		if (affected_by_spell(ch, SONG_CHARMING) && ((ch->following && (ch->following == victim)) || (victim->following && (victim->following == ch))))
		...
		if (ch && victim)
		{
			if (HAS_MEMORY(victim))
			{
				...
				else if (IS_PC_PET(ch) && (GET_MASTER(ch)->in_room == ch->in_room) && CAN_SEE(victim, GET_MASTER(ch)))
				{
					if (!(IS_TRUSTED(GET_MASTER(ch)) && IS_SET(GET_MASTER(ch)->specials.act, PLR_AGGIMMUNE)))
						if (GET_STAT(victim) > STAT_INCAP)
							remember(victim, GET_MASTER(ch));
				}
			}
	...
	int leapSucceed(P_char victim, P_char attacker)
	{
		...
		{
			act("You leap into the air, avoiding $n's attack.", FALSE, attacker, 0, victim, TO_VICT);
			act("$N leaps into the air, avoiding your attack.", FALSE, attacker, 0, victim, TO_CHAR);
			act("$N leaps over $n's attack.", FALSE, attacker, 0, victim, TO_NOTVICT);
		}
		else
		{
			act("You move with lightning speed, quickly evading $n's attack.", FALSE, attacker, 0, victim, TO_VICT);
			act("$N moves with lightning speed, evading your attack.", FALSE, attacker, 0, victim, TO_CHAR);
			act("$N moves with lightning speed, evading $n's attack.", FALSE, attacker, 0, victim, TO_NOTVICT);
		}
		return true;
	}
	...
	int dodgeSucceed(P_char char_dodger, P_char attacker, P_obj wpn)
	{
		...
		if (mount && MIN_POS(mount, POS_STANDING + STAT_NORMAL) && MIN_POS(char_dodger, POS_STANDING + STAT_NORMAL) && !IS_STUNNED(mount) && !IS_BLIND(mount) && !IS_STUNNED(char_dodger) &&
		    !IS_BLIND(char_dodger) && (notch_skill(char_dodger, SKILL_SIDESTEP, get_property("skill.notch.defensive", 17)) || GET_CHAR_SKILL(char_dodger, SKILL_SIDESTEP) / 5 > number(0, 100)))
		{
			act("Your mount sidesteps $n's blow.", FALSE, attacker, 0, char_dodger, TO_VICT | ACT_NOTTERSE);
			act("$N sidesteps your attack causing you to miss.", FALSE, attacker, 0, mount, TO_CHAR | ACT_NOTTERSE);
			act("$N's mount sidesteps $n's blow.", FALSE, attacker, 0, char_dodger, TO_NOTVICT | ACT_NOTTERSE);
			return 1;
		}
		...
		if (has_innate(char_dodger, INNATE_GIANT_AVOIDANCE) && ((GET_RACE(attacker) == RACE_TROLL) || (GET_RACE(attacker) == RACE_OGRE) || (GET_RACE(attacker) == RACE_GIANT)))
		{
			learned *= 1.10;
		}

		// Everybody receives these values. -maybe change this? Drannak
		learned += (int)(((STAT_INDEX(GET_C_AGI(char_dodger))) - (STAT_INDEX(GET_C_DEX(attacker)))) / 2);
		...
		// Modifiers
		if ((has_innate(char_dodger, INNATE_GROUNDFIGHTING) && !MIN_POS(char_dodger, POS_STANDING + STAT_NORMAL)) || affected_by_spell(char_dodger, SKILL_GAZE))
		{
		...
		if (GET_SPEC(char_dodger, CLASS_CLERIC, SPEC_ZEALOT) || GET_SPEC(char_dodger, CLASS_BARD, SPEC_SCOUNDREL))
		{
			percent += 5;
		}
		...
		if (GET_RACE(char_dodger) == RACE_DROW && !IS_STUNNED(char_dodger) && (load_modifier(char_dodger) < 100) && !number(0, 10))
		{
		...
		if (GET_CLASS(char_dodger, CLASS_MONK))
		{
			if (number(0, 1))
			{
				act("You easily lean out of range of $n's vicious attack.", FALSE, attacker, 0, char_dodger, TO_VICT | ACT_NOTTERSE);
				act("$N nimbly swivels out of the path of your attack.", FALSE, attacker, 0, char_dodger, TO_CHAR | ACT_NOTTERSE);
				act("$N nimbly swivels out of the path of $n's attack.", FALSE, attacker, 0, char_dodger, TO_NOTVICT | ACT_NOTTERSE);
			}
			else
			{
				act("You whirl around, just avoiding $n's vicious attack.", FALSE, attacker, 0, char_dodger, TO_VICT | ACT_NOTTERSE);
				act("$N gracefully whirls around your attack.", FALSE, attacker, 0, char_dodger, TO_CHAR | ACT_NOTTERSE);
				act("$N gracefully whirls around $n's attack.", FALSE, attacker, 0, char_dodger, TO_NOTVICT | ACT_NOTTERSE);
			}
		}
		else
		{
			if (!dragoon_dodge)
			{
				act("You dodge $n's vicious attack.", FALSE, attacker, 0, char_dodger, TO_VICT | ACT_NOTTERSE);
				act("$N dodges your futile attack.", FALSE, attacker, 0, char_dodger, TO_CHAR | ACT_NOTTERSE);
				act("$N dodges $n's attack.", FALSE, attacker, 0, char_dodger, TO_NOTVICT | ACT_NOTTERSE);
			}
			else
			{
				act("Your sense $n's vicious attack, as your blood burns with &+rpower&n.", FALSE, attacker, 0, char_dodger, TO_VICT | ACT_NOTTERSE);
				act("$N senses your attack and swiftly manuevers away.", FALSE, attacker, 0, char_dodger, TO_CHAR | ACT_NOTTERSE);
				act("$N senses $n's attack and swiftly manuevers away.", FALSE, attacker, 0, char_dodger, TO_NOTVICT | ACT_NOTTERSE);
			}
		}
		return 1;
	}
	...
	int blockSucceed(P_char victim, P_char attacker, P_obj wpn)
	{
		...
		if (!GET_CHAR_SKILL(victim, SKILL_SHIELD_BLOCK) || !(shield = victim->equipment[WEAR_SHIELD]))
			return false;
		...
		if ((wpn && IS_FLAYING(wpn)) || (attacker->equipment[WIELD] && IS_FLAYING(attacker->equipment[WIELD])))
		...
		{
			act("Your shield &+Yflares&n up and discharges a bolt of &+Blightning&n towards $n.", FALSE, attacker, 0, victim, TO_VICT);
			act("$N's shield &+Yflares&n up and discharges a bolt of &+Blightning&n towards YOU!", FALSE, attacker, 0, victim, TO_CHAR);
			act("$N's shield &+Yflares&n up and discharges a bolt of &+Blightning&n towards $n.", FALSE, attacker, 0, victim, TO_NOTVICT);
		...
		if (GET_CHAR_SKILL(victim, SKILL_IMPROVED_SHIELD_COMBAT) && number(1, 400) < MAX(20, GET_CHAR_SKILL(victim, SKILL_IMPROVED_SHIELD_COMBAT)) && MIN_POS(victim, POS_STANDING + STAT_NORMAL))
		{
			struct damage_messages messages = {"You block $N's lunge at you and immediately slam $M with your shield!",
			...
			int dmg = number(40, MAX(41, GET_CHAR_SKILL(victim, SKILL_IMPROVED_SHIELD_COMBAT) * (int)get_property("skill.improvedShieldCombat.damageMultiplier", 1)));
		...
		{
			act("You expertly block $n's lunge at you.", FALSE, attacker, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("$N easily blocks your futile lunge at $M.", FALSE, attacker, 0, victim, TO_CHAR | ACT_NOTTERSE);
			act("$N easily blocks $n's lunge at $M.", FALSE, attacker, 0, victim, TO_NOTVICT | ACT_NOTTERSE);
			return true;
		}
		else if (learned < 20)
		{
			act("You barely block $n's lunge at you.", FALSE, attacker, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("$N just barely gets $S shield up to block your attack.", FALSE, attacker, 0, victim, TO_CHAR | ACT_NOTTERSE);
			act("$N barely blocks $n's lunge at $M.", FALSE, attacker, 0, victim, TO_NOTVICT | ACT_NOTTERSE);
			return true;
		}
		else
		{
			act("You block $n's lunge at you.", FALSE, attacker, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("$N blocks your futile lunge at $M.", FALSE, attacker, 0, victim, TO_CHAR | ACT_NOTTERSE);
			act("$N blocks $n's lunge at $M.", FALSE, attacker, 0, victim, TO_NOTVICT | ACT_NOTTERSE);
			return true;
		}
		return false;
	}
	...
	int MonkRiposte(P_char victim, P_char attacker, P_obj wpn)
	{
		...
		if (!(attacker) || !(victim) || !GET_CLASS(victim, CLASS_MONK) || !IS_ALIVE(victim) || !(skl) || !IS_ALIVE(attacker) || IS_IMMOBILE(victim) || IS_BLIND(victim) || !IS_AWAKE(victim) ||
		    IS_STUNNED(victim))
		{
			return 0;
		}

		if (IS_PC(victim) && notch_skill(victim, SKILL_MARTIAL_ARTS, get_property("skill.notch.defensive", 17)))
			return 0;
		...
		percent -= (str_app[STAT_INDEX(GET_C_STR(attacker))].tohit + str_app[STAT_INDEX(GET_C_STR(attacker))].todam);
		...
		if (!MIN_POS(victim, POS_STANDING + STAT_NORMAL))
		{
			...
			if (GET_C_AGI(victim) > number(1, 1000) && !get_spell_from_char(victim, SKILL_MARTIAL_ARTS) && GET_CHAR_SKILL(victim, SKILL_MARTIAL_ARTS) > number(1, 100))
			{
				act("$n tucks in $s arms, rolls quickly away, then thrust $s feet skywards, leaping back to $s feet!", TRUE, victim, 0, attacker, TO_NOTVICT);
				act("$n tucks in $s arms, rolls away from $N's attack, then thrust $s feet skywards, and leaps to $s feet!", TRUE, victim, 0, attacker, TO_VICT);
				act("You tuck in your arms, roll away from $N's blow, then leap to your feet!", TRUE, victim, 0, attacker, TO_CHAR);
		...
		if (number(1, 150) > skl)
		{
			act("$n arches around $N's blow, and deals a brutal counterattack!", TRUE, victim, 0, attacker, TO_NOTVICT);
			act("$n arches around your blow, and deals a brutal counterattack to YOU!", TRUE, victim, 0, attacker, TO_VICT);
			act("You arch around $N's blow, and deal a brutal counterattack!", TRUE, victim, 0, attacker, TO_CHAR);
			hit(victim, attacker, NULL);
			return false;
		}

		act("$n completely sidesteps $N's blow, shifts $s balance, and strikes back at $M!", TRUE, victim, 0, attacker, TO_NOTVICT);
		act("$n completely sidesteps your blow, shifts $s balance, and strikes back at YOU!", TRUE, victim, 0, attacker, TO_VICT);
		act("You completely sidestep $N's blow, shift your balance, and strike back at $M!", TRUE, victim, 0, attacker, TO_CHAR);
	...
	int parrySucceed(P_char victim, P_char attacker, P_obj wpn)
	{
		...
		// parry against).
		if (!victim->equipment[WIELD] && !victim->equipment[WIELD2] && !victim->equipment[WIELD3] && !victim->equipment[WIELD4])
			return FALSE;

		// Flaying weapons are !parry.
		if ((wpn && IS_FLAYING(wpn)) || (victim->equipment[WIELD] && IS_FLAYING(victim->equipment[WIELD])))
			return FALSE;
		...
		if (notch_skill(victim, SKILL_PARRY, get_property("skill.notch.defensive", 17)) && !affected_by_spell(victim, SPELL_COMBAT_MIND))
			return FALSE;
		...
		if (GET_SPEC(victim, CLASS_RANGER, SPEC_BLADEMASTER) || GET_SPEC(victim, CLASS_WARRIOR, SPEC_SWASHBUCKLER))
			learnedvictim = (int)(learnedvictim * 1.10);

		//  Better chance for ap's
		if ((GET_CLASS(victim, CLASS_ANTIPALADIN) || GET_CLASS(victim, CLASS_PALADIN)) && is_wielding_paladin_sword(victim))
		...
		{
			act("You anticipate $n's maneuver and &+wmasterfully&n parry the attack.", FALSE, attacker, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("$N anticipates your attack and &+wmasterfully&n parries your blow.", FALSE, attacker, 0, victim, TO_CHAR | ACT_NOTTERSE);
			act("$N anticipates $n's attack and &+wmasterfully&n parries the incoming blow.", FALSE, attacker, 0, victim, TO_NOTVICT | ACT_NOTTERSE);
		}
		...
		{
			act("You parry $n's lunge at you.", FALSE, attacker, 0, victim, TO_VICT | ACT_NOTTERSE);
			act("$N parries your futile lunge at $M.", FALSE, attacker, 0, victim, TO_CHAR | ACT_NOTTERSE);
			act("$N parries $n's lunge at $M.", FALSE, attacker, 0, victim, TO_NOTVICT | ACT_NOTTERSE);
		}
		return TRUE;
	}
	...
	*/
	...
	void stop_destroying(P_char ch)
	{
		...
		{
			for (tmp = destroying_list; tmp && (tmp->specials.next_destroying != ch); tmp = tmp->specials.next_destroying)
				;
			if (!tmp)
			{
	...
	{
		int                    hits;
		struct damage_messages messages = {"$N &+RSCR&+rEA&+RMS &nin pain as their body is filled with a sudden rush of &+rWa&+yrm&+rth!",
	...
	int calculate_attacks(P_char ch, int attacks[])
	{
		...
		{ // not MONK
			if (!IS_AFFECTED2(ch, AFF2_SLOW) || IS_AFFECTED(ch, AFF_HASTE))
			{
				...
				if (HAS_FOUR_HANDS(ch) && ch->equipment[THIRD_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 50) >= number(1, 100)))
				{
			...
			if (ch->equipment[PRIMARY_WEAPON] && ch->equipment[SECONDARY_WEAPON] && (ch->equipment[PRIMARY_WEAPON] != ch->equipment[SECONDARY_WEAPON]))
			{
				if (notch_skill(ch, SKILL_DUAL_WIELD, get_property("skill.notch.offensive.auto", 4)) || number(1, 100) < GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD))
				{
					ADD_ATTACK(SECONDARY_WEAPON);
					// Can swing both secondary hands at once - 50% for newbs, 100% for maxxed dual wield.
					if (HAS_FOUR_HANDS(ch) && ch->equipment[FOURTH_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 50) >= number(1, 100)))
					{
			...
			*/
				...
				if (GET_C_STR(ch) > number(1, 340))
				{
					...
					switch (zealproc)
					{
						case 1:
							...
							act("&nThe power of your &+Wgod&n suddenly fills your body and you feel &+BMUCH&n stronger!&n", FALSE, ch, 0, 0, TO_CHAR);
							act("&n$n gasps suddenly as their body is &+Benhanced&n by some &+Wother-worldly &npower!&n", FALSE, ch, 0, 0, TO_ROOM);
							do_say(ch, "Fill me with your strength!", CMD_SAY);
							break;
						case 2:
							act("&+WYour devotion to your god causes you to unleash a &+rF&+Rlurr&+ry&+W of attacks against $N!&n", FALSE, ch, NULL, GET_OPPONENT(ch), TO_CHAR);
						...
						case 3:
							...
							act("&nThe power of your &+Wgod&n suddenly fills your body and you feel &+BMUCH&n stronger!&n", FALSE, ch, 0, 0, TO_CHAR);
							act("&n$n gasps suddenly as their body is &+Benhanced&n by some &+Wother-worldly &npower!&n", FALSE, ch, 0, 0, TO_NOTVICT);
							act("&+WYour devotion to your god causes you to unleash a &+rF&+Rlurr&+ry&+W of attacks against $N!&n", FALSE, ch, 0, GET_OPPONENT(ch), TO_CHAR);
			...
			if (notch_skill(ch, SKILL_DOUBLE_ATTACK, get_property("skill.notch.offensive.auto", 4)) || GET_CHAR_SKILL(ch, SKILL_DOUBLE_ATTACK) >= number(1, 100))
			{
				ADD_ATTACK(PRIMARY_WEAPON);
				...
				if (HAS_FOUR_HANDS(ch) && ch->equipment[FOURTH_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 45) > number(1, 100)))
				{
					ADD_ATTACK(FOURTH_WEAPON);
				}
			}

			if (notch_skill(ch, SKILL_TRIPLE_ATTACK, get_property("skill.notch.offensive.auto", 4)) || GET_CHAR_SKILL(ch, SKILL_TRIPLE_ATTACK) >= number(1, 100))
			{
				ADD_ATTACK(PRIMARY_WEAPON);
				// Can swing both primary hands at once - 94% chance max.
				if (HAS_FOUR_HANDS(ch) && ch->equipment[THIRD_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 45) > number(1, 100)))
				{
					ADD_ATTACK(THIRD_WEAPON);
				}
				// Can swing primary secondary hand - 94% chance max.
				if (HAS_FOUR_HANDS(ch) && ch->equipment[SECONDARY_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 45) > number(1, 100)))
				{
					ADD_ATTACK(SECONDARY_WEAPON);
				}
			}

			if (notch_skill(ch, SKILL_QUADRUPLE_ATTACK, get_property("skill.notch.offensive.auto", 4)) || GET_CHAR_SKILL(ch, SKILL_QUADRUPLE_ATTACK) > number(1, 100))
			{
				ADD_ATTACK(PRIMARY_WEAPON);
				// Can swing both primary hands at once - 94% chance max.
				if (HAS_FOUR_HANDS(ch) && ch->equipment[THIRD_WEAPON] && ((GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) / 2 + 45) > number(1, 100)))
		...
		if ((weapon == NULL) || (weapon->type == ITEM_WEAPON))
		{
			...
			if (((actpct <= 6) && (GET_C_DEX(ch) >= 125)) || ((actpct <= 20) && (GET_C_DEX(ch) >= 150)))
			{
		...
		*/
		...
		if (GET_CLASS(ch, CLASS_DREADLORD | CLASS_AVENGER) && GET_CHAR_SKILL(ch, required_weapon_skill(ch->equipment[PRIMARY_WEAPON])) > 69)
			ADD_ATTACK(PRIMARY_WEAPON);
		...
		if (affected_by_spell(ch, SPELL_KANCHELSIS_FURY) && GET_CLASS(ch, CLASS_REAVER))
		{
			...
			if (GET_LEVEL(ch) >= 41 && ch->equipment[SECONDARY_WEAPON] && number(1, 100) < GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD))
				ADD_ATTACK(SECONDARY_WEAPON);
		...
		{
			ADD_ATTACK(PRIMARY_WEAPON);
			if ((GET_CLASS(ch, CLASS_RANGER) || GET_SECONDARY_CLASS(ch, CLASS_RANGER)) && number(1, 100) < GET_CHAR_SKILL(ch, SKILL_DUAL_WIELD) && ch->equipment[SECONDARY_WEAPON])
				ADD_ATTACK(SECONDARY_WEAPON);

			int blurattackchance = (GET_LEVEL(ch) / 2);
			if (GET_CLASS(ch, CLASS_RANGER) && (number(1, 100) < blurattackchance) && ch->equipment[SECONDARY_WEAPON])
				ADD_ATTACK(SECONDARY_WEAPON);
		}
...
#define MAX_ATTACKS    256
	void perform_violence(void)
	{
		...
		for (ch = combat_list; ch; ch = combat_next_ch)
		{
			...
			if (!opponent)
			{
				if (ch->in_room != NOWHERE)
				{
					debug("perform_violence: %s fighting null opponent in %s (%d)!", GET_NAME(ch), world[ch->in_room].name, world[ch->in_room].number);
					logit(LOG_DEBUG, "perform_violence: %s fighting null opponent in %s (%d)!", GET_NAME(ch), world[ch->in_room].name, world[ch->in_room].number);
					extract_char(ch);
					return;
				}
				else
			...
			if (melee_exp_pulse && opponent && !IS_IMMOBILE(ch) && (opponent != ch) && IS_PC(opponent) && (opponent->in_room == ch->in_room))
			{
			...
			if (!affected_by_spell(opponent, SKILL_BATTLE_SENSES) && GET_CHAR_SKILL(opponent, SKILL_BATTLE_SENSES) && GET_POS(opponent) == POS_STANDING && !IS_STUNNED(opponent) && !IS_BLIND(opponent))
			{
				...
				{
					act("&+wA sense of awareness &+bflows &+wover you as you move into &+rbattle.&n", FALSE, opponent, 0, 0, TO_CHAR);
					act("$n maneuvers around you like a &+yviper!&n", FALSE, opponent, 0, 0, TO_VICT);
					act("$n &+bflows into battle with the speed of a &+yviper.&n", FALSE, opponent, 0, 0, TO_NOTVICT);
			...
			{
				if (affected_by_spell(opponent, SPELL_SANGUINIS_IGNIS) && !affected_by_spell(opponent, SPELL_SANGUINIS_IGNIS_AFF) && GET_POS(opponent) == POS_STANDING && !IS_STUNNED(opponent) &&
				    !IS_BLIND(opponent))
				{
					...
					{
						act("You smell &+rblood&n as you maneuver your &+Gdr&+Lag&+Gon&n into battle!", FALSE, opponent, 0, 0, TO_CHAR);
						act("$n maneuvers $s &+Gdr&+Lag&+Gon&n around you, tracking your movements!&n", FALSE, opponent, 0, 0, TO_VICT);
						act("$n maneuvers $s &+Gdr&+Lag&+Gon&n into battle, tracking $s prey!&n", FALSE, opponent, 0, 0, TO_NOTVICT);
			...
			if (!IS_SUNLIT(ch->in_room))
			{
				if (GET_CHAR_SKILL(ch, SKILL_SHADOW_MOVEMENT) && !IS_BLIND(ch) && !IS_STUNNED(ch) && GET_POS(ch) == POS_STANDING &&
				    (notch_skill(ch, SKILL_SHADOW_MOVEMENT, get_property("skill.notch.offensive.auto", 4)) || (1 + GET_CHAR_SKILL(ch, SKILL_SHADOW_MOVEMENT) / 4 > number(1, 100))))
				{
					act("$n &+wblinks out of existence ... then reappears &+ybehind&n $N!&n", FALSE, ch, 0, opponent, TO_NOTVICT);
					act("&+wYou blink out of existence and reappear behind&n $N.&n", FALSE, ch, 0, opponent, TO_CHAR);
					if (!IS_BLIND(opponent))
						act("$n &+wblinks out of existence ... then reappears &+ybehind you!&n", FALSE, ch, 0, opponent, TO_VICT);
	...
	void double_strike(P_char ch, P_char victim, P_obj wpn)
	{
		...
		{
			if (IS_TRUSTED(tch) || !on_front_line(tch) || tch == ch || !IS_FIGHTING(tch) || tch == victim || (ch->group && ch->group == tch->group))
				continue;
			count++;
		}
	...
	{
		static struct damage_messages monk_superhit_messages = {"You hit a pressure point on $N with additional &+rforce&n.",
	...
	int pv_common(P_char ch, P_char opponent, const P_obj wpn, int *damAccumulator)
	{
		...
		if (!((wpn_skill = required_weapon_skill(wpn)) && ((wpn_skill != SKILL_1H_FLAYING) || (wpn_skill != SKILL_2H_FLAYING)) &&
		      notch_skill(ch, wpn_skill, get_property("skill.notch.offensive.auto", 4))) &&
		    GET_STAT(opponent) == STAT_NORMAL && !IS_IMMOBILE(ch) && (has_innate(ch, INNATE_EYELESS) || CAN_SEE(opponent, ch) || GET_CHAR_SKILL(opponent, SKILL_BLINDFIGHTING) / 3 > number(0, 100)))
		{
			...
			{
				if (!IS_IMMOBILE(opponent) && (mangleSucceed(opponent, ch, wpn) || parrySucceed(opponent, ch, wpn) || divine_blessing_parry(opponent, ch) || blockSucceed(opponent, ch, wpn) ||
				                               dodgeSucceed(opponent, ch, wpn) || leapSucceed(opponent, ch) || MonkRiposte(opponent, ch, wpn)))
				{
					return FALSE;
				}
			}
		...
		if (IS_ALIVE(opponent) && IS_NPC(opponent) && mob_index[GET_RNUM(opponent)].func.mob && !affected_by_spell(opponent, TAG_CONJURED_PET))
		{
			data.victim = ch;
		...
		else if (notch_skill(ch, SKILL_DOUBLE_STRIKE, get_property("skill.notch.offensive.auto", 4)) || GET_CHAR_SKILL(ch, SKILL_DOUBLE_STRIKE) / 20 > number(0, 100) ||
	...
	{
		int                    dam;
		struct damage_messages messages1 = {"You slam your knee into $N's stomach, winding $M.&N",
		...
		struct damage_messages messages2 = {"You jam your elbow into $N's side, bruising $M!&N",
		...
		{
			act("$n attempts to knee you right in the stomach, but came up short.&N", TRUE, ch, NULL, victim, TO_VICT);
			act("$n tries to knee $N, but can't quite reach.&N", TRUE, ch, NULL, victim, TO_NOTVICT);
			act("You attempt to knee $N, but don't quite make it.&N", TRUE, ch, NULL, victim, TO_CHAR);
		}
		return 0;
	}
	...
	bool is_nopoof(P_obj obj) { return IS_ARTIFACT(obj) || isname("quest", obj->name) || isname("fun", obj->name); }
	...
	void DestroyStuff(P_char victim, int type)
	{
		...
		for (slot = 0; slot < sizeof(proccing_slots) / sizeof(int); slot++)
		{
			...
			{
				//      poofed++;
				statuslog(AVATAR, "%s [%d] was destroyed.", item->short_description, (item->R_num >= 0) ? obj_index[item->R_num].virtual_number : 0);
	...
	bool fear_check(P_char ch, bool force)
	{
		...
		if (GET_CHAR_SKILL(ch, SKILL_INDOMITABLE_RAGE) > number(1, 150) && affected_by_spell(ch, SKILL_BERSERK))
		{
		...
		{
			act("&+yFear is for the weak willed... which you are not!&n", FALSE, ch, 0, 0, TO_CHAR);
			return TRUE;
		}
	...
	bool critical_attack(P_char ch, P_char victim, int msg)
	{
		...
		{
			snprintf(room_msg, MAX_STRING_LENGTH, "$n's mighty %s knocks $N's weapon from $S grasp!&n", attack_hit_text[msg].singular);
			snprintf(attacker_msg, MAX_STRING_LENGTH, "Your mighty %s knocks $N's weapon from $S grasp!&n", attack_hit_text[msg].singular);
			snprintf(victim_msg, MAX_STRING_LENGTH, "$n's mighty %s knocks your weapon from your grasp!&n", attack_hit_text[msg].plural);
		...
		if (random == 2)
		{
			if (affected_by_spell(victim, SPELL_STONE_SKIN))
			{
				...
				snprintf(victim_msg, MAX_STRING_LENGTH, "The magic protecting your body shatters as $n %s you!&N", attack_hit_text[msg].plural);
				act(victim_msg, TRUE, ch, NULL, victim, TO_VICT);

				snprintf(room_msg, MAX_STRING_LENGTH, "$n grins slightly as their %s drops $N's defenses!&N", attack_hit_text[msg].singular);
				act(room_msg, TRUE, ch, NULL, victim, TO_NOTVICT);
			...
			else if (affected_by_spell(victim, SPELL_SHADOW_SHIELD))
			{
				...
				snprintf(room_msg, MAX_STRING_LENGTH, "$n grins slightly as $s %s drops $N's defenses!&N", attack_hit_text[msg].singular);
				act(room_msg, TRUE, ch, NULL, victim, TO_NOTVICT);
		...
		if (random == 3)
		{
			snprintf(attacker_msg, MAX_STRING_LENGTH, "You follow up your %s with a surprise attack!&N", attack_hit_text[msg].singular);
			act(attacker_msg, TRUE, ch, NULL, victim, TO_CHAR);

			snprintf(victim_msg, MAX_STRING_LENGTH, "$n swiftly follows up his devastating %s with a surprise attack!&N", attack_hit_text[msg].singular);
			act(victim_msg, TRUE, ch, NULL, victim, TO_VICT);
