...
void enhance(P_char ch, P_obj source, P_obj material)
{
	...
	while (!validobj)
	{
		...
		else if (!IS_SET(robj->wear_flags, ITEM_TAKE) || IS_SET(robj->extra_flags, ITEM_ARTIFACT) || IS_SET(robj->extra_flags, ITEM_NOSELL) || IS_SET(robj->extra_flags, ITEM_NORENT) ||
		         IS_SET(robj->extra_flags, ITEM_NOSHOW) || IS_SET(robj->extra_flags, ITEM_TRANSIENT) || IS_OBJ_STAT2(robj, ITEM2_QUESTITEM))
		{
			validobj = FALSE;
			extract_obj(robj);
		}
		else if (itemvalue(robj) != newval || (robj->type == ITEM_STAFF && robj->value[3] > 0) || robj->type == ITEM_TREASURE || robj->type == ITEM_POTION || robj->type == ITEM_MONEY ||
