...
void spell_shadow_monster(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	{
		if (cld->type == LNK_PET && IS_NPC(cld->linking) && GET_VNUM(cld->linking) == 1106)
		{
			summoned++;
		}
	}
...
void spell_insects(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	for (cld = ch->linked; cld; cld = cld->next_linked)
	{
		if (cld->type == LNK_PET && IS_NPC(cld->linking) && GET_VNUM(cld->linking) == 1107)
		{
			summoned++;
		}
	}
	...
	for (i = MAX(1, GET_LEVEL(ch) / 14); i > summoned; i--)
	{
		...
		mob->player.m_class = 0;
		remove_plushit_bits(mob);
		GET_MAX_HIT(mob) = GET_HIT(mob) = mob->points.base_hit = dice(GET_LEVEL(mob), 3) + (GET_LEVEL(ch));
...
{
	int                    dam;
	struct damage_messages messages = {"Your huge &+Willusional boulder&n hits $N causing $M to scream out in agony!",
	                                   ...
	                                   "$n&n's &+Wboulder&N rolls into $N flattening $M!",
	                                   0};
	...
	if (!IS_AFFECTED2(victim, AFF2_GLOBE) && (GET_POS(victim) == POS_STANDING) && !IS_ELITE(victim) && !IS_GREATER_RACE(victim))
	{

		if (!number(0, 9))
		{
			act("&+LYour &+Wboulder&n &+Lhit's $N dead on, knocking $M to the ground!&n", 0, ch, 0, victim, TO_CHAR);
			act("$N is knocked to the ground as $n's &+Wboulder hit's $M dead on!&n", 0, ch, 0, victim, TO_NOTVICT);
			act("$n's &+Wboulder hit's dead on knocking you to the ground!&n", 0, ch, 0, victim, TO_VICT);
...
void spell_shadow_travel(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (IS_ROOM(ch->in_room, ROOM_NO_TELEPORT) || world[ch->in_room].sector_type == SECT_OCEAN || IS_HOMETOWN(ch->in_room))
	{
		send_to_char("&+yYou failed.\r\n", ch);
		return;
	}
	...
	if (IS_ROOM(location, ROOM_NO_TELEPORT) || IS_HOMETOWN(location) || world[location].sector_type == SECT_CASTLE || world[location].sector_type == SECT_CASTLE_GATE ||
	    world[location].sector_type == SECT_CASTLE_WALL || racewar(ch, victim) || (GET_MASTER(ch) && IS_PC(victim)))
	{
		send_to_char("&+yYou failed.\r\n", ch);
		return;
	}
	...
	if (victim && IS_PC(ch) && IS_PC(victim) && !is_Raidable(victim, 0, 0))
	{
	...
	{
		if ((IS_AFFECTED(tmp, AFF_BLIND) || (tmp->specials.z_cord != ch->specials.z_cord) || (tmp == ch) || !number(0, 5)) && (IS_PC(ch) && !IS_TRUSTED(ch)))
			continue;
		if (CAN_SEE(tmp, ch))
			act("&+L$n appears to become one with the shadows and silently starts to move away.", FALSE, ch, 0, tmp, TO_VICT);
		else
			act("&+LAn odd looking shadow forms and silently starts to move away.", FALSE, ch, 0, tmp, TO_VICT);
	...
	for (tmp = world[ch->in_room].people; tmp; tmp = tmp->next_in_room)
	{
		if ((IS_AFFECTED(tmp, AFF_BLIND) || (tmp == ch) || !number(0, 5)) && (IS_PC(ch) && !IS_TRUSTED(ch)))
			continue;
		if (CAN_SEE(tmp, ch))
			act("&+LAn odd looking shadow silently arrives and slowly forms into $n.", FALSE, ch, 0, tmp, TO_VICT);
		else
			act("&+LAn odd looking shadow silently arrives then vanishes.", FALSE, ch, 0, tmp, TO_VICT);
		send_to_char(buf, tmp);
	}
}
...
void spell_stunning_visions(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (!IS_ALIVE(ch) || !IS_ALIVE(victim) || GET_HIT(victim) < 1 || IS_TRUSTED(victim) || IS_ZOMBIE(victim) || GET_RACE(victim) == RACE_GOLEM || GET_RACE(victim) == RACE_PLANT ||
	...
	if (GET_POS(victim) != POS_STANDING && IS_ACT2(victim, PLR2_WAIT) && !(affected_by_spell(victim, SKILL_BASH) || affected_by_spell(ch, SKILL_SPRINGLEAP)))
	{
	...
	if (percent > 90)
	{
		...
		act("$n &+ccauses visions to dance before your eyes, causing you to fall in a deep trance!&n.", FALSE, ch, 0, victim, TO_VICT);
	...
	else if (percent > 60)
	{
		...
		act("$n &+ccauses visions to dance before your eyes, causing you to fall into a trance!&n.", FALSE, ch, 0, victim, TO_VICT);
...
void spell_reflection(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("&+L$n &+Cs&+Bs&+bs&+Cp&+Bp&+b&+Cl&+Bl&+bl&+Ci&+Bi&+bi&+Ct&+Bt&+bt&+Cs&+Bs&+bs&+L into many images!&N", FALSE, victim, 0, 0, TO_ROOM);
	send_to_char("&+LYou &+Cs&+Bs&+bs&+Cp&+Bp&+b&+Cl&+Bl&+bl&+Ci&+Bi&+bi&+Ct&+Bt&+bt&+Cs&+Bs&+bs&+L into many images!&N\r\n", victim);
	...
	for (i = 0; i < numb; i++)
	{
		...
		snprintf(Gbuf1, MAX_STRING_LENGTH, "image %s %s", GET_NAME(victim), race_names_table[GET_RACE(victim)].normal);
	...
	if (target)
	{
		...
		else
		{
			if (target)
			{
				...
				snprintf(tbuf, MAX_STRING_LENGTH, "&+LYour image shifts and &+bb&+Blur&+bs&+L into %s!&N\r\n", target->player.name);
...
void spell_watching_wall(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(
			ch->in_room, var, ch, level, WATCHING_WALL, level, 1800, "&+La greyish stone wall covered with hundreds of eyes&n", "&+yA massive stone wall embedded with eyes is here to the %s.&n", 0))
	{
		...
		snprintf(buf1, MAX_STRING_LENGTH, "&+yA massive eye-covered stone wall appears to the %s!&n\r\n", dirs[var]);
		snprintf(buf2, MAX_STRING_LENGTH, "&+yA massive eye-covered stone wall appears to the %s!&n\r\n", dirs[rev_dir[var]]);
...
void spell_illusionary_wall(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	char buf1[MAX_STRING_LENGTH], buf2[MAX_STRING_LENGTH], Gbuf4[MAX_STRING_LENGTH];
	int  var = 0;
	...
	if (create_walls(ch->in_room, var, ch, level, ILLUSIONARY_WALL, level, 1800, "&+wan illusionary wall&n", "&+LAn &Nil&+Wlusiona&Nry&+L wall is here to the %s.&n", ITEM_SECRET))
	{
		...
		SET_BIT(VIRTUAL_EXIT((world[ch->in_room].dir_option[var])->to_room, rev_dir[var])->exit_info, EX_ILLUSION);
	}
}
...
void spell_nightmare(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (IS_DEMON(victim) || IS_DRAGON(victim) || IS_TRUSTED(victim))
	{
		...
		act("How can you give $N nightmares? Nothing scares $M!", FALSE, ch, 0, victim, TO_CHAR);
	...
	if (!saves_spell(victim, SAVING_SPELL))
	{
		...
		{
			case RACE_TROLL:
				act("You show $N a nightmare of $M taking a bath!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of you taking a bath!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_HUMAN:
				act("You show $N a nightmare of an ogre sitting on $S face!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of an ogre sitting on your face!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_MOUNTAIN:
			case RACE_DUERGAR:
				act("You show $N a nightmare of $M getting $S beard cut off!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare with you getting your beard cut off!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_HALFLING:
				act("You show $N a nightmare of $M getting $S hand caught in a cookie jar!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare with you getting your hand caught in a cookie jar!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_OGRE:
				act("You show $N a nightmare of a little white mouse.", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of a little white mouse jumping in front of you!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_THRIKREEN:
				act("You show $N a nightmare of $M getting stuffed into a freezer.", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of you getting stuffed into a freezer!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_CENTAUR:
				act("You show $N a nightmare of a old lady storm giant trying to ride $M.", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of a old lady storm giant trying to mount you!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_MINOTAUR:
				act("You show $N a nightmare of $M getting killed by a bull fighter!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of you getting killed by a bull fighter!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_GOBLIN:
			case RACE_GNOME:
				act("You show $N a nightmare of $M getting stepped upon by a giant foot!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare with you getting stepped on!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			case RACE_LICH:
			case RACE_PVAMPIRE:
				act("You show $N a nightmare of $M getting stabbed by a wooden stake through the heart!", FALSE, ch, 0, victim, TO_CHAR);
				act("A wave of painful terror overcomes you as $n shows you a nightmare of you getting stabbed through the heart by a wooden stake!", FALSE, ch, 0, victim, TO_VICT);
				act("$n shows $N! his worst nightmare!", FALSE, ch, 0, victim, TO_NOTVICT);
				break;
			default:
	...
	{
		act("$N doesn't look frightened of $n's nightmares.. $E DOES look pissed off though.", FALSE, ch, 0, victim, TO_CHAR);
...
{
	struct damage_messages messages = {"Your massive &+Bhammer&N causes $N to double over in pain!",
...
void spell_dream_travel(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
		...
		for (group = ch->group; group; group = group->next)
		{
			...
			if (IS_ALIVE(targ) && CAN_SEE(ch, targ))
			{
				...
				if (IS_NPC(targ) || is_linked_to(ch, targ, LNK_CONSENT))
				{
					...
					act("&+LYou &+Wblink&+L and $n &+Lis gone! &+CIt was just a &+Wdream&+C; $e &+Cwas never here to begin with.&N", FALSE, targ, 0, 0, TO_ROOM);
	...
	else
	{
		...
		act("&+LYou &+Wblink&+L and $n &+Lis gone! &+CIt was just a &+Wdream&+C, $e &+Cwas never here to begin with.&N", FALSE, ch, 0, 0, TO_ROOM);
		act("&+LYou drift into the world of dreams...&N", FALSE, ch, 0, 0, TO_CHAR);
...
void spell_imprison(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	char  buf[1024];
	P_obj shell;
	int   shell_hps;
	...
	if (number(0, 1))
	{
		...
		{
			act("$N is almost encased in a $q, but $E believes it is just an illusion.", TRUE, victim, shell, victim, TO_ROOM);
			act("$n tries to encase you in a $q, but you disbelieve it!", FALSE, ch, shell, victim, TO_VICT);
			return;
		}
	}
	...
	if (NewSaves(victim, SAVING_FEAR, 3))
	{
		...
		act("You blink in &+cdi&+Csb&+cel&+Cie&+cf &nas a $q &ncreated by $n wraps around you!", FALSE, ch, shell, victim, TO_VICT);
	}
	else
	{
		act("$N freezes in &+ct&+Ce&+cr&+Cr&+co&+Cr &nas a $q &nencases $M!", TRUE, victim, shell, victim, TO_ROOM);
		act("You freeze in &+ct&+Ce&+cr&+Cr&+co&+Cr &nas a $q &ncreated by $n wraps around you!", FALSE, ch, shell, victim, TO_VICT);
	}
	...
	snprintf(buf, 1024, shell->description, victim->player.short_descr ? victim->player.short_descr : "someone");
...
bool handle_imprison_damage(P_char ch, P_char victim, int dam)
{
	...
	for (t_obj = world[victim->in_room].contents; t_obj; t_obj = t_obj->next_content)
	{
	...
	{
		act("A $q encasing $N suddenly blurs and fades into nothing under $n's assault!", TRUE, ch, t_obj, victim, TO_NOTVICT);
		act("A $q encasing $N suddenly blurs and fades into nothing under your assault!", TRUE, ch, t_obj, victim, TO_CHAR);
		act("A $q encasing you suddenly blurs and fades into nothing under $n's assault!", FALSE, ch, t_obj, victim, TO_VICT);
		REMOVE_BIT(victim->specials.affected_by5, AFF5_IMPRISON);
	...
	t_obj->value[1] -= dam;
	act("A $q encasing $N flares brightly as it absorbs $n's assault!", TRUE, ch, t_obj, victim, TO_NOTVICT);
	act("A $q encasing $N flares brightly as it absorbs your assault!", TRUE, ch, t_obj, victim, TO_CHAR);
	act("A $q encasing you flares brightly as it absorbs $n's assault!", FALSE, ch, t_obj, victim, TO_VICT);
...
void spell_nonexistence(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	{
		LOOP_THRU_PEOPLE(victim, ch) { SET_BIT(victim->specials.affected_by, AFF_HIDE); }
	}
...
void spell_titan(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	snprintf(Gbuf1, 512, "titan huge illusion %s %s", GET_NAME(ch), race_names_table[GET_RACE(ch)].normal);
...
void spell_delirium(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("$n sends a delirius ray streaking towards $N!", TRUE, ch, 0, victim, TO_NOTVICT);
	act("$n sends a delirius ray streaking YOU!", TRUE, ch, 0, victim, TO_VICT);
	act("You grin evilly as you send delirius ray streaking towards $N!", TRUE, ch, 0, victim, TO_CHAR);
...
void spell_flicker(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (ch && ch->group)
	{
		...
		act("&+L$n creates a sh&+cim&+Cme&+cri&+Lng orb of light, casting s&+mha&+Mdo&+mw&+Ls over the room!&n", TRUE, ch, 0, victim, TO_NOTVICT);
		act("&+LYou create a sh&+cim&+Cme&+cri&+Lng orb of light, casting s&+mha&+Mdo&+mw&+Ls over the room!&n", TRUE, ch, 0, victim, TO_CHAR);
...
void spell_greater_flicker(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj) { send_to_char("This spell is not yet implemented.\r\n", ch); }
...
void spell_sound_suppression(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (!affected_by_spell(victim, SPELL_SUPPRESSION))
	{
		...
		if (number(1, 100) > (counter * 2))
		{
			...
			act("$n becomes deathly silent.&n", FALSE, victim, 0, 0, TO_ROOM);
			act("You are surrounded in silence.&n", FALSE, victim, 0, 0, TO_CHAR);
		}
		affect_to_char(victim, &af);
	}
}

void spell_shadow_merge(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	P_char               t_ch;
	struct affected_type af;
	...
	if (!affected_by_spell(ch, SPELL_SHADOW_MERGE))
	{
		...
		if (ch->following)
		{
			...
			for (t_ch = world[ch->in_room].people; t_ch; t_ch = t_ch->next_in_room)
			{
				...
				if (t_ch == ch->following)
				{
					...
					act("$n fades in to the &+Lshadow&n of $N.&n", FALSE, ch, 0, ch->following, TO_ROOM);
					act("You step back into the &+Lshadow&n of $N.&n", FALSE, ch, 0, ch->following, TO_CHAR);
				}
			}
		}
		else
		{
			...
			act("$n fades in to the &+Lshadows&n.&n", FALSE, ch, 0, 0, TO_ROOM);
			act("You step back into the &+Lshadows&n.&n", FALSE, ch, 0, 0, TO_CHAR);
		}
		SET_BIT(ch->specials.affected_by, AFF_HIDE);
	}
}

void cast_ardgral(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	struct portal_create_messages msg = {/*ch   */ "&+cSomething prevents your escapist dreams here&n.\r\n",
	                                     /*ch r */ 0,
	                                     /*vic  */ 0,
	                                     /*vic r*/ 0,
	...
	if (IS_ROOM(ch->in_room, ROOM_NO_GATE) || IS_HOMETOWN(ch->in_room))
	{
...
void event_shadow_spawn(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if ((sbbdata->room != ch->in_room) || (sbbdata->waves < 1))
	{
		send_to_char("&+LThe swirling v&+ror&+Rt&+re&+Lx &+Lcollapses!\n", ch);
		;
		act("&+LThe swirling v&+ror&+Rt&+re&+Lx &+Lcollapses!", TRUE, ch, 0, 0, TO_ROOM);
		return;
	}

	act("&+LThe v&+ror&+Rt&+re&+Lx splits open and horrific n&+mi&+Lghtm&+ma&+Lr&+me&+Ls explode into the room!", FALSE, ch, 0, 0, TO_CHAR);
	act("&+LThe v&+ror&+Rt&+re&+Lx splits open and horrific n&+mi&+Lghtm&+ma&+Lr&+me&+Ls explode into the room!", FALSE, ch, 0, 0, TO_ROOM);
	cast_as_damage_area(ch, spell_shadow_spawn, GET_LEVEL(ch), NULL, get_property("spell.area.minChance.spawn", 90), get_property("spell.area.chanceStep.spawn", 10));

	sbbdata->waves--;

	if (ch && IS_NPC(ch) && !GET_MASTER(ch))
	{
		splat = 1;
	}
	else
	...
	if ((sbbdata->waves > 0) && !number(0, splat))
	{
...
void spell_shadow_spawn(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	struct damage_messages messages = {"&+LYou conjure up the worst &+Rfears&+L of your foes and hurl it at them!",
		...
		if (!NewSaves(victim, SAVING_FEAR, savemod))
		{
			...
			*/
			if (!number(0, affmod))
			{
				...
				act("&+LThe n&+mi&+Lghtm&+ma&+Lr&+me&+L is too much and you close your eyes!&n", FALSE, ch, 0, victim, TO_VICT);
				act("&+LThe n&+mi&+Lghtm&+ma&+Lr&+me&+L is too much for $N, whose eyes close!", FALSE, ch, 0, victim, TO_NOTVICT);
				if (ch && victim && !resists_spell(ch, victim))
				{
			...
			if (!number(0, affmod))
			{
				...
				act("&+LA wave of utter terror overcomes you as the n&+mi&+Lghtm&+ma&+Lr&+me&+L consumes you!&n", FALSE, ch, 0, victim, TO_VICT);
...
void event_asphyxiate(P_char ch, P_char victim, P_obj obj, void *data)
{
	int                    rounds = *((int *)data);
	int                    dam, level;
	struct damage_messages messages = {"&+cYou smirk as a pair of &+Wgh&+Los&+wtl&+Wy h&+Lan&+wds &+Rtighten &+caround $N&+c's throat!&n",
	                                   ...
	                                   "&+c$N &+Cgasps &+cfor air, &+Weyes &+Mbulging&+c, then falls over, slowly becoming &+Lmotionless&+c.&n",
	                                   0};
	...
	if (spell_damage(ch, victim, dam, SPLDAM_GENERIC, SPLDAM_NODEFLECT, &messages) == DAM_NONEDEAD && (--rounds > 0))
	{
...
void spell_asphyxiate(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	act("&+LA pair of &+wGh&+Los&+Wt&+wly &+WH&+La&+wnd&+Ws &+Lappear before &+c$N&+L, close around $S neck and begin to &+Rsqueeze&+L!&n", FALSE, ch, NULL, victim, TO_CHAR);
	act("&+wGh&+Los&+Wt&+wly &+Wh&+La&+wnd&+Ws &+Lappear before &+c$N&+L, close around $S neck and begin to &+Rsqueeze&+L!&n", FALSE, ch, NULL, victim, TO_NOTVICT);
	act("&+wGh&+Los&+Wt&+wly &+Wh&+La&+wnd&+Ws &+Lappear before you, close around your neck and begin to &+Rsqueeze&+L!&n", FALSE, victim, NULL, NULL, TO_CHAR);
