...
void cast_control_weather(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	const char         *variables[] = {"cold", "warm", "wet", "dry", "windy", "calm", "\n"};
	...
	switch (type)
	{
		case SPELL_TYPE_SPELL:
			...
			if ((zone->climate.flags & NON_CONTROLLABLE) || (zone->conditions.flags & WEATHER_CONTROLLED))
			{
			...
			switch (var - 1)
			{
				case 0: /* cold */
					...
					send_to_weather_sector(ch->in_room, "The temperature suddenly dips.\r\n");
					break;
				case 1: /* warm */
					zone->conditions.temp = MIN(zone->conditions.temp + level / 2, 45 + level);
					send_to_weather_sector(ch->in_room, "The temperature suddenly rises.\r\n");
					break;
				case 2: /* wet */
					if (zone->climate.season_precip[(int)get_season(in_weather_sector(ch->in_room))] == SEASON_NO_PRECIP_EVER)
					{
					...
					if (!zone->conditions.precip_rate)
					{
						...
						if (zone->conditions.temp > 0)
							send_to_weather_sector(ch->in_room, "A few drops of rain begin to fall.\r\n");
						else
							send_to_weather_sector(ch->in_room, "A few flakes of snow begin to fall.\r\n");
					}
					else
					{
						...
						if (zone->conditions.temp > 0)
							...
							send_to_weather_sector(ch->in_room, "The snow comes down a bit harder.\r\n");
					}
				...
				case 3: /* dry */
					...
					else
					{
						if (zone->conditions.precip_rate)
						{
							zone->conditions.precip_rate = MAX(0, zone->conditions.precip_rate - level);
							if (zone->conditions.precip_rate)
							{
								if (zone->conditions.temp > 0)
									send_to_weather_sector(ch->in_room, "The rain lets up suddenly.\r\n");
								else
									send_to_weather_sector(ch->in_room, "The snow suddenly lets up a bit.\r\n");
							}
						}
						else
						{
							zone->conditions.humidity = MAX(0, zone->conditions.humidity - level);
							send_to_weather_sector(ch->in_room, "The air feels a bit drier.\r\n");
						}
					}
					break;
				case 4: /* windy */
				...
				case 5: /* calm */
					...
					if (zone->conditions.windspeed <= 0)
					{
						...
						send_to_weather_sector(ch->in_room, "The air seems to come to a stagnant, dead halt.\r\n");
					}
					else
	...
	switch (type)
	{
		case SPELL_TYPE_SPELL:
			...
			else if (t_ch == ch)
			{
				...
				act("$n's eyes roll back in $s head as $e begins the incantation... specs of light begin to form in the room.", FALSE, ch, 0, 0, TO_ROOM);
				act("Your eyes roll back in your head as you begin the incantation... specs of light begin to form in the room.", FALSE, ch, 0, 0, TO_CHAR);
...
int planes_room_num[] = {23801, 23201, 12401, 24401, 19701, 25401, SURFACE_MAP_START, 32385, 26600, 0};

const char *planes_name[] = {"earth", "water", "ethereal", "air", "astral", "fire", "prime", "hell", "negative", "\n"};
...
void cast_gate(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	struct portal_create_messages msg = {/*ch   */ "The portal opens for a brief second and then closes.\r\n",
	                                     /*ch r */ 0,
	                                     /*vic  */ 0,
	                                     /*vic r*/ 0,
	...
	if ((ch && !is_Raidable(ch, 0, 0)) || (tar_ch && !is_Raidable(tar_ch, 0, 0)))
	{
	...
	switch (type)
	{
		case SPELL_TYPE_SPELL:
			...
			{
				if ((from_zone == world[MAX(0, real_room(SURFACE_MAP_START))].zone) || (from_zone == world[MAX(0, real_room(UD_MAP_START))].zone))
				{
					/* trying to use gate as a teleport, nah nah  */
					send_to_char(msg.bad_target_caster, ch);
					return;
				...
				do
				{
					to_room = number(real_room(planes_room_num[6]), top_of_world);
				} while ((world[MAX(0, real_room(planes_room_num[1]))].zone == world[to_room].zone) || (world[to_room].sector_type == SECT_OCEAN) || (world[to_room].sector_type == SECT_FIREPLANE) ||
				         (world[to_room].zone == 83) || (world[to_room].zone == 260) || (world[to_room].sector_type == SECT_AIR_PLANE) ||
				         ...
				         (zone_table[world[to_room].zone].flags & ZONE_CLOSED) || (!IS_MAP_ROOM(to_room)) // not a map room
				);
			}

			if ((to_room == NOWHERE) || (to_room == from_room) || IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(from_room, ROOM_NO_GATE) || IS_ROOM(to_room, ROOM_NO_GATE) || IS_HOMETOWN(to_room) ||
			...
			if (is_prime_plane(from_room) && is_prime_plane(to_room) && !IS_MAP_ROOM(from_room))
			{
				send_to_char(msg.fail_to_caster, ch);
				return;
			}
...
void cast_nether_gate(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	switch (type)
	{
		case SPELL_TYPE_SPELL:
			...
			if ((to_room == NOWHERE) || (to_room == from_room) || IS_ROOM(from_room, ROOM_NO_GATE) || IS_ROOM(to_room, ROOM_NO_GATE) || IS_HOMETOWN(to_room) || IS_HOMETOWN(from_room))
			{
				send_to_char(msg.fail_to_caster, ch);
				return;
			}
...
void cast_plane_shift(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	switch (type)
	{
		case SPELL_TYPE_SPELL:
			...
			if ((to_room == NOWHERE) || (to_room == ch->in_room) || IS_ROOM(to_room, ROOM_NO_MAGIC) || IS_ROOM(ch->in_room, ROOM_NO_GATE) || IS_ROOM(to_room, ROOM_NO_GATE) || IS_HOMETOWN(to_room) ||
...
void spell_wall_of_flames(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_FLAMES, dice(level / 4, 10), 1800, "&+Ra billowing wall of flames&n", "&+RA towering wall of flames is here to the %s.&n", 0))
	{
		snprintf(buf1, MAX_STRING_LENGTH, "&+RYou feel a blast of heat as a huge wall of flames bursts to the %s!&n\r\n", dirs[var]);
		snprintf(buf2, MAX_STRING_LENGTH, "&+RYou feel a blast of heat as a huge wall of flames bursts to the %s!&n\r\n", dirs[rev_dir[var]]);
...
void spell_wall_of_ice(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_ICE, 40, 1800, "&+Wa wall of ice&N", "&+WA huge block of solid ice is here to the %s.&n", 0))
	{
		...
		snprintf(buf1, MAX_STRING_LENGTH, "&+WYou feel a gust of cold as a huge block of ice forms to the %s!&n\r\n", dirs[var]);
		snprintf(buf2, MAX_STRING_LENGTH, "&+WYou feel a gust of cold as a huge block of ice forms to the %s!&n\r\n", dirs[rev_dir[var]]);
...
void cast_life_ward(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, LIFE_WARD, 40, 1800, "&+La wall of &n&+bnegative energy&n", "&+LA strange blackness cloaks the %s exit here.&n", 0))
	{
...
void spell_wall_of_stone(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_STONE, level, 1800, "&+La greyish stone wall&N", "&+LA greyish stone wall is here to the %s.&n", 0))
	{
...
void spell_wall_of_iron(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_IRON, level, 1800, "&+ya massive wall of iron&N", "&+yA massive wall of iron is here to the %s.&n", 0))
	{
		SET_BIT(EXIT(ch, var)->exit_info, EX_BREAKABLE);
		SET_BIT(VIRTUAL_EXIT((world[ch->in_room].dir_option[var])->to_room, rev_dir[var])->exit_info, EX_BREAKABLE);
...
void spell_wall_of_force(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WALL_OF_FORCE, level, 1800, "&+Wa wall of force&n", "&+WThe outline of some wall is here to the %s.&n", ITEM_INVISIBLE))
	{
		SET_BIT(EXIT(ch, var)->exit_info, EX_BREAKABLE);
		SET_BIT(VIRTUAL_EXIT((world[ch->in_room].dir_option[var])->to_room, rev_dir[var])->exit_info, EX_BREAKABLE);
...
void cast_lightning_curtain(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, LIGHTNING_CURTAIN, dice(level / 4, 10), 1800, "&+Ya crackling curtain of lightning&n", "&+BA rippling curtain of lightning crackles to the %s.&n", 0))
	{
...
void cast_web(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (create_walls(ch->in_room, var, ch, level, WEB, 0, 1800, "&+Wa sticky web&n", "A large web appears %s!", 0))
	{
...
void cast_prismatic_cube(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	for (dir = 0; dir < NUM_EXITS; dir++)
	{
		...
		if (create_walls(room,
		                 ...
		                 0))
		{
			snprintf(buf1, MAX_STRING_LENGTH, "A &+rw&N&+ca&N&+bl&N&+yl&N of &+gs&N&+Rh&N&+Ci&N&+Bf&N&+Yt&N&+Gi&N&+rn&N&+Cg&N &+bc&N&+yo&N&+gl&N&+ro&N&+cr&N appears to the %s!\r\n", dirs[dir]);
			snprintf(
				buf2, MAX_STRING_LENGTH, "A &+rw&N&+ca&N&+bl&N&+yl&N of &+gs&N&+Rh&N&+Ci&N&+Bf&N&+Yt&N&+Gi&N&+rn&N&+Cg&N &+bc&N&+yo&N&+gl&N&+ro&N&+cr&N appears to the %s!\r\n", dirs[rev_dir[dir]]);
...
void event_earthen_tomb(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	{
		send_to_room("&+yThe ground &+Lr&+yu&+Lm&+ybl&+ye&+Ls &+yand quivers under your feet.&n\r\n", room);
		add_event(event_earthen_tomb, (int)(PULSE_VIOLENCE * 1.5), 0, 0, 0, 0, &room, sizeof(room));
		return;
	}
	...
	if (create_walls(room, exit, NULL, 50, WALL_OF_STONE, 50, 1800, "&+yAn earthen wall&n", "&+yAn earthen wall blocks the exit to the %s.&n", 0))
	{
		snprintf(buf1, MAX_STRING_LENGTH, "&+ySuddenly a tall earthen wall rises from the ground blocking the exit to the %s!&n\r\n", dirs[exit]);
		snprintf(buf2, MAX_STRING_LENGTH, "&+ySuddenly a tall earthen wall rises from the ground blocking the exit to the %s!&n\r\n", dirs[rev_dir[exit]]);
...
void cast_earthen_tomb(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	add_event(event_earthen_tomb, PULSE_VIOLENCE, 0, 0, 0, 0, &ch->in_room, sizeof(ch->in_room));
}
...
//-------------------------------------------------------------------------------
bool prepare_room_transmute(P_char ch, int room, int skill, int duration, int delay, event_func_type trans_func, event_func_type func_bye)
{
...
void finish_room_transmute(P_char ch, struct grow_data *data)
{
	...
	add_event(event_transmute_bye, data->duration, 0, NULL, NULL, 0, &g_data, sizeof(g_data));
}
//-------------------------------------------------------------------------------
...
void spell_ethereal_grounds(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_ETHEREAL_GROUNDS, 4 * 60, 4 * seconds, event_ethereal_grounds, event_ethereal_grounds_bye))
		return;
...
void cast_transmute_mud_rock(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_MUD_ROCK, 4 * 60, 4 * seconds, event_trans_mud_rock, event_mud_rock_bye))
		return;
...
void cast_transmute_rock_mud(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_ROCK_MUD, 4 * 60, 4 * seconds, event_trans_rock_mud, event_rock_mud_bye))
		return;
...
void cast_transmute_mud_water(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_MUD_WATER, 4 * 60, 4 * seconds, event_trans_mud_water, event_mud_water_bye))
		return;
...
void cast_transmute_water_mud(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_WATER_MUD, 4 * 60, 4 * seconds, event_trans_water_mud, event_water_mud_bye))
		return;
...
void cast_transmute_water_air(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_WATER_AIR, 4 * 60, 4 * seconds, event_trans_water_air, event_water_air_bye))
		return;
...
void cast_transmute_air_water(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_AIR_WATER, 4 * 60, 4 * seconds, event_trans_air_water, event_air_water_bye))
		return;
...
void cast_transmute_rock_lava(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_ROCK_LAVA, 4 * 60, 4 * seconds, event_trans_rock_lava, event_rock_lava_bye))
		return;
...
void cast_transmute_lava_rock(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_TRANS_LAVA_ROCK, 4 * 60, 4 * seconds, event_trans_lava_rock, event_lava_rock_bye))
		return;
...
void cast_depressed_earth(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_DEPRESSED_EARTH, 4 * 60, 4 * seconds, event_depressed_earth, event_depressed_earth_bye))
		return;
...
void cast_grow(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!prepare_room_transmute(ch, ch->in_room, SPELL_GROW, 4 * 60, 4 * seconds, event_grow, event_grow_bye))
		return;
...
void event_spike_growth(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	struct damage_messages message = {"From out of nowhere &+yspikes in the ground&n shoot out piercing $N!",
	...
	for (tch = world[sgd->room].people; tch; tch = tch->next_in_room)
	{
		...
		switch (technique)
		{
			case 1:
				...
				break;
			case 2:
				act("A &+yHUGE SPIKE&n bursts from the &+yground&n and pierces you in the foot!.", FALSE, tch, 0, 0, TO_CHAR);
				act("A &+yHUGE SPIKE&n bursts from the &+yground&n piercing&n $n's &+yfoot!&n", FALSE, tch, 0, 0, TO_NOTVICT);
...
spike_grow_next:
	add_event(event_spike_growth, WAIT_SEC * 2, ch, NULL, NULL, 0, sgd, sizeof(struct spike_growth_data));
}
...
void event_awaken_forest(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	struct damage_messages messages = {"From out of nowhere &+ybranches&n wrap themselves around $N crushing $M in their grip.",
...
awaken_next:
	add_event(event_awaken_forest, WAIT_SEC * 3 / 2, ch, NULL, NULL, 0, awd, sizeof(struct awaken_forest_data));
}
...
void cast_awaken_forest(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	if (!get_spell_from_room(&world[ch->in_room], SPELL_GROW) && world[ch->in_room].sector_type != SECT_FOREST)
	{
	...
	add_event(event_awaken_forest, WAIT_SEC, ch, NULL, NULL, 0, &awd, sizeof(awd));
}
...
void cast_hurricane(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	struct damage_messages messages = {"&+WHurricane&n winds sweep through the room tearing at $N!.",
	...
	for (tch = world[ch->in_room].people; tch; tch = next)
	{
		...
		if (OUTSIDE(ch))
		{
			if (affchance < chance)
			{
				act("The gail force of your spell sends $N crashing to the ground!", FALSE, ch, 0, tch, TO_CHAR);
				act("The gail force of $n's spell sends you crashing to the ground!", FALSE, ch, 0, tch, TO_VICT);
				act("The gail force of $n's spell sends $N crashing to the ground!", FALSE, ch, 0, tch, TO_NOTVICT);
				SET_POS(tch, POS_SITTING + GET_STAT(tch));
			}
		}
		spell_damage(ch, tch, dam, SPLDAM_GENERIC, 0, &messages);
...
void cast_storm_shield(int level, P_char ch, char *arg, int type, P_char tar_ch, P_obj tar_obj)
{
	...
	act("&+bYour $q &+bstarts to crackle with energy, as small &+Wbolts &+bof &+Wlightning &+brun along its surface.&n", FALSE, ch, ch->equipment[WEAR_SHIELD], 0, TO_CHAR);

	act("&+b$n&+b's $q &+bstarts to crackle with energy, as small &+Wbolts &+bof &+Wlightning &+brun along its surface.&n", FALSE, ch, ch->equipment[WEAR_SHIELD], 0, TO_ROOM);
}
...
void cast_bloodstone(int level, P_char ch, char *arg, int type, P_char victim, P_obj tar_obj)
{
	...
	act("You feel as if though your blood starts to flow slower in your veins.", FALSE, victim, 0, 0, TO_CHAR);
...
bool exit_wallable(int room, int dir, P_char ch)
{
	...
	if (IS_SET(exit->exit_info, EX_BLOCKED) || IS_SET(exit->exit_info, EX_SECRET))
	{
		if (ch != NULL)
			send_to_char("You see no exit in that direction!\r\n", ch);
		return FALSE;
	...
	if (!VIRTUAL_EXIT(dir_room, reverse_exit) || world[dir_room].dir_option[reverse_exit]->to_room != room)
	{
		if (ch != NULL)
	...
	for (gl = ch->group; gl; gl = gl->next)
	{
		if (ch->in_room == gl->ch->in_room)
		{
			...
			if (IS_RACEWAR_GOOD(gm))
			{
				...
				} while (race != RACE_HUMAN && race != RACE_GREY && race != RACE_MOUNTAIN && race != RACE_BARBARIAN && race != RACE_GNOME && race != RACE_HALFLING && race != RACE_HALFELF &&
				         race != RACE_CENTAUR && race != RACE_GITHZERAI && race != RACE_AGATHINON && race != RACE_THRIKREEN && race != RACE_MINOTAUR && race != RACE_FIRBOLG && race != RACE_WOODELF);
			}
			else if (IS_RACEWAR_EVIL(gm))
			{
				...
				} while (race != RACE_DROW && race != RACE_DUERGAR && race != RACE_GITHYANKI && race != RACE_OGRE && race != RACE_GOBLIN && race != RACE_ORC && race != RACE_OROG &&
				         race != RACE_TROLL && race != RACE_THRIKREEN && race != RACE_MINOTAUR && race != RACE_DRIDER && race != RACE_PILLITHID && race != RACE_KUOTOA);
			}
			else if (IS_RACEWAR_UNDEAD(gm))
			{
				...
				{
					race = number(0, LAST_RACE);
				} while (race != RACE_LICH && race != RACE_PVAMPIRE && race != RACE_PDKNIGHT && race != RACE_SHADE && race != RACE_REVENANT && race != RACE_PSBEAST && race != RACE_WIGHT &&
				         race != RACE_GARGOYLE && race != RACE_PHANTOM);
			}
			else
			{
				race = number(0, LAST_RACE);
			...
			snprintf(buff,
			         MAX_STRING_LENGTH,
			         "&+LYour &+wi&+Wmag&+we &+ms&+Mh&+Lif&+Mt&+ms &+Land &+bb&+Blur&+bs &+Linto %s %s.&n",
			         VOWEL(race_names_table[race].normal[0]) ? "an" : "a",
			         race_names_table[race].ansi);
			act(buff, TRUE, gm, 0, ch, TO_CHAR);
			snprintf(buff,
			         MAX_STRING_LENGTH,
			         "&+LThe &+wi&+Wmag&+we &+Lof $n &+ms&+Mh&+Lif&+Mt&+ms &+Land &+bb&+Blur&+bs &+Linto %s %s.&n",
			         VOWEL(race_names_table[race].normal[0]) ? "an" : "a",
			         race_names_table[race].ansi);
			act(buff, FALSE, gm, 0, ch, TO_ROOM);
			// snprintf(buff, MAX_STRING_LENGTH, "$N's arcane magic forms an illusion about $n turning $m into %s %s.&n", VOWEL(race_names_table[race].normal[0]) ? "an" : "a",
			// race_names_table[race].ansi); act(buff, TRUE, gm, 0, ch, TO_NOTVICTROOM);
		}
	}
}
...
void event_change_yzar_race(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	int                   yzar_races[NUM_YZARS] = {RACE_SHADE, RACE_REVENANT, RACE_LICH, RACE_PVAMPIRE, RACE_PDKNIGHT, RACE_PSBEAST, RACE_WIGHT, RACE_PHANTOM, RACE_PARASITE, RACE_SLIME};
	if (!IS_ALIVE(ch))
	{
		return;
	}
