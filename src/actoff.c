...
struct failed_takedown_messages failed_bash_messages[] = {
	{		  TAKEDOWN_GHOST,
     "Your attempt to bash $N fails as you simply pass through $M.",                        "$n makes a valiant attempt to bash $N, but simply falls through $M.",
     "$n's attempt to bash you fails as $e simply passes through you."																																										},
	{  TAKEDOWN_AIR_ELEMENTAL,
     "&+WYour attempt to bash $N fails as $E blows you on your ass!",                        "&+W$n makes a valiant attempt to bash $N, but gets blown on $s ass!",
     "$n's attempt to bash you fails as you simply blow $m onto $s ass!"																																									  },
	{TAKEDOWN_EARTH_ELEMENTAL,
     "&+WYour attempt to bash $N feels like you just hit a brick wall!",                                    "&+W$n just fell flat on $s ass after trying to bash $N!",
     "$n's attempt to bash you fails miserably."																																															  },
	{ TAKEDOWN_FIRE_ELEMENTAL,
     "&+WYour attempt to bash $N almost burns you to a crisp!",                        "&+W$n tries to bash $N, but falls right through $M and onto $s ass!",
     "$n's attempt to bash you almost burns $m to a crisp!"																																												   },
	{TAKEDOWN_WATER_ELEMENTAL,                 "&+WYou almost drown in your attempt to bash $N!",                                            "$n almost drowns as $e passes right through $N!", "$n almost drowns as $e attempts to bash you!"},
	{		  TAKEDOWN_HARPY,
     "Your attempt to bash $N fails as $M flutters into the air.",                       "$n makes a valiant attempt to bash $N, but $E flutters into the air.",
     "$n's attempt to bash you fails as you flutter into the air."																																											},
	{		TAKEDOWN_GRAPPLE,
     "As you try to bash $N, $E sidesteps, trips you, and slams you face first into the ground.",           "As $n tries to bash $N, $N slips away from $m and slams $s face into the ground.",
     "As $n tries to bash you, you slip away from $m and slam $s face into the ground."																																					   },
	{		  TAKEDOWN_EVADE,
     "Your powerful bash knocks $N to the ground yet $E quickly rolls back up onto $S feet.", "$N spins out of the way and rolls back onto $S feet as $n tries to knock $M to the ground.",
     "You use the momentum of $n's powerful bash to leap back up onto your feet!"																																							 },
	{		TAKEDOWN_DEFAULT,
     "You make a futile attempt to bash $N, but $E is simply immovable.",                          "$n makes a futile attempt to bash $N, but $E is simply immovable.",
     "$n makes a futile attempt to bash you, but you are simply immovable."																																								   },
};

struct failed_takedown_messages failed_trip_messages[] = {
	{  TAKEDOWN_GHOST,
     "Your attempt to trip $N fails as you simply pass through $M.",              "$n makes a valiant attempt to trip $N, but simply falls through $M.",
     "$n's attempt to trip you fails as $e simply passes through you."                 },
	{  TAKEDOWN_HARPY,
     "Your attempt to trip $N fails as $M flutters into the air.",             "$n makes a valiant attempt to trip $N, but $E flutters into the air.",
     "$n's attempt to trip you fails as you flutter into the air."                     },
	{TAKEDOWN_GRAPPLE,
     "As you try to trip $N, $E sidesteps, trips you, and slams you face first into the ground.", "As $n tries to trip $N, $N slips away from $m and slams $s face into the ground.",
     "As $n tries to trip you, you slip away from $m and slam $s face into the ground."},
	{  TAKEDOWN_EVADE,
     "In an amazing display of dexterity $N does a backward flip avoiding being tripped!",                    "As $n tries to trip $N $E flips over $S shoulder and escapes!",
     "You spin out of reach as $n attempts to sweep your feet."                        },
	{TAKEDOWN_DEFAULT,
     "You make a futile attempt to trip $N, but $E is simply immovable.",                "$n makes a futile attempt to trip $N, but $E is simply immovable.",
...
struct failed_takedown_messages failed_round_kick_messages[] = {
	{		  TAKEDOWN_GHOST,
     "Your attempt to kick $N fails as you simply pass through $M.",  "$n makes a valiant attempt to kick $N, but simply falls through $M.",
     "$n's attempt to kick you fails as $e simply passes through you."																																  },
	{  TAKEDOWN_AIR_ELEMENTAL,
     "&+WYour attempt to kick $N fails as $E blows you on your ass!",  "&+W$n makes a valiant attempt to kick $N, but gets blown on $s ass!",
     "$n's attempt to kick you fails as you simply blow $m onto $s ass!"																																},
	{TAKEDOWN_EARTH_ELEMENTAL,
     "&+WYour attempt to kick $N feels like you just hit a brick wall!",              "&+W$n just fell flat on $s ass after trying to kick $N!",
     "$n's attempt to kick you fails miserably."																																						},
	{ TAKEDOWN_FIRE_ELEMENTAL,
     "&+WYour attempt to kick $N almost burns you to a crisp!",  "&+W$n tries to kick $N, but falls right through $M and onto $s ass!",
     "$n's attempt to kick you almost burns $m to a crisp!"																																			 },
	{TAKEDOWN_WATER_ELEMENTAL, "&+WYou almost drown in your attempt to kick $M!",                      "$n almost drowns as $e passes right through $M!", "$n almost drowns as $e attempts to kick you!"},
	{		  TAKEDOWN_HARPY,
     "Your attempt to kick $N fails as $M flutters into the air.", "$n makes a valiant attempt to kick $N, but $E flutters into the air.",
     "$n's attempt to kick you fails as you flutter into the air."																																	  },
	{		TAKEDOWN_DEFAULT, "&+WYou attempt to kick $M but bounce right off!",                           "$n tries to kick $N but bounces right off!",  "$n tries to kick you but bounces right off!"}
};

struct failed_takedown_messages failed_bodyslam_messages[] = {
	{		  TAKEDOWN_GHOST,
     "Your attempt to bodyslam $N fails as you simply pass through $M.",                                          "$n tries to bodyslam $N, but simply falls through $M.",
     "$n tries to bodyslam you, but simply falls through."																																																  },
	{  TAKEDOWN_AIR_ELEMENTAL,
     "&+WYour attempt to bodyslam $N fails as $E blows you on your ass!",                        "&+W$n makes a valiant attempt to bodyslam $N, but gets blown on $s ass!",
     "$n's attempt to bodyslam you fails as you simply blow $m onto $s ass!"																																												},
	{TAKEDOWN_EARTH_ELEMENTAL,
     "&+WYour attempt to bodyslam $N feels like you just hit a brick wall!",                                    "&+W$n just fell flat on $s ass after trying to bodyslam $N!",
     "$n's attempt to bodyslam you fails miserably."																																																		},
	{ TAKEDOWN_FIRE_ELEMENTAL,
     "&+WYour attempt to bodyslam $N almost burns you to a crisp!",                        "&+W$n tries to bodyslam $N, but falls right through $M and onto $s ass!",
     "$n's attempt to bodyslam you almost burns $m to a crisp!"																																															 },
	{TAKEDOWN_WATER_ELEMENTAL,                 "&+WYou almost drown in your attempt to bodyslam $M!",                                                "$n almost drowns as $e passes right through $M!",   "$n almost drowns as $e attempts to bodyslam you!"},
	{		 TAKEDOWN_DRAGON,							"You simply bounce off $N's massive form.",                                              "$n tries to bodyslam $N and bounces off, stunned!", "$n tries to bodyslam you and bounces off, stunned!"},
	{		  TAKEDOWN_HARPY,
     "Your attempt to bodyslam $N fails as $M flutters into the air.",                                         "$n tries to bodyslam $N, but $E flutters into the air.",
     "$n's tries to bodyslam you but, you flutter into the air."																																															},
	{		TAKEDOWN_GRAPPLE,
     "As you try to bodyslam $N, $E sidesteps, trips you, and slams you face first into the ground.",           "As $n tries to bodyslam $N, $N slips away from $m and slams $s face into the ground.",
     "As $n tries to bodyslam you, you slip away from $m and slam $s face into the ground."																																								 },
	{		  TAKEDOWN_EVADE,
     "The force of your bodyslam sends $N on $S ass but $E quickly recovers and leaps back up.", "$n's bodyslam sends $N on $S ass but with great dexterity $E rolls backward and leaps back up.",
     "$n bodyslams you to the ground but you use the momentum to roll backward onto your feet."																																							 },
	{		TAKEDOWN_DEFAULT,
     "You make a futile attempt to bodyslam $N, but $E is simply immovable.",                          "$n makes a futile attempt to bodyslam $N, but $E is simply immovable.",
...
struct failed_takedown_messages failed_tackle_messages[] = {
	{		  TAKEDOWN_GHOST,
     "Your attempt to tackle $N fails as you simply pass through $M.",              "$n makes a valiant attempt to tackle $N, but simply falls through $M.",
     "$n's attempt to tackle you fails as $e simply passes through you."																																								  },
	{  TAKEDOWN_AIR_ELEMENTAL,
     "&+WYour attempt to tackle $N fails as $E blows you on your ass!",              "&+W$n makes a valiant attempt to tackle $N, but gets blown on $s ass!",
     "$n's attempt to tackle you fails as you simply blow $m onto $s ass!"																																								},
	{TAKEDOWN_EARTH_ELEMENTAL,
     "&+WYour attempt to tackle $N feels like you just hit a brick wall!",                          "&+W$n just fell flat on $s ass after trying to tackle $N!",
     "$n's attempt to tackle you fails miserably."																																														},
	{ TAKEDOWN_FIRE_ELEMENTAL,
     "&+WYour attempt to tackle $N almost burns you to a crisp!",              "&+W$n tries to tackle $N, but falls right through $M and onto $s ass!",
     "$n's attempt to tackle you almost burns $m to a crisp!"																																											 },
	{TAKEDOWN_WATER_ELEMENTAL,                 "&+WYou almost drown in your attempt to tackle $M!",                                    "$n almost drowns as $e passes right through $M!", "$n almost drowns as $e attempts to tackle you!"},
	{		  TAKEDOWN_HARPY,
     "Your attempt to tackle $N fails as $M flutters into the air.",             "$n makes a valiant attempt to tackle $N, but $E flutters into the air.",
     "$n's attempt to tackle you fails as you flutter into the air."																																									  },
	{		TAKEDOWN_GRAPPLE,
     "As you try to tackle $N, $E sidesteps, trips you, and slams you face first into the ground.", "As $n tries to tackle $N, $N slips away from $m and slams $s face into the ground.",
     "As $n tries to tackle you, you slip away from $m and slam $s face into the ground."																																				 },
	{		  TAKEDOWN_EVADE,
     "As you try to tackle $N $E casually sidesteps forcing you to abort or topple to the ground.", "As $n tries to tackle $N $E casually sidesteps causing $m to almost slip and fall.",
     "You casually sidesteps $n as $e tries to tackle you causing $m to stumble and almost fall."																																		 },
	{		TAKEDOWN_DEFAULT,
     "You make a futile attempt to tackle $N, but $E is simply immovable.",                "$n makes a futile attempt to tackle $N, but $E is simply immovable.",
...
struct failed_takedown_messages failed_springleap_messages[] = {
	{  TAKEDOWN_GHOST,
     "You fly head first through $N, landing on your ass.",                                     "$n attempts to springleap $N but simply flies through $M.",
     "$n attempts to springleap you.  Apparently $e didn't notice you're incorporeal."                  },
	{ TAKEDOWN_DRAGON,
     "You fly head first directly into what might as well be a brick wall.",                                      "$n flies into $N, and simply drops off the massive form.",
     "$n is really quite an idiot to think $e could do that fancy crap on YOU."                         },
	{  TAKEDOWN_HARPY,
     "Your attempt to springleap $N fails as $M flutters into the air.",                                   "$n attempts to springleap $N, but $E flutters into the air.",
     "$n's attempts to springleap you fails as you flutter into the air."                               },
	{TAKEDOWN_GRAPPLE,
     "As you try to springleap $N, $E sidesteps, trips you, and slams you face first into the ground.",        "As $n tries to springleap $N, $N slips away from $m and slams $s face into the ground.",
     "As $n tries to springleap you, you slip away from $m and slam $s face into the ground."           },
	{  TAKEDOWN_EVADE,
     "Your springleap knocks $N to the ground, yet with great dexterity $E rolls back onto $S feet.", "$n's springleap knocks $N to the ground, yet with great dexterity $E rolls back onto $S feet.",
     "$n flies into you knocking you off your feet, but you tuck and roll springing up on to your feet."},
	{TAKEDOWN_DEFAULT,
     "You make a futile attempt to springleap $N, but $E is simply immovable.",                       "$n makes a futile attempt to springleap $N, but $E is simply immovable.",
...
struct failed_takedown_messages failed_maul_messages[] = {
	{  TAKEDOWN_GHOST,"Your attempt to maul $N fails as you simply pass through $M.",                                "$n attempts to maul $N, but simply falls through $M.","$n's maul passes right through you."                                                },
	{  TAKEDOWN_HARPY,
     "Your attempt to maul $N fails as $M flutters into the air.",                "$n makes a valiant attempt to maul $N, but $E flutters into the air.",
     "$n's attempt to maul you fails as you flutter into the air."                    },
	{TAKEDOWN_GRAPPLE,
     "As you attempt to maul $N, $E sidesteps, trips you, and slams you face first into the ground.", "As $n attempts to maul $N, $N slips away from $m and slams $s face into the ground.",
     "As $n attempts to maul, you slip away from $m and slam $s face into the ground."},
	{  TAKEDOWN_EVADE,
     "In an amazing display of dexterity $N does a backward flip avoiding your maul!",                       "As $n tries to maul $N $E flips over $S shoulder and escapes!",
     "You spin out of reach as $n attempts to maul you into the ground!"              },
	{TAKEDOWN_DEFAULT,
     "You make a futile attempt to maul $N, but $E is simply immovable.",                   "$n makes a futile attempt to maul $N, but $E is simply immovable.",
...
void show_failed_takedown_messages(P_char ch, P_char victim, int skill, int reason)
{
	...
	switch (skill)
	{
		case SKILL_BASH:
			...
			break;
		case SKILL_BODYSLAM:
			messages_set = (struct failed_takedown_messages *)failed_bodyslam_messages;
			break;
		case SKILL_ROUNDKICK:
			messages_set = (struct failed_takedown_messages *)failed_round_kick_messages;
			break;
		case SKILL_TACKLE:
			messages_set = (struct failed_takedown_messages *)failed_tackle_messages;
			break;
		case SKILL_SPRINGLEAP:
			messages_set = (struct failed_takedown_messages *)failed_springleap_messages;
			break;
		case SKILL_TRIP:
	...
	i = 0;
	while (messages_set[i].reason != reason && messages_set[i].reason != TAKEDOWN_DEFAULT)
	{
		i++;
	}
...
float takedown_check(P_char ch, P_char victim, float chance, int skill, ulong applicable)
{
	...
	if (GET_CHAR_SKILL(victim, SKILL_EVADE) && !get_spell_from_char(victim, SKILL_EVADE) &&
	    (notch_skill(victim, SKILL_EVADE, get_property("skill.notch.offensive", 7)) || GET_CHAR_SKILL(victim, SKILL_EVADE) / 3 >= number(1, 100)))
	{
		show_failed_takedown_messages(ch, victim, skill, TAKEDOWN_EVADE);
	...
	{
		if (notch_skill(victim, SKILL_GRAPPLE, get_property("skill.notch.offensive", 7)) || GET_CHAR_SKILL(victim, SKILL_GRAPPLE) / 6 >= number(1, 100))
		{
	...
	if (affected_by_spell(victim, SPELL_GUARDIAN_SPIRITS))
	{
		...
		{
			debug("Takedown_check: attacker (%s) reduced by Guardian Spirit from (%.2f) to (%.2f).", GET_NAME(ch), chance, chance * get_property("spell.GuardianSpirit", 0.750));
		}
...
P_char ParseTarget(P_char ch, char *argument)
{
	...
	if (!victim && GET_OPPONENT(ch))
	{
		...
		if ((!IS_ALIVE(victim)) || (victim->in_room != ch->in_room) || victim->specials.z_cord != ch->specials.z_cord)
		{
			stop_fighting(ch);
			victim = NULL;
		}
...
void do_hit(P_char ch, char *argument, int cmd)
{
	...
	if (!*argument && has_innate(ch, INNATE_SENSE_WEAKNESS))
	{
		...
		for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
		{
			...
			{
				if (CAN_SEE(ch, tch) && on_front_line(tch) && (tch != ch) && (victim == NULL || GET_HIT(tch) < GET_HIT(victim)))
				{
					victim = tch;
				}
			}
		...
		if (victim != NULL)
		{
			act("You feel the &+rbl&+Loo&+rdl&+Lus&+rt&n pulse in your veins as you spot $N and charge!", FALSE, ch, 0, victim, TO_CHAR);
			act("$n grins wickedly as he spots you!", FALSE, ch, 0, victim, TO_VICT);
		}
	}
	...
	if ((!IS_PC(ch) && !IS_PC_PET(ch)) || (on_front_line(ch) && on_front_line(victim)))
	{
		attack(ch, victim);
	}
	else if (ch->equipment[PRIMARY_WEAPON] && IS_REACH_WEAPON(ch->equipment[PRIMARY_WEAPON]))
		attack(ch, victim);
	else
		send_to_char("You can't quite seem to reach them...\n", ch);
}

void do_murde(P_char ch, char *argument, int cmd) { send_to_char("To prevent 'accidents', you may not abbreviate 'murder'.\n", ch); }
...
void lance_charge(P_char ch, char *argument)
{
	...
	percent_chance = (int)(0.9 * MAX(70, GET_CHAR_SKILL(ch, SKILL_LANCE_CHARGE)));
	...
	percent_chance = (int)(percent_chance * ((double)BOUNDED(8, 10 + (GET_LEVEL(ch) - GET_LEVEL(victim)) / 10, 11)) / 10);
	...
	if (dir == -1)
	{
		act("$n spurs $s mount and charges $N, the $q pointed at $S chest.", TRUE, ch, weapon, victim, TO_NOTVICT);
		act("$n spurs $s mount and charges $N, the $q pointed at YOUR chest.", TRUE, ch, weapon, victim, TO_VICT);
		act("You spur your mount and charge $N, the $q pointed at $S chest.", TRUE, ch, weapon, victim, TO_CHAR);
	}
	else
	{
	...
	if (dir != -1 && CAN_GO(ch, dir) && !check_wall(ch->in_room, dir) && can_enter_room(ch, EXIT(ch, dir)->to_room, 0))
		continue_dir = dir;
	else
	{
		for (i = 0; i < NUM_EXITS; i++)
		{
			...
			{
				if (CAN_GO(ch, i) && !check_wall(ch->in_room, dir) && (dir == -1 || dir != rev_dir[dir]) && can_enter_room(ch, EXIT(ch, i)->to_room, 0))
				{
					available_exits++;
				}
			}
	...
	if (!notch_skill(ch, SKILL_LANCE_CHARGE, get_property("skill.notch.offensive", 7)) && percent_chance < number(1, 100))
	{
		// failure code.
		...
		{ // basically a critical miss
			act("You misjudge the distance to your target and topple off your mount!", FALSE, ch, 0, ch, TO_CHAR);
			act("$n misjudges the distance to $s target and topples off $s mount!", FALSE, ch, 0, ch, TO_ROOM);
		...
		else if (effect < 3)
		{
			...
			while (tries-- && (!new_victim || (ch->group && (ch->group == new_victim->group))))
			{
		...
		{
			act("You thunder past as you fail to impale $N with your $q.", TRUE, ch, weapon, victim, TO_CHAR);
			act("$n thunders past as $e fails to impale you with $s $q.", TRUE, ch, weapon, victim, TO_VICT);
			act("$n thunders past as $e fails to impale $N with $s $q.", TRUE, ch, weapon, victim, TO_NOTVICT);
			return;
		}
	}
	...
	if (ch && victim)
	{
		...
		CharWait(victim, (int)(PULSE_VIOLENCE * get_property("skill.lance.charge.VictimLag", 1.000)));
	}
	...
	switch (effect)
	{
		...
		case 1:
			dam = dam + dice(20, 5);
			act("With a &+Wbone c&+wr&+Wu&+ws&+Wh&+wi&+Wn&+wg thud the $q slams into $N impaling $M.", FALSE, ch, weapon, victim, TO_NOTVICT);
			act("With a &+Wbone c&+wr&+Wu&+ws&+Wh&+wi&+Wn&+wg thud the $q slams into $N impaling $M.", FALSE, ch, weapon, victim, TO_CHAR);
			act("With a &+Wbone c&+wr&+Wu&+ws&+Wh&+wi&+Wn&+wg thud $n's $q slams into you!", FALSE, ch, weapon, victim, TO_VICT);
			if (!number(0, 100))
				DamageOneItem(ch, SPLDAM_GENERIC, weapon, FALSE);
			break;
		case 2:
			dam = dam / 2 + dice(15, 5);
			act("You glance $N with the tip of your $q causing a small surface wound.", FALSE, ch, weapon, victim, TO_CHAR);
			act("$n glances $N with the tip of $s $q causing a small surface wound.", FALSE, ch, weapon, victim, TO_NOTVICT);
			act("$n glances you with the tip of $s $q causing a small surface wound.", FALSE, ch, weapon, victim, TO_VICT);
			knockdown_chance = 0;
			if (!number(0, 100))
				DamageOneItem(ch, SPLDAM_GENERIC, weapon, FALSE);
			break;
		...
		case 4:
			dam = dam + dice(30, 6);
			act("You ram your $q right through $N causing &+Rb&+rl&+Ro&+ro&+Rd&n to pour from $S body.", FALSE, ch, weapon, victim, TO_CHAR);
			act("$n rams $s $q right through $N causing &+Rb&+rl&+Ro&+ro&+Rd&n to pour from $S body.", FALSE, ch, weapon, victim, TO_NOTVICT);
			act("$n rams $s $q right through your body making you &+RB&+rL&+RE&+rE&+RD&n all over!", FALSE, ch, weapon, victim, TO_VICT);
			if (!number(0, 100))
				DamageOneItem(ch, SPLDAM_GENERIC, weapon, FALSE);
			break;
		default:
	...
	for (other_weapon = ch->carrying; other_weapon; other_weapon = other_weapon->next_content)
...
void do_charge(P_char ch, char *argument, int cmd)
{
	...
	struct damage_messages messages = {"Your charge impales $N.",
	...
	if (!has_innate(ch, INNATE_CHARGE) || (IS_RIDING(ch) && (GET_CLASS(ch, CLASS_PALADIN) || GET_CLASS(ch, CLASS_ANTIPALADIN) || GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_LANCER))))
	{
	...
	if (dir != -1)
	{
		if (!EXIT(ch, dir) || IS_SET(EXIT(ch, dir)->exit_info, EX_CLOSED) || IS_SET(EXIT(ch, dir)->exit_info, EX_SECRET) || IS_SET(EXIT(ch, dir)->exit_info, EX_BLOCKED) ||
	...
	*/
	...
	if (number(1, 100) <= (int)percent_chance || IS_IMMOBILE(victim) || !IS_AWAKE(victim))
	{
		if (get_takedown_size(victim) <= get_takedown_size(ch) && !number(0, 2))
		{
			...
			act("$n &+ycharges through the room and crashes into&n $N &+yknocking $M to the &+Yground!", 0, ch, 0, victim, TO_NOTVICT);
			act("$n &+ycharges into you, knocking you to the &+Yground!&n &+yYou hear a crunching noise as &+Wbones break!&n", 0, ch, 0, victim, TO_VICT);
		...
		else
		{
			...
			act("$n &+ycharges through the room and crashes into&n $N! &+yYou hear the sound of &+Wbreaking bones!", 0, ch, 0, victim, TO_NOTVICT);
			act("$n &+ycharges into you! You hear a crunching noise as &+Wbones break!&n", 0, ch, 0, victim, TO_VICT);
		}
	...
	{
		if ((dir != -1) && EXIT(ch, dir) && EXIT(ch, dir)->to_room != NOWHERE && !(IS_SET(EXIT(ch, dir)->exit_info, EX_CLOSED | EX_BLOCKED | EX_SECRET)) &&
		    !IS_ROOM(ch->in_room, ROOM_GUILD)) // Prevents charging past golems
		{
...
void do_kill(P_char ch, char *argument, int cmd)
{
	...
	else
	{
		...
		else
		{
			if (IS_PC(victim))
			{
				statuslog(victim->player.level, "%s killed by %s at %s [%d]", GET_NAME(victim), GET_NAME(ch), world[victim->in_room].name, world[victim->in_room].number);
			}

			act("You grab $M by the throat and rip out $S still beating heart!", FALSE, ch, 0, victim, TO_CHAR);
			act("$N grabs you by the throat and rips out your still beating heart!", FALSE, victim, 0, ch, TO_CHAR);
			act("$n grabs $N by the throat and rips out $S still beating heart!", FALSE, ch, 0, victim, TO_NOTVICT);
...
void event_uncircle(P_char ch, P_char victim, P_obj obj, void *args) { unlink_char(ch, victim, LNK_CIRCLING); }

void circling_broken(struct char_link_data *cld) { act("$N suddenly orients $Mself into a more defensive position.", FALSE, cld->linking, 0, cld->linked, TO_CHAR); }
...
bool circle(P_char ch, P_char victim)
{
	...
	if (!notch_skill(ch, SKILL_CIRCLE, get_property("skill.notch.offensive", 7)) && skill < number(1, 100))
	{
		act("Damn! You weren't quite stealthy enough, and $N noticed your attempt to circle $M!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n clumsily tries to sneak behind $N, but $N notices $m anyway.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("You notice $n attempting to maneuver behind you, and take a more defensive stance. Who is $e fooling?", FALSE, ch, 0, victim, TO_VICT);
		return 0;
	}
	...
	act("$n moves in the shadows, suddenly reappearing behind $N!", FALSE, ch, 0, victim, TO_NOTVICT);
	act("$n disappears from your vision for a moment, suddenly appearing behind you; you fail to counter $s advance...", FALSE, ch, 0, victim, TO_VICT);
...
void do_order(P_char ch, char *argument, int comd)
{
	...
	if (IS_SET(ch->specials.affected_by2, AFF2_MAJOR_PARALYSIS) || IS_SET(ch->specials.affected_by2, AFF2_MINOR_PARALYSIS))
	{
		...
		if (victim)
		{
			...
			*/
			...
			else
			{
				...
				{
					act("$N seems a bit busy at the moment, try later.", FALSE, ch, 0, victim, TO_CHAR);
					return;
				}
			}
		}
				...
				if (k && ch)
				{
					if (k->specials.z_cord == org_cord)
					{
						...
						*/
								...
								{
									act("$N seems a bit busy at the moment, try later.", FALSE, ch, 0, k, TO_CHAR);
								}
								else
								{
									/*
...
bool bad_flee_dir(const P_char ch, const int to_room)
{
	...
	return ((world[to_room].sector_type == SECT_NO_GROUND || world[to_room].chance_fall || world[to_room].sector_type == SECT_UNDRWLD_NOGROUND) && !IS_TRUSTED(ch) && !IS_AFFECTED(ch, AFF_LEVITATE) &&
	        !IS_AFFECTED(ch, AFF_FLY) && !(mount && (IS_AFFECTED(mount, AFF_LEVITATE) || IS_AFFECTED(mount, AFF_FLY))));
}
...
void do_flee(P_char ch, char *argument, int cmd)
{
	...
	if (IS_NPC(ch))
		if (mob_index[GET_RNUM(ch)].virtual_number == 11002 || mob_index[GET_RNUM(ch)].virtual_number == 11003 || mob_index[GET_RNUM(ch)].virtual_number == 11004 ||
		    IS_SET(ch->specials.act2, ACT2_NO_LURE) || GET_RACE(ch) == RACE_CONSTRUCT)
			return;
	...
	if (IS_AFFECTED2(ch, AFF2_STUNNED) && IS_FIGHTING(ch))
	{
		act("&+cYo&+Cu t&+cry &+Cto&+c ov&+Cer&+cco&+Cme &+cyo&+Cur &+cw&+Goo&+czy &+Cfe&+cel&+Cing.&n", FALSE, ch, 0, 0, TO_CHAR);
	...
	if (IS_FIGHTING(ch) && ch->equipment[WEAR_BODY] && (obj_index[ch->equipment[WEAR_BODY]->R_num].virtual_number == DRAGONSLAYER_VNUM))
	{
		if (cmd != CMD_FLEE)
		{
	...
	if (IS_NPC(ch) && IS_SET(ch->specials.act, ACT_MOUNT) && cmd != CMD_FLEE)
	{
		if (rider)
		{
	...
	for (i = 0; i < NUM_EXITS; i++)
		if (world[ch->in_room].dir_option[i])
			if (CAN_GO(ch, i) && !bad_flee_dir(ch, EXIT(ch, i)->to_room) && can_enter_room(ch, EXIT(ch, i)->to_room, 0))
				available_exits++;
	...
	if (!IS_SET(ch->specials.act, ACT_MOUNT) && IS_FIGHTING(ch) && (notch_skill(ch, SKILL_EXPEDITIOUS_RETREAT, 10) || (GET_CHAR_SKILL(ch, SKILL_EXPEDITIOUS_RETREAT) > number(1, 100))))
		expeditious = true;
	...
	if (*arg && IS_PC(ch))
	{
		if (GET_CLASS(ch, CLASS_ROGUE))
		{
			...
			if (skl && (notch_skill(ch, SKILL_CONTROL_FLEE, get_property("skill.notch.offensive", 7)) || skl > number(1, 130)))
	...
	chance_when_engaged = MIN_CHANCE_TO_FLEE + MIN(3, available_exits - 1) * (MAX_CHANCE_TO_FLEE - MIN_CHANCE_TO_FLEE) / 3;
	...
	if (!IS_TRUSTED(ch) && !(GET_CLASS(ch, CLASS_DRUID) || (IS_MULTICLASS_PC(ch) && GET_SECONDARY_CLASS(ch, CLASS_DRUID))) && get_spell_from_room(&world[ch->in_room], SPELL_WANDERING_WOODS))
	{
		if (number(1, (int)((GET_C_INT(ch) - 100) / 20) + 100) < 61)
		{
			send_to_char("You try to leave, but just end up going in circles!\n", ch);
			act("$n leaves one direction and enters from another!", FALSE, ch, 0, 0, TO_ROOM);
			succeded = FALSE;
		}
	}
	...
	if (start_room == ch->in_room)
	{
		...
		if (mount_opponent && !IS_FIGHTING(mount) && !mount->specials.next_fighting)
	...
	{
		if (IS_PC(ch) && !GET_CLASS(ch, CLASS_ROGUE) && !has_innate(ch, INNATE_IMPROVED_FLEE))
		{
		...
		else if (GET_CLASS(ch, CLASS_ROGUE) && GET_VITALITY(ch) > 0)
...
char *monk_combos_messages[][2][3] = {
	{{"$n spins around on $s foot and swings at $N with $s leg...", "You spin around on your foot and swing at $N with your leg...", "$n spins around on $s foot and swings at you with $s leg..."},
     {"$n pulls $N's head down and brings $s knee up, smashing it...",
     "You pull $N's head down and bring your knee up, smashing it...",
     "$n pulls $N's head down and brings $s knee up, smashing it..."}																														  },
	{				  {"...then swings $s elbow around, smashing $N's face...", "...then swing your elbow around, smashing $N's face...", "...then swings $s elbow around, smashing your face..."},
	...
	{										   {"...then smashes $N's head between $s fists...", "...then smash $N's head between your fists...", "...then smashes your head between $s fists..."},
     {"...then pounds $N's face with a series of swift jabs...", "...then pound $N's face with a series of swift jabs...", "...then pounds your face with a series of swift jabs..."}          },
	...
	{													   {"...then nails $N with a swift uppercut...", "...then nail $N with a swift uppercut...", "...then nails you with a swift uppercut..."},
     {"...then launches a mighty kick to $N's groin...", "...then launch a mighty kick to $N's groin...", "...then launches a mighty kick to your groin..."}                                   },
	{																											{"...then whirls around behind $N sending a crushing blow to $S spine...",
     ...
     {"...then shifts balance, and rams $s head into $N's face...", "...then shift balance, and ram your head into $N's face...", "...then shifts balance, and rams $s head into your face..."}},
...
void event_combination(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if ((GET_RACE(victim) == RACE_GOLEM) || (GET_RACE(victim) == RACE_DRAGON) || (GET_RACE(victim) == RACE_GHOST) || (IS_AFFECTED4(victim, AFF4_PHANTASMAL_FORM)))
	{
	...
	do
	{
		...
		stage++;
	} while (result == DAM_NONEDEAD && skill >= skill_req && percent > number(0, 100) && stage < 7);
...
void do_combination(P_char ch, char *argument, int cmd)
{
	...
	if (!affect_timer(ch, get_property("timer.secs.monkCombination", 30) * WAIT_SEC, SKILL_COMBINATION))
	{
	...
	add_event(event_combination, GET_LEVEL(ch) < 51 ? 2 * PULSE_VIOLENCE : PULSE_VIOLENCE, ch, 0, 0, 0, 0, 0);
	act("&+L$n begins moving into position for a combination...&n", TRUE, ch, 0, 0, TO_ROOM);
	act("&+LYou begin moving into position for a combination...&n", TRUE, ch, 0, 0, TO_CHAR);
}
...
void event_barrage(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	if (!number(0, 30))
	{
		act("&+R$n attempts a fancy attack, but almost cuts $s fingers off in the process!", FALSE, ch, 0, 0, TO_ROOM);
		act("&+RYou attempt your blade barrage, but fail to execute the deadly attack!", FALSE, ch, 0, 0, TO_CHAR);
		return;
	}
...
void do_consume(P_char ch, char *argument, int cmd)
{
	...
	struct damage_messages messages = {"The &=LBlightning bolt&N hits $N with full impact.",
...
void rush(P_char ch, P_char victim)
{
	...
	if (GET_OPPONENT(ch) && GET_OPPONENT(ch) == victim)
	{
	...
	if (notch_skill(ch, SKILL_RUSH, get_property("skill.notch.offensive", 7)) || number(1, 100) > GET_CHAR_SKILL(ch, SKILL_RUSH))
	{
		act("$n rushes toward $N in a furious rage, but misses $s target completely.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n rushes toward you in a furious rage, but you easily evade $s attack.", FALSE, ch, 0, victim, TO_VICT);
		act("You try to madly rush at $N, but end up almost tripping yourself.", FALSE, ch, 0, victim, TO_CHAR);
	}
	else
	{
		act("$n rushes toward $N in a furious rage...", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n rushes toward you in a furious rage...", FALSE, ch, 0, victim, TO_VICT);
		act("&+rA red haze fills your vision as you madly rush at $N.", FALSE, ch, 0, victim, TO_CHAR);
...
void do_rush(P_char ch, char *argument, int cmd)
{
	...
	{
		send_to_char("&+gYour eyes glow &+rred&+g with hatred, but you can't seem to find your victim.\n", ch);
		return;
	}
...
void do_rescue(P_char ch, char *argument, int cmd)
{
	...
	if (GET_SPEC(ch, CLASS_WARRIOR, SPEC_GUARDIAN) && strstr(argument, "all") && GET_LEVEL(ch) >= 46)
...
bool isMaulable(P_char ch, P_char victim)
{
	...
	if (!IS_ALIVE(ch) || !IS_ALIVE(victim) || !CanDoFightMove(ch, victim) || !GET_CHAR_SKILL(ch, SKILL_MAUL) || GET_CHAR_SKILL(ch, SKILL_MAUL) < 1 || GET_POS(ch) != POS_STANDING ||
	    GET_POS(victim) != POS_STANDING || ch->in_room != victim->in_room || IS_IMMATERIAL(victim) || IS_ELEMENTAL(victim) || IS_WATERFORM(victim) || IS_DRAGON(victim) ||
	    (IS_NPC(victim) && IS_SET(victim->specials.act, ACT_NO_BASH)) || ch->specials.z_cord != 0)
	{
		return false;
	}

	if ((has_innate(victim, INNATE_HORSE_BODY) || has_innate(victim, INNATE_SPIDER_BODY)) && get_takedown_size(ch) <= get_takedown_size(victim))
	{
		return false;
	}
...
int chance_kick(P_char ch, P_char victim)
{
	...
	percent_chance = (int)(percent_chance * ((int)BOUNDED(80, GET_C_STR(ch) + GET_C_AGI(ch) - GET_C_AGI(victim), 125)) / 100);
...
void kick(P_char ch, P_char victim)
{
	...
	if (!notch_skill(ch, SKILL_KICK, get_property("skill.notch.offensive", 7)) && (percent_chance < number(1, 100) || IS_IMMATERIAL(victim)))
	{
	...
	{
		act("Your nimble kick slams into $N's groin, and $N whimpers before crashing to the ground!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n's nimble kick crashes into your groin .. OUCH!!! .. and down you go!", FALSE, ch, 0, victim, TO_VICT);
		act("$n's nimble kick connects with $N's groin...\r$N crashes to the ground!", FALSE, ch, 0, victim, TO_NOTVICT);
		SET_POS(victim, POS_SITTING + GET_STAT(victim));
...
void do_dragon_roar(P_char ch, char *argument, int cmd)
{
	...
	if (!has_dragoon_mount(ch))
	{
		send_to_char("You must command a &+Gdr&+Lag&+Gon&n to do that.\n", ch);
		return;
	}

	if (!is_dragoon_mounted(ch))
	{
		send_to_char("You must be riding a &+Gdr&+Lag&+Gon&n to do that.\n", ch);
		return;
	}
	...
	if (affected_by_spell(ch, SKILL_DRAGON_ROAR) || affected_by_spell(ch, SKILL_DRAGON_BREATH) || affected_by_spell(ch, SKILL_DRAGON_STRIKE))
	{
	    ...
	    /*&& number(0, 75) < GET_CHAR_SKILL(ch, SKILL_MOUNT)*/)
	{
		...
		else
		{
			for (victim = world[ch->in_room].people; victim; victim = victim->next_in_room)
			{
				...
				else
				{
					...
					if (!IS_AFFECTED4(victim, AFF4_NOFEAR) && !IS_ELITE(victim) && !IS_GREATER_RACE(victim))
					{
						if (is_hunter)
						{
							if ((number(0, 100) < (int)get_property("skill.dragon_roar.hunter.flee.chance", 10.000)) && (number(0, 100) < GET_CHAR_SKILL(ch, SKILL_DRAGON_ROAR)) &&
							    ((GET_LEVEL(ch) + MIN(5, (empower / 20))) >= GET_LEVEL(victim)) && !IS_TRUSTED(victim) && !fear_check(victim))
							{
			...
			snprintf(buf,
			         MAX_STRING_LENGTH,
			         "The &+Gdr&+Lag&+Gon&n god's &+rpower&n drains from your %d%s circle of knowledge.\n",
			         circle,
...
void do_dragon_breath(P_char ch, char *argument, int cmd)
{
	...
	if (!SanityCheck(ch, "dragon_strike"))
		return;
	...
	{
		send_to_char("Your &+Gdr&+Lag&+Gon&n cannot breath fire at itself!\r\n", ch);
		return;
	}
	...
	if (victim == NULL)
	{
		send_to_char("Breath fire at who or what?\r\n", ch);
		return;
	}
	...
	if ((!notch_skill(ch, SKILL_MOUNTED_COMBAT, get_property("skill.notch.offensive", 7)) && (percent_chance + power) < number(1, 100)) /*|| number(0, 65) > GET_CHAR_SKILL(ch, SKILL_MOUNT)*/)
	{
		if (IS_DRAGON_FORM(ch))
		{
			send_to_char("The &+Gdr&+Lag&+Gon&n god's &+rpower&n betrays you.\r\n", ch);
			...
			if (GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_PRIEST))
			{
				if (strcmp(argument, "") != 0 && strcmp(argument, " ") != 0)
				{
					...
					if (spellName)
					{
						...
						{
							if (ch->specials.undead_spell_slots[2])
								chosenCircle = 2;
							else if (ch->specials.undead_spell_slots[1])
								chosenCircle = 1;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
						else if (is_abbrev(spellName, "flameburst"))
						{
							if (ch->specials.undead_spell_slots[4])
								chosenCircle = 4;
							else if (ch->specials.undead_spell_slots[3])
								chosenCircle = 3;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
						...
						{
							if (ch->specials.undead_spell_slots[6])
								chosenCircle = 6;
							else if (ch->specials.undead_spell_slots[5])
								chosenCircle = 5;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
						...
						{
							if (ch->specials.undead_spell_slots[8])
								chosenCircle = 8;
							else if (ch->specials.undead_spell_slots[7])
								chosenCircle = 7;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
						...
						{
							if (ch->specials.undead_spell_slots[10])
								chosenCircle = 10;
							else if (ch->specials.undead_spell_slots[9])
								chosenCircle = 9;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
						...
						{
							if (ch->specials.undead_spell_slots[12])
								chosenCircle = 12;
							else if (ch->specials.undead_spell_slots[11])
								chosenCircle = 11;
							else
								send_to_char("You must commune with the &+Gdr&+Lag&+Gon&n god for more &+rpower&n\n", ch);
						}
...
void do_dragon_strike(P_char ch, char *argument, int cmd)
{
	...
	char                   buf[512];
	char                   name[MAX_INPUT_LENGTH];
	struct damage_messages messages = {"Your &+Gdr&+Lag&+Gon&N hearkens your call and sweeps $s tail into $N with a magnificent &-L&+RSWOOSH&n!",
	...
	struct damage_messages messages_dragon_form = {"You sweep your tail into $N with a magnificent &-L&+RSWOOSH&n!",
	                                               "$n's tail sweeps into you with a magnificent &-L&+RSWOOSH&n!",
	                                               "$n's tail sweeps into $N's body with a magnificent &-L&+RSWOOSH&n!",
	...
	if (victim == NULL)
	{
		send_to_char("Tail strike who or what?\r\n", ch);
		return;
	}
	...
	is_lancer = GET_SPEC(ch, CLASS_DRAGOON, SPEC_DRAGON_LANCER);
	...
	knockdown_chance = (int)(knockdown_chance * ((double)BOUNDED(8, 10 + (GET_LEVEL(ch) - GET_LEVEL(victim)) / 10, 11)) / 10);
	knockdown_chance = (int)(knockdown_chance * ((double)BOUNDED(83, 50 + GET_C_AGI(ch) / 2, 120)) / 100);
	...
	if (is_lancer)
	{
		dam = (int)(dam * get_property("skill.dragon_strike.lancer.DamageMultiplier", 2.000));
	}
	...
	if ((ch_size > vict_size + 1) && !(is_lancer && (vict_size == ch_size - 2)))
	{
		percent_chance   = (int)(percent_chance * 0.9);
	...
	if (!notch_skill(ch, SKILL_MOUNTED_COMBAT, get_property("skill.notch.offensive", 7)) && (percent_chance + power) < number(1, 100))
	{
		...
		else
		{
			if (IS_DRAGON_FORM(ch))
			{
				...
				act("$N thrashes wildy $n as they miss thier target!", FALSE, ch, 0, mount, TO_ROOM);
			}
			else
			{
				act("As $N thrashes wildy as you are thrown to the ground!", FALSE, ch, 0, mount, TO_CHAR);
				act("As $N thrashes wildy as $n is thrown to the ground!", FALSE, ch, 0, mount, TO_ROOM);
			}

			if (50 + GET_C_AGI(ch) / 2 < number(0, 100))
			{
				SET_POS(ch, POS_SITTING + GET_STAT(ch));
				act("You face plant hard into the ground!", FALSE, ch, 0, mount, TO_CHAR);
				act("CRASH! $n face plants hard into the ground!", FALSE, ch, 0, victim, TO_ROOM);
	...
	else
	{
		...
		if (IS_PC(ch) && GET_POS(victim) == POS_STANDING)
		{
			debug("DRAGON_STRIKE: (%s) trying to dragon strike with (%d) percent knock down. Victim is (%s).", GET_NAME(ch), knockdown_chance, GET_NAME(victim));
		}
...
void do_kick(P_char ch, char *argument, int cmd)
{
	...
	else if ((GET_RACE(ch) == RACE_SNAKE) || (GET_RACE(ch) == RACE_INSECT) || (GET_RACE(ch) == RACE_PARASITE))
	{
...
int chance_roundkick(P_char ch, P_char victim)
{
	...
	if ((get_takedown_size(victim) > get_takedown_size(ch)) || (get_takedown_size(victim) < get_takedown_size(ch) - 1))
	{
	...
	percent_chance = (int)(percent_chance * ((double)BOUNDED(6, 10 + (GET_LEVEL(ch) - GET_LEVEL(victim)) / 10, 15)) / 10);

	/* agility is the prime stat of the monk */
	percent_chance = (int)(percent_chance * ((double)BOUNDED(20, 100 + (int)(1.5 * (GET_C_AGI(ch) - GET_C_AGI(victim))), 150)) / 100);
...
{
	int                    percent_chance, dam;
	struct damage_messages messages = {"Your roundhouse kick hits $N in the solar plexus!",
	...
	dam = GET_C_DEX(ch) + GET_CHAR_SKILL(ch, SKILL_ROUNDKICK) - GET_C_AGI(victim);

	if (!notch_skill(ch, SKILL_ROUNDKICK, get_property("skill.notch.offensive", 7)) && percent_chance < number(0, 100))
	{
	...
	else if (melee_damage(ch, victim, MAX(1, dam), PHSDAM_TOUCH, &messages) == DAM_NONEDEAD)
	{
		...
		if (GET_C_DEX(ch) - GET_C_AGI(victim) > number(0, 100))
		{
			...
			act("$n swings about and delivers a &+ymighty roundhouse kick&n to $N!", FALSE, ch, 0, victim, TO_NOTVICT);
...
void do_assist_core(P_char ch, P_char victim)
{
	...
	if (IS_ROOM(ch->in_room, ROOM_SINGLE_FILE))
	{
		if (AdjacentInRoom(ch, victim))
			act("$N is in your way!  You can't get past to assist $M!", FALSE, ch, 0, victim, TO_CHAR);
		else
	...
	{
		act("Too late, doesn't look like $N really needs any help.", FALSE, ch, 0, victim, TO_CHAR);
		statuslog(56, "SHABOOM!  %s assisting %s against deader.", GET_NAME(ch), GET_NAME(victim));
		return;
	}
	if (victim == ch)
	{
...
void knock_out(P_char ch, int duration)
{
	...
	if (ch)
	{
		...
		affect_to_char_with_messages(ch, &af, "&+GThe &+Cworld&n seems to come back into focus.", "$n seems to have come back to his senses.");
...
void do_headbutt(P_char ch, char *argument, int cmd)
{
	...
	struct damage_messages  messages_humanoid = {"You leave a huge, &+rred&N swollen lump on $N's temple.",
	                                             ...
	                                             "$n bashes $N's skull in with a swift, stern butt to the head.",
	                                             0};
	struct damage_messages *messages          = &messages_humanoid;

	struct damage_messages messages_other = {"You slam your head into $N, leaving a huge, &+rred&N swollen lump.",
	...
	struct damage_messages fail_messages = {"You get the best of $N as $E headbutts you.",
	                                        ...
	                                        "$N skull splits open as it collides with $n in the wrong way.",
	                                        0};
   ...
   */
	...
	*/
	...
	else
	{
		...
		debug("do_headbutt: (%s) butting (%s) dam (%d) skill (%d).", GET_NAME(ch), GET_NAME(victim), (int)dam, GET_CHAR_SKILL(ch, SKILL_HEADBUTT));
...
void event_sneaky_strike(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	struct damage_messages messages = {"You find weakness in $N's defenses and land a sneaky surprise attack!",
	                                   ...
	                                   "$n finishes $N off with a sudden surprise attack!",
	                                   0,
	                                   0};
	...
	dam = (int)(dam * number(25, 30) * (1 + ((double)GET_LEVEL(ch)) / MAXLVLMORTAL) * (((double)skill) / 100));
	...
	if (GET_SPEC(ch, CLASS_ROGUE, SPEC_THIEF) || (GET_CLASS(ch, CLASS_BARD))) // proc skill for thief - possibility to blind on sneaky strike
	{
		...
		if (number(1, 100) < i)
		{
			...
			{
				act("&+LYou quickly step into the sh&+wad&+Wows&+L, and suddenly appear behind $N &+Lstriking violently...", FALSE, ch, 0, victim, TO_CHAR);
				act("$n &+Lquickly steps into the sh&+wad&+Wows&+L, and suddenly appears behind $N &+Lstriking violently...", TRUE, ch, 0, victim, TO_NOTVICT);
			...
			{
				act("&+LYou lash out from the sh&+wad&+Wows&+L, hitting $N right in the &+Wface&+L, causing them to temporarily black out.", FALSE, ch, 0, victim, TO_CHAR);
				act("$n &+Lsteps quickly into the sh&+wad&+Wows&+L and darts quickly at your &+Wface&+L, causing you to temporarily black out.", TRUE, ch, 0, victim, TO_VICT);
				act("$n &+Lfades into the sh&+wad&+Wows&+L and quickly strikes at $N's &+Wface&+L, causing them to see stars.", TRUE, ch, 0, victim, TO_NOTVICT);
	...
	if (GET_SPEC(ch, CLASS_ROGUE, SPEC_ASSASSIN)) // proc skill for assassin and bard
	{
		...
		if (number(1, 100) < i)
		{
			...
			{
				int numb = number(4, 7);
				act("&+rYou sneak up on $N &+rand drive your &+Lweapon &+rdeep into a vital organ!&N", TRUE, ch, obj, victim, TO_CHAR);
				act("&+L$n &+rsneaks up on $N&+r driving their &+Lweapon &+rdeep into a vital organ!&N", TRUE, ch, obj, victim, TO_NOTVICT);
				act("&+L$n&+r appears behind you and drives their &+Lweapon &+rdeep into one of your vital organs!&N", TRUE, ch, obj, victim, TO_VICT);
				add_event(event_bleedproc, PULSE_VIOLENCE, ch, victim, 0, 0, &numb, sizeof(numb));
				act("&+L...$n &+Lthen quickly fades into the sh&+wad&+Wow&+L, re-appearing only to attack again!", TRUE, ch, 0, victim, TO_NOTVICT);
				single_stab(ch, victim, weapon);
			}
			else
			{
	...
	if (GET_CLASS(ch, CLASS_MERCENARY)) // proc skill for mercenary - sucks moves
	{
		...
		if (number(1, 100) < i)
		{
			...
			{
				act("&+yYou fill with the &+RRAGE &+yof your &+Yancestors&+y, striking $N &+ywith all of your &+Bmight&+y!", FALSE, ch, 0, victim, TO_CHAR);
				act("$n &+yseems to fill with unearthly &+RRAGE&+y, and strikes $N &+ywith all of their &+Bmight&+y!", TRUE, ch, 0, victim, TO_NOTVICT);
				Stun(victim, victim, PULSE_VIOLENCE, FALSE);
				;
			}
			else
			{
				act("&+yYou tighten up and skillfully hit $N right in the &+Ystomach&+y! $N appears a bit less energetic after that hit.", FALSE, ch, 0, victim, TO_CHAR);
				act("$n &+yseems to tense up before unleasing a &+Ypowerful &+ystrike to your abdomen! You feel the a lot less &+Yenergetic&+y.", TRUE, ch, 0, victim, TO_VICT);
				act("$n &+yseems to tense up before unleasing a &+Ypowerful &+ystrike to $N's &+yabdomen&+y, knocking the &+Cwind &+yout of them!", TRUE, ch, 0, victim, TO_NOTVICT);
				i = number(20, 45);
...
bool single_stab(P_char ch, P_char victim, P_obj weapon)
{
	...
	struct damage_messages messages = {"$N makes a strange sound as you place $p in $S back.",
	...
	{
		crit = notch_skill(ch, SKILL_CRITICAL_STAB, get_property("skill.notch.offensive", 7)) || ((critical_stab * GET_CHAR_SKILL(ch, SKILL_CRITICAL_STAB)) > number(1, 100));
	}

	if ((IS_AFFECTED(victim, AFF_AWARE) && !crit && !IS_AFFECTED(victim, AFF_KNOCKED_OUT) && ((GET_RACE(ch) != RACE_MOUNTAIN) && (GET_RACE(ch) != RACE_DUERGAR))) ||
	...
	if (affected_by_spell(victim, SPELL_STONE_SKIN) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+Lstone skin&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+Lstone skin&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+Lstone skin&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (affected_by_spell(victim, SPELL_DRAKESCALE_AEGIS) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+Gdrak&+Les&+Gcale&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+Gdrak&+Les&+Gcale&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+Gdrak&+Les&+Gcale&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (affected_by_spell(victim, SPELL_SHADOW_SHIELD) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+Lshadow shield&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+Lshadow shield&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+Lshadow shield&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (affected_by_spell(victim, SPELL_BIOFEEDBACK) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+Gbiofeedback&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+Gbiofeedback&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+Gbiofeedback&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (affected_by_spell(victim, SPELL_VINES) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+Gvines&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+Gvines&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+Gvines&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (affected_by_spell(victim, SPELL_IRONWOOD) && dam > 1)
	{
		...
		dam = 1;
		act("$n attempts to stab you in the back, but your &+yironwood&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to stab $N in the back, but their &+yironwood&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to stab $N in the back, but $N's &+yironwood&n quickly absorbs the impact before fading away!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
	}
	...
	if (crit)
	{
		...
		if (melee_damage(ch, victim, dam, PHSDAM_NOREDUCE | PHSDAM_NOPOSITION, &messages) || !is_char_in_room(ch, room))
		{
			return TRUE;
		}
	...
	// else if - can not spinal as well as critical stab. (drannak 1/7/14)
	         ...
	         (notch_skill(ch, SKILL_SPINAL_TAP, get_property("skill.notch.offensive", 7)) || (spinal_tap * GET_CHAR_SKILL(ch, SKILL_SPINAL_TAP)) > number(1, 100)))
	{
		if (DEBUG)
		{
			debug("single_stab: (%s) stabbing (%s) dam (%d) weapon (%dd%d) skill (%d).", GET_NAME(ch), GET_NAME(victim), dam, weapon->value[1], weapon->value[2], skill);
	...
	else
	{
		...
		if (melee_damage(ch, victim, dam, PHSDAM_NOREDUCE | PHSDAM_NOPOSITION, &messages) || (room < 1) || !is_char_in_room(ch, room))
		{
			return TRUE;
		}
	}
	...
	if (spinal)
	{
		act("With a swift tug you wrench the weapon free, ramming it into $N's spine!.", FALSE, ch, 0, victim, TO_CHAR);
		act("With a swift tug $n wrenches the weapon free, ramming it into you again.", FALSE, ch, 0, victim, TO_VICT);
		act("With a swift tug $n wrenches the weapon free, ramming it into $N's spine!", FALSE, ch, 0, victim, TO_NOTVICTROOM);
		if (melee_damage(ch, victim, (number(-12, 12) + GET_LEVEL(ch)) / (quarter ? 4 : 1), PHSDAM_NOREDUCE | PHSDAM_NOPOSITION, &messages) || !char_in_list(ch))
		{
			return TRUE;
		}
	}
...
bool backstab(P_char ch, P_char victim)
{
	...
	if ((get_takedown_size(ch) > get_takedown_size(victim) + 1) && IS_AWAKE(victim) && GET_POS(victim) > POS_KNEELING)
	{
		send_to_char("Smirk. I don't believe you could sneak behind to stab it.\n", ch);
		return FALSE;
	}

	if (get_takedown_size(ch) + 1 < get_takedown_size(victim) && GET_POS(victim) > POS_KNEELING)
	{
		send_to_char("Smirk. Backstabbing in a calf is not very effective.\n", ch);
		return FALSE;
	}
	...
	if ((!first_w || !IS_BACKSTABBER(first_w)) && (!second_w || !IS_BACKSTABBER(second_w)))
	{
	...
	*/
	...
	if (has_innate(victim, INNATE_DRAGONMIND) && number(0, 1))
	{
		...
		act("$n's attempt to backstab you fails as the dragon mind within takes control!", FALSE, ch, 0, victim, TO_VICT);
		set_fighting(ch, victim);
	...
	if (IS_AFFECTED2(victim, AFF2_AIR_AURA))
	{
		if (number(1, 20) < 7)
		{
			act("As you try to backstab $N you simply pass through $M.", FALSE, ch, 0, victim, TO_CHAR);
			act("$n tries to backstab $N but simply falls through $M.", FALSE, ch, 0, victim, TO_NOTVICT);
			act("$n's attempt to backstab you fails as he passes through you.", FALSE, ch, 0, victim, TO_VICT);
			set_fighting(ch, victim);
			return FALSE;
		}
	}
	...
	{
		stabbed = TRUE;
		if (notch_skill(ch, SKILL_BACKSTAB, get_property("skill.notch.offensive", 7)) || percent_chance > number(0, 100) || GET_STAT(victim) <= STAT_SLEEPING)
		{
	...
	if ((!stabbed || GET_CLASS(ch, CLASS_ASSASSIN) || GET_SPEC(ch, CLASS_ROGUE, SPEC_ASSASSIN)) && second_w && IS_BACKSTABBER(second_w))
	{
...
void attack(P_char ch, P_char victim)
{
	...
	if (!ch || !victim || !CanDoFightMove(ch, victim) ||
	    IS_IMMOBILE(ch)
	...
	{
		if (!CanDoFightMove(GET_OPPONENT(ch), ch) || IS_IMMOBILE(ch) || GET_STAT(GET_OPPONENT(ch)) <= STAT_SLEEPING)
		{
			stop_fighting(ch);
		...
		if (notch_skill(ch, SKILL_SWITCH_OPPONENTS, get_property("skill.notch.switch", 10)) || skl >= number(1, 101))
		{
			stop_fighting(ch);
			act("$n turns to focus $s attack on $N!", FALSE, ch, 0, victim, TO_ROOM);
			victim = guard_check(ch, victim);
...
bool isKickable(P_char ch, P_char victim)
{
	...
	if (!IS_ALIVE(ch) || !IS_ALIVE(victim) || IS_IMMOBILE(ch) || !CanDoFightMove(ch, victim) || IS_IMMATERIAL(victim) || (ch->in_room != victim->in_room) || !HAS_FOOTING(ch) || LEGLESS(ch))
	{
		return false;
	}
...
bool isBashable(P_char ch, P_char victim, bool ignoreImmaterial)
{
	...
	// non-size checks
	if (IS_IMMATERIAL(victim) || IS_ELEMENTAL(victim) || IS_WATERFORM(victim) || (IS_IMMATERIAL(ch) && !ignoreImmaterial) || IS_DRAGON(victim) || IS_DRAGON(ch) ||
	    (IS_NPC(victim) && IS_SET(victim->specials.act, ACT_NO_BASH)) || ((ch->specials.z_cord == 0) && !HAS_FOOTING(ch)) || !CanDoFightMove(ch, victim) || GET_POS(ch) != POS_STANDING ||
	    IS_STUNNED(ch))
	{
		return FALSE;
	}
...
int good_for_skewering(P_obj obj)
{
	...
	if ((obj->value[0] == WEAPON_AXE || obj->value[0] == WEAPON_LONGSWORD || obj->value[0] == WEAPON_2HANDSWORD || obj->value[0] == WEAPON_POLEARM || obj->value[0] == WEAPON_SPEAR ||
	     obj->value[0] == WEAPON_LANCE || obj->value[0] == WEAPON_TRIDENT || obj->value[0] == WEAPON_SICKLE || obj->value[0] == WEAPON_HORN) &&
...
void bash(P_char ch, P_char victim, bool _debug)
{
	...
	if (IS_AFFECTED2(ch, AFF2_AIR_AURA) || affected_by_spell(ch, SPELL_ETHEREAL_FORM) || ((GET_RACE(ch) == RACE_F_ELEMENTAL || GET_RACE(ch) == RACE_A_ELEMENTAL) && !IS_NPC(ch)))
	{
	...
	if (GET_POS(victim) != POS_STANDING)
	{
		act("As $N avoids your bash, you topple over and fall to the ground.", FALSE, ch, 0, victim, TO_CHAR);
		act("You dodge a bash from $n, who loses $s balance and falls.", FALSE, ch, 0, victim, TO_VICT);
		act("$N avoids being bashed by $n, who loses $s balance and falls.", FALSE, ch, 0, victim, TO_NOTVICT);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
	...
	if (ch_size < vict_size - 1 || ((has_innate(victim, INNATE_HORSE_BODY) || has_innate(victim, INNATE_SPIDER_BODY) || GET_RACE(ch) == RACE_QUADRUPED) && ch_size <= vict_size))
	{
		act("$n makes a futile attempt to bash $N, but $E is simply immovable.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n makes a futile attempt to bash you, but you are simply immovable.", FALSE, ch, 0, victim, TO_VICT);
		act("You attempt to bash $N, but $E is simply too huge for you to move!", FALSE, ch, 0, victim, TO_CHAR);
	...
	if (IS_HUMANOID(ch))
	{
		...
		if (!(ch->equipment[WEAR_SHIELD]) && (GET_CHAR_SKILL(ch, SKILL_SHIELDLESS_BASH) < 1) && (GET_CLASS(ch, CLASS_PALADIN | CLASS_ANTIPALADIN) && IS_PALADIN_SWORD(weap)))
		{
	...
	if (skewer > 0 && IS_FIGHTING(ch) && GET_OPPONENT(victim) != ch && GET_OPPONENT(ch) != victim)
	{
		skewer = (int)(skewer * get_property("skill.skewer.OffTarget.Penalty", 0.500));
	}

	if (skewer > 0 && GET_POS(victim) != POS_STANDING && weap && good_for_skewering(weap) && (notch_skill(ch, SKILL_SKEWER, get_property("skill.notch.offensive", 7)) || skewer / 3 > number(1, 100)))
	{
		act("$n grins as $e skewers you with $s $q.", FALSE, ch, weap, victim, TO_VICT);
		act("You dive at $N skewering $M with your $q.", FALSE, ch, weap, victim, TO_CHAR);
		act("$n dives at $N skewering $M with $s $q.", FALSE, ch, weap, victim, TO_NOTVICT);
	...
	if (!IS_PC_PET(ch))
	{
		...
		*/
		...
		modifier = 100 + GET_C_STR(ch) / 2.0 + ((IS_THRIKREEN(ch) && GET_C_DEX(ch) > 100) ? 100 : GET_C_DEX(ch)) - GET_C_AGI(victim);
	...
	if (vict_size > ch_size || ((has_innate(victim, INNATE_HORSE_BODY) || has_innate(victim, INNATE_SPIDER_BODY) || GET_RACE(ch) == RACE_QUADRUPED) && ch_size == vict_size))
	{
		bigger_victim = TRUE;
	}
	...
	if (IS_PC_PET(ch) && IS_ELEMENTAL(ch) && get_linked_char(ch, LNK_PET) && get_linked_char(ch, LNK_PET)->in_room == ch->in_room)
	{
	...
	if (!notch_skill(ch, SKILL_BASH, get_property("skill.notch.offensive", 7)) && percent_chance < rolled)
	{
		if (bigger_victim && number(1, 2) == 1)
		{
			act("As $N withstands your bash, you bounce back and fall to the ground.", FALSE, ch, 0, victim, TO_CHAR);
			act("You withstand a bash from $n, who bounces back and falls.", FALSE, ch, 0, victim, TO_VICT);
			act("$N withstands a bash from $n, who bounces back and falls.", FALSE, ch, 0, victim, TO_NOTVICT);
		}
		else
		{
			act("As $N avoids your bash, you topple over and fall to the ground.", FALSE, ch, 0, victim, TO_CHAR);
	...
	else
	{
		...
		if (GET_CHAR_SKILL(ch, SKILL_SKEWER) > 0 && weap && good_for_skewering(weap))
		{
			...
			if (notch_skill(ch, SKILL_SKEWER, get_property("skill.notch.offensive", 7)) || percent_chance > number(0, 100))
			{
				if (!IS_ALIVE(victim))
				{
...
void parlay(P_char ch, P_char victim)
{
	...
	if (GET_OPPONENT(victim))
	{
		if (number(85, 200) < skl_lvl)
		{
			...
			act("&+W$n speaks of &+Gharmony&+W and &+Ypeace&+W, convincing $N&+W to lay down their arms... &+Ckumbaya!&n", FALSE, ch, 0, victim, TO_NOTVICT);
			act("&+WAfter listening to $n, you are convinced that fighting doesn't solve anything. Kumbaya my friend..&n", FALSE, ch, 0, victim, TO_VICT);
			act("&+WYou convince $N&+W to lay down their arms!&n", FALSE, ch, 0, victim, TO_CHAR);
			do_flee(victim, 0, 2);
			CharWait(ch, PULSE_VIOLENCE * 2);
			return;
		}
		else if (skl_lvl > number(1, 100))
		{
			...
			act("&+W$n&+W convinces you that your quarrel with $m is futile!&n", FALSE, ch, 0, victim, TO_VICT);
...
void do_tackle(P_char ch, char *arg, int cmd)
{
	...
	if ((has_innate(vict, INNATE_HORSE_BODY) || has_innate(vict, INNATE_SPIDER_BODY) || GET_RACE(ch) == RACE_QUADRUPED) && (get_takedown_size(ch) <= get_takedown_size(vict)))
	{
		act("$n makes a futile attempt to tackle $N, but $E is simply immovable.", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n makes a futile attempt to tackle you, but you are simply immovable.", FALSE, ch, 0, vict, TO_VICT);
		act("&+WThat damn half-horse half-man is too hefty to tackle!", FALSE, ch, 0, vict, TO_CHAR);
	...
	if (IS_AFFECTED(vict, AFF_AWARE))
	{
		...
		if (GET_CHAR_SKILL(ch, SKILL_TACKLE) > number(1, 110) && number(0, 4) == 4) // Easter egg message
		{
	...
	if ((notch_skill(ch, SKILL_TACKLE, get_property("skill.notch.offensive", 7)) || number(1, 100) < percent_chance || IS_TRUSTED(ch)) && GET_POS(vict) == POS_STANDING)
	{
		if (!IS_TRUSTED(ch))
			set_short_affected_by(ch, SKILL_BASH, (int)(3.5 * PULSE_VIOLENCE));

		if (!IS_FIGHTING(ch) && !number(0, 4) && GET_POS(vict) == POS_STANDING && !IS_ROOM(ch->in_room, ROOM_GUILD)) // Prevents tackling past golems.
		{
			door = number(0, NUM_EXITS - 1);

			if (CAN_GO(ch, door) && !check_wall(ch->in_room, door) && !IS_OCEAN_ROOM(target_room = world[ch->in_room].dir_option[door]->to_room))
			{
				act("You &+ctackle&N $N &+RHARD&N in the chest and go &+Wflying&N out of the room with $m.", FALSE, ch, 0, vict, TO_CHAR);
				act("$n &+ctackles&N you &+RHARD&N and sends you &+Wflying&N out of the room with $M.", FALSE, ch, 0, vict, TO_VICT);
				act("$n &+ctackles&N $N &+RHARD&N sending them both &+Wflying&N out of the room!", FALSE, ch, 0, vict, TO_NOTVICT);
		...
		act("You &+ctackle&n $N square in the chest knocking the &+Cwind&n out of $M!", FALSE, ch, 0, vict, TO_CHAR);
		act("$n &+ctackles&n $N square in the chest knocking the &+Cwind&n out of $M!", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n &+ctackles&n you square in the chest knocking the &+Cwind&n out of you!", FALSE, ch, 0, vict, TO_VICT);
		...
		if (!affected_by_spell(vict, SKILL_TACKLE))
		{
			...
			act("You suddenly don't have the energy to fight!", FALSE, ch, 0, vict, TO_VICT);
		}
	}
	else
	{
		...
		{
			act("You leap at $N only to realize $E is down already.", FALSE, ch, 0, vict, TO_CHAR);
			act("$n leaps at $N only to realize $E is down already.", FALSE, ch, 0, vict, TO_NOTVICT);
			act("$n leaps at you only to realize you are down already.", FALSE, ch, 0, vict, TO_VICT);
		...
		{
			act("You try to &+ctackle&N $N and &+Wfly &Nover $S head landing &+Gface &Nfirst in the &+ydirt&N!", FALSE, ch, 0, vict, TO_CHAR);
			act("$n tries to &+ctackle&N $N and &+Wmisses completely&N, making a nice &+Gdent &Nin $s forehead and the ground!", FALSE, ch, 0, vict, TO_NOTVICT);
			act("$n tries to &+ctackle&N you and &+Wmisses completely&N, making a nice &+Gdent &Nin $s forehead and the ground!", FALSE, ch, 0, vict, TO_VICT);
...
void buck(P_char ch)
{
	...
	if ((IS_PC(ch) && !IS_CENTAUR(ch)) || (IS_NPC(ch) && !IS_SET(ch->specials.act, ACT_MOUNT)))
	{
		send_to_char("You need to be at least somewhat horse-like in order to buck, don't you think?\n", ch);
		return;
	}
	...
	if (IS_FIGHTING(ch))
	{
		send_to_char("You're a tad busy right now, try again when you're not in battle.\n", ch);
		return;
	}

	if (IS_FIGHTING(victim))
	{
		send_to_char("Your rider seems a tad busy right now, try again when they're not in battle.\n", ch);
		return;
	}
	...
	if (number(1, 50) <= 10)
	{
		act("You buck with ferocious might, but $N&n holds on too tightly.", TRUE, ch, 0, victim, TO_CHAR);
		act("$n&n suddenly bucks in an attempt to remove you, but you hold on too tightly.", TRUE, ch, 0, victim, TO_VICT);
		act("$n&n bucks in an attempt to dismount $N&n, but $E holds on too tightly.", TRUE, ch, 0, victim, TO_NOTVICT);
	...
	act("You buck with ferocious might, sending $N&n flying!", TRUE, ch, 0, victim, TO_CHAR);
	act("$n&n suddenly bucks, sending you flying!", TRUE, ch, 0, victim, TO_VICT);
	act("$n&n suddenly bucks, sending $N&n flying!", TRUE, ch, 0, victim, TO_NOTVICT);
...
void do_retreat(P_char ch, char *arg, int cmd)
{
	...
	*/
	...
	if (!EXIT(ch, dir) || IS_SET(EXIT(ch, dir)->exit_info, EX_SECRET) || IS_SET(EXIT(ch, dir)->exit_info, EX_BLOCKED))
	{
	...
	if (!world[ch->in_room].dir_option[dir] || !can_enter_room(ch, world[ch->in_room].dir_option[dir]->to_room, TRUE))
	{
	...
	{ // Success...
		if (found)
		{
			if (expdr && !IS_RIDING(ch) && expdr > number(1, 110))
			{
				...
				{
					act("&+RYour cantrip fails!  You fail the retreat!&n", FALSE, ch, 0, 0, TO_CHAR);
					return;
				}
			}
			else
...
void rescue(P_char ch, P_char rescuee, bool rescue_all)
{
	...
	if (IS_RIDING(ch))
	{
		if (!IS_IMMOBILE(get_linked_char(ch, LNK_RIDING)) && !IS_BLIND(get_linked_char(ch, LNK_RIDING)))
		{
	...
	for (t_ch = world[ch->in_room].people; t_ch; t_ch = t_ch->next_in_room)
	{
		...
		{
			found = TRUE;
			if (notch_skill(ch, SKILL_RESCUE, get_property("skill.notch.rescue", 10)) || number(1, 100) > GET_CHAR_SKILL(ch, SKILL_RESCUE))
			{
				act("$n futilely tries to rescue $N!", FALSE, ch, 0, rescuee, TO_NOTVICT);
				act("$n fails miserably in $s attempt to rescue you.", FALSE, ch, 0, rescuee, TO_VICT);
			...
			else
			{
				if (affected_by_spell(t_ch, SPELL_BLEAK_FOEMAN))
				{
					...
					snprintf(Gbuf1,
					         MAX_STRING_LENGTH,
					         "&+LAs you rush in to rescue %s&+L, the black magic surrounding $N &+Lcauses you\n&+Lto to tremble in terror! You spinelessly abandon your comrade!&n",
					         GET_NAME(rescuee));
					act(Gbuf1, FALSE, ch, 0, t_ch, TO_CHAR);

					act("&+LAs $N &+Lattempts to rescue $S comrade, the black aura around you flares, and you\n"
					    "&+Lgrin evilly as $E reels backward, unable to complete $S maneuver!&n",
					    FALSE,
					    t_ch,
					    NULL,
					    ch,
					...
					snprintf(Gbuf1,
					         MAX_STRING_LENGTH,
					         "&+LA feeling of hope fills your body as you see %s &+Lrushing to your rescue - but $n\n&+Lgrins evilly, and %s &+Lreels in terror!&n",
					         GET_NAME(ch),
					         GET_NAME(ch));
					act(Gbuf1, FALSE, t_ch, 0, rescuee, TO_VICT);

					act("&+LThe black aura surrounding $n &+Lflares, and $e grins evilly as he blocks an\n"
					    "&+Lattempt to save $N!&n",
					    FALSE,
					    t_ch,
					    0,
					    rescuee,
				...
				act("&+WYou are rescued by $N&+W, you are confused, but grateful!", FALSE, rescuee, 0, ch, TO_CHAR);
				if (!rescue_all)
					act("$n &+Wheroically rescues $N&+W.", FALSE, ch, 0, rescuee, TO_NOTVICT);
				else
					act("$n &+Wheroically dives at $N &+Wand pulls $M out of combat.", FALSE, ch, 0, rescuee, TO_NOTVICT);
...
void maul(P_char ch, P_char victim)
{
	...
	bool                   too_big;
	struct affected_type   af;
	struct damage_messages messages = {"You ferociously &+yMAUL&N $N!",
	...
	if (GET_CLASS(ch, CLASS_BERSERKER) && affected_by_spell(ch, SKILL_BASH))
	{
	...
	act("$n fills with &+rBLooDLuST&n and makes a ferocious leap at $N!", FALSE, ch, 0, victim, TO_NOTVICT);
	act("$n fills with &+rBLoodLuST&n and makes a ferocious leap at YOU!", FALSE, ch, 0, victim, TO_VICT);
	act("You fill with &+rBLoodLuST&n and make a ferocious attack against $N!", FALSE, ch, 0, victim, TO_CHAR);
	...
	dam = dice(MIN(30, GET_LEVEL(ch)), 6) * (get_property("skill.maul.damfactor", 1.000));

	if (GET_SPEC(ch, CLASS_BERSERKER, SPEC_MAULER))
		dam *= 1.5;

	percent_chance = (int)(percent_chance * (get_property("skill.maul.hitchance", 1.000)));
	 ...
	 */
	if (IS_FIGHTING(ch) && GET_OPPONENT(victim) != ch && GET_OPPONENT(ch) != victim && !GET_SPEC(ch, CLASS_BERSERKER, SPEC_MAULER))
	{
	...
	if (too_big)
	{
		act("$n &+ymakes a futile attempt to maul&n $N, &+ybut $E is simply immovable.&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+ymakes a futile attempt to maul you, but you are simply immovable.&n", FALSE, ch, 0, victim, TO_VICT);
		act("&+yYou slam up against&n $N!", FALSE, ch, 0, victim, TO_CHAR);

		if (!number(0, 3))
		{
			act("$n &+ygrowls&n on impact, and slams $s head into $N before falling to $s knees!", FALSE, ch, 0, victim, TO_NOTVICT);
			act("But $e slams his head into you anyways, before falling to $s knees!", FALSE, ch, 0, victim, TO_VICT);
			act("But you slam your head into $N before crashing to your knees!", FALSE, ch, 0, victim, TO_CHAR);
			melee_damage(ch, victim, (int)(dam / 2), PHSDAM_TOUCH, 0);
		}

		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		CharWait(ch, (PULSE_VIOLENCE * get_property("skill.maul.failedmaul.lag", 1.500)));
	}
	else if (vict_size < ch_size - 1)
	{
		act("$n &+ytopples over $mself as $e tries to maul&n $N.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+ytopples over $mself as $e tries to maul you.&n", FALSE, ch, 0, victim, TO_VICT);
		send_to_char("&+yMauling creatures of this minute size is hopeless at best.\n", ch);

		if (!number(0, 3))
		{
			act("As $n falls, $e delivers a nasty backhand blow to $N!", FALSE, ch, 0, victim, TO_NOTVICT);
			act("As $e goes down, $e backhands you!", FALSE, ch, 0, victim, TO_VICT);
			send_to_char("\n", ch);
			act("But you get the last laugh as you &+mbitch slap&n $N!", FALSE, ch, 0, victim, TO_CHAR);
	...
	else if (percent_chance > percentroll || notch_skill(ch, SKILL_MAUL, get_property("skill.notch.offensive", 7)))
	{

		act("$n &+yknocks&n $N &+ydown with a mighty blow!&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+yknocks you down with a mighty blow!&n", FALSE, ch, 0, victim, TO_VICT);
		act("&+yYou knock&n $N &+ydown with a mighty blow!&n", FALSE, ch, 0, victim, TO_CHAR);
		...
		if (GET_SPEC(ch, CLASS_BERSERKER, SPEC_MAULER) || IS_ELITE(ch))
		{
			CharWait(victim, (PULSE_VIOLENCE * 1.200));
			CharWait(ch, (PULSE_VIOLENCE * 1.800));
			set_short_affected_by(ch, SKILL_BASH, (PULSE_VIOLENCE * get_property("skill.maul.IsMauler.ShieldBashlag", 2.000)));
		}
		else
		{
			CharWait(victim, (PULSE_VIOLENCE * 1.000));
			CharWait(ch, (PULSE_VIOLENCE * 2.000));
			set_short_affected_by(ch, SKILL_BASH, (PULSE_VIOLENCE * get_property("skill.maul.IsNotMauler.ShieldBashlag", 2.200)));
		}
	...
	{
		act("$N &+yavoids&n $n's &+yferocious maul, and down $e goes!&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+yattempts to maul you, but falters and falls to $s knees!&n", FALSE, ch, 0, victim, TO_VICT);
		act("You &+ygrowl&n as $N avoids your maul, and you become &+RFURIOUS&n when you hit the ground!", FALSE, ch, 0, victim, TO_CHAR);
		SET_POS(ch, POS_KNEELING + GET_STAT(ch));
...
void shieldpunch(P_char ch, P_char victim)
{
	...
	struct damage_messages messages = {"You slam your shield into $N with incredible force!",
	...
	CharWait(ch, (int)(PULSE_VIOLENCE * get_property("skill.shieldpunch.lag", 1.000)));
	...
	{
		act("$n smashes $N with $s shield, but $E does not look impressed.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n hits you with $s puny shield.", FALSE, ch, 0, victim, TO_VICT);
		act("You smash $N with your shield, but $E does not look impressed.", FALSE, ch, 0, victim, TO_CHAR);
		engage(ch, victim); // Simplified initiate combat code from above.
		return;
	}
	if (vict_size < ch_size - 1)
	{
		act("$n tries to hit $N with $s shield, but $E easily evades the attack.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n makes a swift move trying to hit you with $s shield but misses.", FALSE, ch, 0, victim, TO_VICT);
		act("You try to hit $N with your shield, but $E easily evades your attack.", FALSE, ch, 0, victim, TO_CHAR);
		engage(ch, victim); // Simplified initiate combat code from above.
		return;
	}
	if (IS_HARPY(victim) && (number(0, 99) < 5))
	{
		act("Your attempt to shieldpunch $N fails as $M flutters into the air.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n makes a valiant attempt to shieldpunch $N, but $E flutters into the air.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n's attempt to shieldpunch you fails as you flutter into the air.", FALSE, ch, 0, victim, TO_VICT);
		engage(ch, victim); // Simplified initiate combat code from above.
		return;
	}
	...
	if (IS_IMMATERIAL(victim))
	{
		act("Your attempt to shieldpunch $N fails as you simply pass through $M.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n makes a valiant attempt to shieldpunch $N, but simply falls through $M.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n's attempt to shieldpunch you fails as $e simply passes through you.", FALSE, ch, 0, victim, TO_VICT);
		engage(ch, victim); // Simplified initiate combat code from above.
		return;
	}
	...
	{
		percent_chance = (int)(percent_chance * ((double)BOUNDED(6, 10 + (GET_C_AGI(ch) - GET_C_AGI(victim)) / 10, 15)) / 10);
	}
	...
	{
		act("Your opponent isn't standing, but you try anyway.", FALSE, ch, 0, victim, TO_CHAR);
	...
	if (notch_skill(ch, SKILL_SHIELDPUNCH, get_property("skill.notch.offensive", 7)) || number(0, 100) < percent_chance)
	{
	...
	{
		act("You try to crush $N with your shield but fail.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n lunges at you with $s shield but misses.", FALSE, ch, 0, victim, TO_VICT);
		act("$n lunges at $N with $s shield but $s aim is off.", FALSE, ch, 0, victim, TO_NOTVICT);
	}
}
...
void do_sweeping_thrust(P_char ch, char *argument, int cmd)
{
	...
	double                 mod;
	struct damage_messages messages = {"You feign and sweep downward slashing $N at the back of the knee.",
	...
	{
		act("You are too stunned to perform a sweeping attack.", FALSE, ch, 0, victim, TO_VICT);
		return;
	}

	if (!ch->equipment[WIELD] && // Players need a weapon, but mobs can perform the action nevertheless.
	    !ch->equipment[WIELD2] && IS_PC(ch))
	{
	...
	if (IS_IMMATERIAL(victim))
	{
		act("Your attempted sweeping thrust at $N fails as it is like &+wmist!&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$n makes a valiant attempt to thrust $N, but simply passes through $M.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n's attempts a fancy attack, but it simply passes through you.", FALSE, ch, 0, victim, TO_VICT);
	...
	{
		act("$n tries to find a weak point around $N's knees, but $E is simply too big!", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n performs some swift maneuver around your feet. Did not hurt.", FALSE, ch, 0, victim, TO_VICT);
		act("You try to find a weak point around $N's knees, but $E is simply too big!", FALSE, ch, 0, victim, TO_CHAR);
	...
	if (vict_size < ch_size - 1)
	{
		act("$n tries to find a weak point around $N's knees, but $E is just too small!", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n lashes toward your legs, but just gets confused by your morphology.", FALSE, ch, 0, victim, TO_VICT);
		act("You try to find a weak point around $N's knees, but $E is just too small!", FALSE, ch, 0, victim, TO_CHAR);
	...
	if (IS_HARPY(victim) && (int)(number(0, 99) < GET_LEVEL(victim) / 10))
	{
		act("Your attempt to thrust $N fails as $M flutters into the air.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n makes a valiant attempt to thrust $N, but $E flutters into the air.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n's attempt to thrust you fails as you flutter into the air.", FALSE, ch, 0, victim, TO_VICT);
	...
	percent_chance = // Attacker checks dex, defender checks agi
		(int)(percent_chance * ((double)(BOUNDED(6, 10 + (GET_C_DEX(ch) - GET_C_AGI(victim)) / 5, 15)) / 10));
	...
	{
		act("You cleanly perform a &+csweeping attack&n on $N!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n beats on you while you are down with a &+cwide arcing attack!&n", FALSE, ch, 0, victim, TO_VICT);
		act("$n leans forward and &+cswipes&n $N who is powerless to stop the attack!", FALSE, ch, 0, victim, TO_NOTVICT);
	...
	if (IS_NPC(ch) && (number(0, 99) > percent_chance))
	{
		act("$n sweeps low but you quickly sidestep out of reach.", FALSE, ch, 0, victim, TO_VICT);
		act("$n does a sweeping motion but $N reacts quickly and moves out of reach.", FALSE, ch, 0, victim, TO_NOTVICT);

		engage(ch, victim);
		return;
	}

	if (notch_skill(ch, SKILL_SWEEPING_THRUST, get_property("skill.notch.offensive", 7)) || number(1, 100) < percent_chance)
	{
		if (GET_LEVEL(ch) > 46)
		{
			sweepthrustaffect--;
		...
		if (!number(0, sweepthrustaffect) && GET_POS(victim) == POS_STANDING && (takedown_chance != TAKEDOWN_PENALTY))
		{
			act("$N was knocked off $S feet by $n's &+csweeping attack!&n", FALSE, ch, 0, victim, TO_NOTVICT);
			act("$N was knocked off $S feet by your &+csweeping attack!&n", FALSE, ch, 0, victim, TO_CHAR);
			act("You were knocked off your feet by $n's &+csweeping attack!&n", FALSE, ch, 0, victim, TO_VICT);
		...
		{
			act("$n delivers &+ca sound hit&n on $N but does not sweep $M off $S feet.&n", FALSE, ch, 0, victim, TO_NOTVICT);
			act("Your &+csweeping attack&n fails to drop $N, but you deliver a sound hit.&n", FALSE, ch, 0, victim, TO_CHAR);
			act("$n performs a low &+csweeping attack&n and you are hit!", FALSE, ch, 0, victim, TO_VICT);
	...
	else if (!number(0, 2) && (number(0, 100) > GET_C_AGI(ch)))
	{
		SET_POS(ch, POS_KNEELING + GET_STAT(ch));
		update_pos(ch);
	...
	{
		act("You feign and try to strike $N but $E sidesteps easily.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n sweeps low but you quickly sidestep out of reach.", FALSE, ch, 0, victim, TO_VICT);
		act("$n does a sweeping motion but $N reacts quickly and moves out of reach.", FALSE, ch, 0, victim, TO_NOTVICT);
...
void do_rearkick(P_char ch, char *argument, int cmd)
{
	...
	char                   buf[512];
	struct damage_messages messages = {"You shift your weight and viciously slam your rear hooves into $N with a &+Wstunning&n force!",
	...
	dam = (int)(MAX(GET_C_STR(ch) / 2, (GET_CHAR_SKILL(ch, SKILL_KICK)) * get_property("skill.rearkick.dam", 2)));
	...
	act("&+gYou tense up and prepare to rearkick&n $N &+gwith your hind legs!&n", FALSE, ch, 0, victim, TO_CHAR);

	if (!notch_skill(ch, SKILL_KICK, get_property("skill.notch.offensive", 7)) && percent_chance <= number(0, 100))
	{
		act("But fail miserably!", FALSE, ch, 0, victim, TO_CHAR);
		act("You narrowly avoid $n's hooves!", FALSE, ch, 0, victim, TO_VICT);
		act("$n attempts to kick $N but fails miserably!", FALSE, ch, 0, victim, TO_NOTVICT);
		engage(ch, victim);
		return;
	}
	...
	if (IS_GREATER_RACE(victim) || LEGLESS(victim) || IS_TRUSTED(victim) || IS_ELITE(ch))
		return;
	...
	if (takedown_chance > number(0, 100))
	{
		...
		if ((CAN_GO(victim, door)) && (!check_wall(victim->in_room, door)))
		{
			...
			if (char_to_room(victim, target_room, -1))
			{
				act("$n &+Lflies in, crashing on the floor!&n", TRUE, victim, 0, 0, TO_ROOM);
				CharWait(victim, (int)(PULSE_VIOLENCE * get_property("kick.roomkick.victimlag", 1.000)));
				SET_POS(victim, POS_PRONE + GET_STAT(victim));
				stop_fighting(victim);
				if (IS_DESTROYING(victim))
		...
		{
			act("&+rYour mighty rearkick sends&n $N &+rcrashing onto the ground!&n", FALSE, ch, 0, victim, TO_CHAR);
			act("$n's &+rmighty kick sends you crashing onto the ground!&n", FALSE, ch, 0, victim, TO_VICT);
			act("$n's &+rmighty kick sends&n $N &+rcrashing onto the ground!&n", FALSE, ch, 0, victim, TO_NOTVICT);
			CharWait(victim, (int)(PULSE_VIOLENCE * get_property("kick.wallkick.victimlag", 1.5)));
			SET_POS(victim, POS_SITTING + GET_STAT(victim));
			stop_fighting(victim);
			if (IS_DESTROYING(victim))
...
void do_trample(P_char ch, char *argument, int cmd)
{
	...
	knockdown_chance = (int)(knockdown_chance * ((double)BOUNDED(8, 10 + (GET_LEVEL(ch) - GET_LEVEL(victim)) / 10, 11)) / 10);
	knockdown_chance = (int)(knockdown_chance * ((double)BOUNDED(83, 50 + GET_C_DEX(ch) / 2, 120)) / 100);
	...
	if (!notch_skill(ch, SKILL_MOUNTED_COMBAT, get_property("skill.notch.offensive", 7)) && (percent_chance + charisma) < number(1, 100))
	{
		if (50 + GET_C_DEX(ch) / 2 > number(0, 100) || is_knight)
		{
			act("$N bucks madly disobeying $n's order.", FALSE, ch, 0, mount, TO_ROOM);
			act("$N bucks madly disobeying your order.", FALSE, ch, 0, mount, TO_CHAR);
			SET_POS(ch, POS_STANDING + STAT_NORMAL);
		}
		else
		{
			act("As $N bucks madly you shoot up in the air!", FALSE, ch, 0, mount, TO_CHAR);
			act("As $N bucks madly $n shoots up in the air!", FALSE, ch, 0, mount, TO_ROOM);

			if (50 + GET_C_AGI(ch) / 2 < number(0, 100))
			{
				SET_POS(ch, POS_SITTING + GET_STAT(ch));
				act("You land on your ass with a stunning force!", FALSE, ch, 0, mount, TO_CHAR);
				act("WHOP! $n landed heavily on $s ass!", FALSE, ch, 0, victim, TO_ROOM);
				CharWait(ch, PULSE_VIOLENCE * 3);
			}
			if (damage(ch, ch, number(1, GET_LEVEL(ch)), DAMAGE_FALLING))
				return;
	...
	else
	{
		...
		if (IS_PC(ch) && GET_POS(victim) == POS_STANDING)
		{
			debug("TRAMPLE: (%s) trying to trample with (%d) percent knock down. Victim is (%s).", GET_NAME(ch), knockdown_chance, GET_NAME(victim));
		}
...
void bodyslam(P_char ch, P_char victim)
{
	...
	percent_chance = (int)(percent_chance * ((double)BOUNDED(80, 100 + GET_LEVEL(ch) - GET_LEVEL(victim), 125)) / 100);
	percent_chance = (int)(percent_chance * ((double)BOUNDED(60, 100 + (GET_C_AGI(ch) - GET_C_AGI(victim)) / 2, 155)) / 100);
...
void do_springleap(P_char ch, char *argument, int cmd)
{
	...
	if ((has_innate(vict, INNATE_HORSE_BODY) || has_innate(vict, INNATE_SPIDER_BODY)) && (get_takedown_size(ch) < get_takedown_size(vict)))
	{
		act("$n makes a futile attempt to springleap $N, but $E is simply immovable.", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n makes a futile attempt to springleap you, but you are simply immovable.", FALSE, ch, 0, vict, TO_VICT);
		act("&+WThat damn half-horse half-man is too hefty to springleap!", FALSE, ch, 0, vict, TO_CHAR);
	...
	else if ((get_takedown_size(vict) < (get_takedown_size(ch))) || (get_takedown_size(vict) > (get_takedown_size(ch) + 1)))
	{
	...
	if ((GET_POS(vict) < POS_STANDING))
	{
		...
		act("$n passes well above $s target and lands on $s head.", FALSE, ch, 0, 0, TO_NOTVICT);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		CharWait(ch, PULSE_VIOLENCE * 2);
	...
	if (RANGED_LEAP)
	{
		act("$n dives in from nearby into a forward roll and leaps into $N!\nwho groans as $n spins around kicking $M in the chest.", FALSE, ch, 0, vict, TO_ROOM);

		act("$N flies in from nearby into a forward roll and leaps right into you!", FALSE, ch, 0, vict, TO_VICT);
		act("You fly into the room, twist around and leap into $N!", FALSE, ch, 0, vict, TO_CHAR);
	}
	if (!SUPER_SPRINGLEAP || (number(0, 3) && SUPER_SPRINGLEAP))
	{
		act("$n's leap knocks into you, sending you slightly off balance...", FALSE, ch, 0, vict, TO_VICT);

		act("Your spring at $N sends $M slightly off balance...", FALSE, ch, 0, vict, TO_CHAR);

		act("$n's leap knocks into $N, sending $M slightly off balance...", FALSE, ch, 0, vict, TO_NOTVICT);
	}
	else
	{
	...
	if (percent_chance < number(1, 100))
	{
		act("&+WHowever, you grab $n's leg after $s funny little maneuver and yank $m to the ground!", FALSE, ch, 0, vict, TO_VICT);
		act("&+WUnfortunately. $N grabs your leg and yanks you down to the ground!", FALSE, ch, 0, vict, TO_CHAR);
		act("&+WHowever, $N grabs $n's leg after $s happy little maneuver and pulls $m to the ground!", FALSE, ch, 0, vict, TO_NOTVICT);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		CharWait(ch, (int)(PULSE_VIOLENCE * 1.5));
		update_pos(ch);
	}
...
void do_whirlwind(P_char ch, char *argument, int cmd)
{
	...
	{
		send_to_char("You are not skilled enough to even attempt such sophisticated technique!\n", ch);
		return;
	}
	...
	if (!notch_skill(ch, SKILL_WHIRLWIND, get_property("skill.notch.offensive", 7)) && GET_CHAR_SKILL(ch, SKILL_WHIRLWIND) <= number(1, 100))
	{
...
void do_trip(P_char ch, char *argument, int cmd)
{
	...
	if (IS_DESTROYING(ch))
	{
		act("You futilely try to trip $p.&n", FALSE, ch, ch->specials.destroying_obj, 0, TO_CHAR);
		return;
	}

	if (IS_FIGHTING(ch) && GET_OPPONENT(vict) != ch && GET_OPPONENT(ch) != vict)
	{
		act("You are locked in combat with someone else and unable to trip $N.&n", FALSE, ch, 0, vict, TO_CHAR);
		return;
	}
	...
	/* you must be size of centaur to springleap it */
	if ((has_innate(vict, INNATE_HORSE_BODY) || has_innate(vict, INNATE_SPIDER_BODY) || GET_RACE(vict) == RACE_QUADRUPED) && (get_takedown_size(ch) < get_takedown_size(vict) + 1))
	{
		act("$n makes a futile attempt to trip $N, but $E is simply immovable.", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n makes a futile attempt to trip you, but you are simply immovable.", FALSE, ch, 0, vict, TO_VICT);
		act("&+WThat damn half-horse half-man is too hefty to trip!", FALSE, ch, 0, vict, TO_CHAR);

		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		return;
	}

	percent_chance = (int)(percent_chance * ((double)BOUNDED(72, GET_C_AGI(ch), 160)) / 100);
	...
	if (GET_POS(vict) != POS_STANDING)
	{
		...
		act("$n does some sort of dance, and winds up flat on $s ass.", TRUE, ch, 0, vict, TO_VICT);
		act("$n seems to dance with $N, but winds up flat on $s ass.", TRUE, ch, 0, vict, TO_NOTVICT);
		stop_singing(ch);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		CharWait(ch, 2 * PULSE_VIOLENCE);
		update_pos(ch);
	...
	if (!notch_skill(ch, SKILL_TRIP, get_property("skill.notch.offensive", 7)) && number(1, 100) > percent_chance)
	{
		send_to_char("You manage, with complete incompetence, to throw yourself head-first into the ground!\n", ch);
		act("$n, in a show of awesome skill, manages to trip up $mself!", FALSE, ch, 0, 0, TO_ROOM);
		stop_singing(ch);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
	...
	{
		act("$n dances an amazing maneuver in battle, and trips up $N!", FALSE, ch, 0, vict, TO_NOTVICT);
		act("You do an amazing maneuver, tripping $N.", FALSE, ch, 0, vict, TO_CHAR);
		act("$n dances an amazing maneuver in battle, tripping you.", FALSE, ch, 0, vict, TO_VICT);
		SET_POS(vict, POS_PRONE + GET_STAT(vict));
	}
	else
	{
		act("$n dances an acrobatic maneuver in battle, and trips up $N!", FALSE, ch, 0, vict, TO_NOTVICT);
		act("You do a fancy maneuver, tripping $N.", FALSE, ch, 0, vict, TO_CHAR);
		act("$n dances an acrobatic maneuver in battle, tripping you.", FALSE, ch, 0, vict, TO_VICT);
		SET_POS(vict, POS_SITTING + GET_STAT(vict));
	}
	...
	{
		act("&+WHowever, you take $n down with you!", FALSE, ch, 0, vict, TO_VICT);
		act("&+WUnfortunately, $N proves to be quite agile $Mself, and you go down as well.", FALSE, ch, 0, vict, TO_CHAR);
		act("&+Wbut $N manages to take $n down with $M!&N", FALSE, ch, 0, vict, TO_NOTVICT);
		stop_singing(ch);
		SET_POS(ch, POS_SITTING + GET_STAT(ch));
		update_pos(ch);
	}
...
void event_unflank(P_char ch, P_char victim, P_obj obj, void *args) { unlink_char(ch, victim, LNK_FLANKING); }
...
bool flank(P_char ch, P_char victim)
{
	...
	if (!notch_skill(ch, SKILL_FLANK, get_property("skill.notch.offensive", 15)) && GET_CHAR_SKILL(ch, SKILL_FLANK) < number(0, 100))
	{
		act("You fail to properly maneuver yourself to attack $N's flank.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n attempts to flank $N, but fails miserably.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("You notice $n's clumsy attempt to flank you and turn to face $m.", FALSE, ch, 0, victim, TO_VICT);
		return FALSE;
	}
	...
	act("$n moves around $N in an attempt to flank $M.", FALSE, ch, 0, victim, TO_NOTVICT);
	act("$n sidesteps to the fight in an attempt to flank you.", FALSE, ch, 0, victim, TO_VICT);
...
void do_call_grave(P_char ch, char *argument, int cmd)
{
	...
	act("&+LYou draw the power of the damned and beckon them to rise.", FALSE, ch, 0, 0, TO_CHAR);
...
void event_call_grave_target(P_char ch, P_char victim, P_obj obj, void *data)
{
	...
	act("Skeletons and other abominations rise from the cold graves tearing and clawing at $N's flesh.", FALSE, ch, 0, victim, TO_CHAR);

	act("Skeletons and other abominations rise from the cold graves tearing and clawing at $N's flesh.", FALSE, ch, 0, victim, TO_NOTVICT);

	act("Skeletons and other abominations rise from the cold graves tearing and clawing at your flesh.", FALSE, ch, 0, victim, TO_VICT);
...
void event_bye_grave(P_char ch, P_char victim, P_obj obj, void *data)
{
	if (ch->following && ch->in_room == ch->following->in_room)
	{
		act("$N &nturns toward you and bows slightly before vanishing, "
		    "sinking back into its cold grave.",
		    FALSE,
		    ch->following,
		    0,
		    ch,
...
void battle_orders(P_char ch, P_char victim)
{
	...
	if (notch_skill(ch, SKILL_BATTLE_ORDERS, get_property("skill.notch.switch", 10)) || percent_chance > number(0, 100))
	{
		act("You shout out your orders.", FALSE, ch, 0, victim, TO_CHAR);
		act("Your skin crawls as $n directs his troops in a hollow voice.", FALSE, ch, 0, victim, TO_NOTVICT);
	...
	else
	{
		if (50 > number(0, 100))
		{
			act("You shout out your orders, but no one hears you over the fighting.", FALSE, ch, 0, victim, TO_CHAR);
		}
		else
		{
			act("You shout out your orders. Hmm, what was it you really ordered?", FALSE, ch, 0, victim, TO_CHAR);
			act("Your skin crawls as $n directs his troops in a hollow voice.", FALSE, ch, 0, victim, TO_NOTVICT);
			...
			{
				gch = group->ch;
				tmp = get_random_char_in_room(ch->in_room, ch, DISALLOW_SELF | DISALLOW_GROUPED);
				if (tmp)
...
void do_gaze(P_char ch, char *argument, int cmd)
{
	...
	{
		act("You stare deep into $N's eyes... Fascinating.", FALSE, ch, 0, victim, TO_CHAR);
		return;
	}
...
void gaze(P_char ch, P_char victim)
{
	...
	if (ch == victim)
	{
		send_to_char("You pull out a small mirror and stare into it for a few seconds.\n", ch);
		CharWait(ch, PULSE_VIOLENCE);
		return;
	}
	...
	{
		act("You turn your &+Lstare&n toward $N, but it simply &+yhisses&n at you!", FALSE, ch, 0, victim, TO_CHAR);
		act("$N turns towards $n and hisses a deep monotone sound...", FALSE, ch, 0, victim, TO_NOTVICT);
		act("You turn towards $N and hiss a deep monotone sound...", FALSE, ch, 0, victim, TO_VICT);
	...
	if (get_takedown_size(victim) > get_takedown_size(ch) + 1)
	{
		act("You try to hold $N with your gaze but $E's just too tall.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n tries to hold $N with $s gaze but $E's just too tall.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n tries to hold you with $s gaze but $E's just too short.", FALSE, ch, 0, victim, TO_VICT);
		CharWait(ch, (int)(0.5 * PULSE_VIOLENCE));
		return;
	}
	...
	{
		act("You try to hold $N with your gaze but $E's just too short.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n tries to hold $N with $s gaze but $E's just too short.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n tries to hold you with $s gaze but $E's just too tall.", FALSE, ch, 0, victim, TO_VICT);
		CharWait(ch, (int)(0.5 * PULSE_VIOLENCE));
		return;
	}
	...
	if (!on_front_line(ch))
	{
		act("$N is too far away for you to lock $M with your gaze.", FALSE, ch, 0, victim, TO_CHAR);
		return;
	}
	...
	if (GET_POS(victim) != POS_STANDING)
	{
		act("$E is not in a proper position, but you try anyway!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n has difficulty locking $N with $s eyeless stare.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n has difficulty locking you with $s eyeless stare.", FALSE, ch, 0, victim, TO_VICT);
		percent_chance /= 5; // Reduced chance to gaze a non standing opponent.
	}

	if (get_takedown_size(victim) == get_takedown_size(ch) + 1 || get_takedown_size(victim) == get_takedown_size(ch) - 2)
	{
	...
	percent_chance = (int)(percent_chance * GET_C_POW(ch) / GET_C_POW(victim)) + GET_LEVEL(ch) - GET_LEVEL(victim) + number(-15, 0);

	percent_chance = BOUNDED(1, percent_chance, 90);

	if (IS_ROOM(ch->in_room, ROOM_SINGLE_FILE) && !AdjacentInRoom(ch, victim) && !IS_TRUSTED(ch))
	{
		act("$N is difficult to gaze because this area is narrow.", FALSE, ch, 0, victim, TO_CHAR);
		percent_chance = (int)(percent_chance * 0.65);
	}

	if (IS_FIGHTING(ch) && GET_OPPONENT(victim) != ch && GET_OPPONENT(ch) != victim)
	{
		act("You aren't battling $N, but you notice $M across the &+rfield of battle...&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$n sweeps $s &+Lsightless stare&n across the &+rfield of battle.&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n has difficulty locking you with $s eyeless &+Lstare.&n", FALSE, ch, 0, victim, TO_VICT);
	...
	debug("GAZE: (%s) with (%d) percent at (%s&n) - standing? (%d) battling? (%d).", GET_NAME(ch), percent_chance, J_NAME(victim), standing, battling);
	// ---------------------------------------

	if (notch_skill(ch, SKILL_GAZE, get_property("skill.notch.offensive", 7)) || percent_chance > number(0, 100))
	{
		...
		else
		{
			if (GET_CLASS(ch, CLASS_AVENGER))
			{
				act("&+WYou stare at&n $N &+Wforcing $M into &+wsubmission.&n", FALSE, ch, 0, victim, TO_CHAR);
				act("$N &+wis lost in thought as $E meets&n $n's &+wgaze.", FALSE, ch, 0, victim, TO_NOTVICT);
				act("&+WCalming visions sweep over you as you gaze into&n $n's &+Weyes.&n", FALSE, ch, 0, victim, TO_VICT);
			}
			else
			{
				act("You turn your &+Leyeless stare&n toward $N freezing $M with your gaze.", FALSE, ch, 0, victim, TO_CHAR);
				act("$N &+Lturns ashen and struggles to turn away as&n $n &+Lstares into $S eyes.&n", FALSE, ch, 0, victim, TO_NOTVICT);
				act("&+yThe world ceases to be as&n $n &+yturns $s &+Leyeless gaze&n at you.", FALSE, ch, 0, victim, TO_VICT);
			}
	...
	{
		act("&+wYou try to lock&n $N &+wwith your gaze but $E turns away.&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$N &+wshivers slightly, but turns away as&n $n &+wlooks at $M.&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("&+wYou avert your eyes as&n $n &+wtries to lock you with $s gaze.&n", FALSE, ch, 0, victim, TO_VICT);
	}
}
...
void restrain(P_char ch, P_char victim)
{
	...
	if (IS_NPC(victim) && (IS_GREATER_RACE(victim) || IS_ELITE(victim)))
	{
		act("&+LYou call your &+rminions &+Lfrom the depths to harness&n $N&+L, but it simply shrugs them away!", FALSE, ch, 0, victim, TO_CHAR);
		act("$N &+Lsimply shrugs away&n $n's &+Lattempt to restrain it...", FALSE, ch, 0, victim, TO_NOTVICT);
		act("&+LYou simply shrug away&n $N's &+Lattempt to restrain you...", FALSE, ch, 0, victim, TO_VICT);
	...
	if (get_takedown_size(victim) > get_takedown_size(ch) + 2)
	{
		act("&+LYou try to restrain&n $N&+L, but it easily breaks free of your &+rrestraints&+L.&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$n &+Ltries to restrain&n $N&+L, but &n$E &+Leasily breaks free of the &+rrestraings&+L.&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n's&+L minions attempt to restrain you, but you simply break free of the &+rrestraints&+L.&n", FALSE, ch, 0, victim, TO_VICT);
		CharWait(ch, (int)(1.5 * PULSE_VIOLENCE));
		return;
	}
	...
	if (get_takedown_size(victim) < get_takedown_size(ch) - 2)
	{
		act("&+LYou try to restrain&n $N&+L, but it easily evades your &+rminions.", FALSE, ch, 0, victim, TO_CHAR);
		act("$n &+Ltries to restrain&n $N &+L, but they easily evade&n $n's &+rminions&n.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+Ltries to restrain you, but you easily evade the attempt!&n", FALSE, ch, 0, victim, TO_VICT);
		CharWait(ch, (int)(1.5 * PULSE_VIOLENCE));
		return;
	}
	...
	if (GET_POS(victim) != POS_STANDING)
	{
		act("$E &+Lis not in a proper position to be restrained, but you summon your &+rminions &+Lanyway!", FALSE, ch, 0, victim, TO_CHAR);
		act("$n &+Lcalls forth $s &+rminions &+Lwhich have trouble restraining &n$N.", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+Lcalls forth $s &+rminions &+Lwhich have trouble restraining you&n.", FALSE, ch, 0, victim, TO_VICT);
		percent_chance = 0; // Cannot restrain !standing victim.
	}

	if (get_takedown_size(victim) == get_takedown_size(ch) + 2 || get_takedown_size(victim) == get_takedown_size(ch) - 2)
	{
		percent_chance = (int)(percent_chance * 0.6);
	}
	...
	percent_chance = (int)(percent_chance * GET_C_POW(ch) / GET_C_POW(victim)) + GET_LEVEL(ch) - GET_LEVEL(victim) + number(-15, 5);

	if (IS_ROOM(ch->in_room, ROOM_SINGLE_FILE) && !AdjacentInRoom(ch, victim) && !IS_TRUSTED(ch))
	{
		act("&+LYour &+rminions &+Lwould have a difficult time in such a narrow area.", FALSE, ch, 0, victim, TO_CHAR);
		percent_chance = (int)(percent_chance * 0.65);
	}

	if (IS_FIGHTING(ch) && GET_OPPONENT(victim) != ch && GET_OPPONENT(ch) != victim)
	{
		act("&+LYou turn from your current victim and direct your &+rsummons&+L toward&n $N&+L...&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$n &+Lturns $s focus away from the current fight and makes a strange &+rgesture &+Ltoward &n$N...&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("$n &+Lturns $s focus away from the current fight and makes a strange &+rgesture &+Ltoward you.&n", FALSE, ch, 0, victim, TO_VICT);
	...
	debug("RESTRAIN: (%s) with (%d) percent at (%s&n) - standing? %s battling? %s.", GET_NAME(ch), percent_chance, J_NAME(victim), YESNO(standing), YESNO(battling));
	// ---------------------------------------

	if (notch_skill(ch, SKILL_RESTRAIN, get_property("skill.notch.offensive", 15)) || percent_chance > number(0, 100))
	{
		anatomy_skill = GET_CHAR_SKILL(ch, SKILL_ANATOMY) - 25; // returns -25 to 70 int
			...
			*/
			{
				act("&+LYou turn to &n$N &+Land make an un&+who&+Wly &+Lgesture.\n"
				    ...
				    "&+Lare whisked away into the &+rabyss&+L.",
				    FALSE,
				    ch,
				    0,
				    victim,
		...
		else
		{
			...
			else
			{
				act("&+LYou turn and make an un&+who&+Wly &+Lgesture in&n $N's &+Ldirection...\n"
				    ...
				    victim,
				    TO_CHAR);
				act("$N &+Lis suddenly overcome by&n $n's &+Lband of un&+who&+Wly &+rminions&+L!&n", FALSE, ch, 0, victim, TO_NOTVICT);
				act("&+LYou suddenly feel unable to move as&n $n's &+rminions &+Lsubdue you!&n", FALSE, ch, 0, victim, TO_VICT);
			}
	...
	{
		act("&+LYour &+rminions&+L attempt to restrain&n $N &+Lbut $E deftly escapes the attack.&n", FALSE, ch, 0, victim, TO_CHAR);
		act("$N &+Lmakes a dextrous move, quickly evading&n $n's &+rminions&+L.&n", FALSE, ch, 0, victim, TO_NOTVICT);
		act("&+LYou quickly jump out of the way of&n $n's &+Levil &+rmionions&+L.&n", FALSE, ch, 0, victim, TO_VICT);
	}
}
...
char isSpringable(P_char ch, P_char victim)
{
	...
	if (!IS_ALIVE(ch) || !victim || IS_IMMATERIAL(victim) || affected_by_spell(ch, SKILL_BASH) || IS_ELEMENTAL(victim) || IS_WATERFORM(victim) || GET_CHAR_SKILL(ch, SKILL_SPRINGLEAP) < 1 ||
	    IS_INCORPOREAL(victim) || IS_DRAGON(victim) || IS_DRAGON(ch) || (IS_NPC(victim) && IS_SET(victim->specials.act, ACT_NO_BASH)) || (ch->specials.z_cord == 0) || !CanDoFightMove(ch, victim) ||
...
bool MobShouldFlee(P_char ch)
{
	...
	if (GET_POS(ch) != POS_STANDING || IS_STUNNED(ch) || GET_STAT(ch) <= STAT_INCAP || !room_has_valid_exit(ch->in_room))
		return false;

	if (IS_ROOM(ch->in_room, (ROOM_NO_MAGIC | ROOM_SILENT)) || affected_by_spell(ch, SPELL_FEEBLEMIND) || IS_AFFECTED(ch, AFF_BLIND) || IS_AFFECTED2(ch, AFF2_SILENCED) ||
	    IS_SET(ch->specials.act, ACT_WIMPY) || get_spell_from_room(&world[ch->in_room], SPELL_WANDERING_WOODS) || affected_by_spell(ch, SPELL_WITHER) ||
	    affected_by_spell(ch, SPELL_RAY_OF_ENFEEBLEMENT))
	{
		...
		if (IS_MELEE_CLASS(ch) && !number(0, 9))
			return true;
	}
	return false;
}
...
void do_dreadnaught(P_char ch, char *, int)
{
	...
	if (number(1, 101) < skl_lvl)
	{
		...
		af.flags      = AFFTYPE_SHORT | AFFTYPE_NODISPEL;
		af.level      = GET_CHAR_SKILL(ch, SKILL_DREADNAUGHT);
		af.duration   = duration;
		affect_to_char(ch, &af);
		...
		affect_to_char_with_messages(ch, &af, "&+yYou lower your &+Yguard&+y, and assume an offensive stance.", "$n &+ylowers his guard, and assumes an offensive stance.&n");

		set_short_affected_by(ch, TAG_DREADNAUGHT, duration + PULSE_VIOLENCE);
...
void do_shadowstep(P_char ch, char *, int)
{
	...
	if (number(1, 102) < GET_CHAR_SKILL(ch, SKILL_SHADOWSTEP))
	{
		...
		affect_to_char_with_messages(
			ch, &af, "&+LYou grin as the sha&+wdo&+Wws &+Lslowly coalesce around you.&n", "$n &+Lflashes a &+rwicked&+L grin as the sha&+wdo&+Wws &+Lslowly coalesce around them.&n");
...
void do_garrote(P_char ch, char *argument, int cmd)
{
	...
	{
		act("&+LYou try to slip behind &n$N&+L, but they notice the attempt and block your advance!", FALSE, ch, 0, victim, TO_CHAR);
		act("&+LYou notice &n$n &+Lattempting to sneak behind you, but quickly block their advance!", FALSE, ch, 0, victim, TO_VICT);
		act("$N &+Ladeptly notices &n$n's attempt to &+rgarrote&+L them and blocks the attempt!", FALSE, ch, 0, victim, TO_NOTVICT);
		set_short_affected_by(ch, SKILL_GARROTE, PULSE_VIOLENCE);
		CharWait(ch, (3 * WAIT_SEC) / 2);
		return;
	}

	act("&+LYou skillfully slip behind&n $N &+Land &+rslit &+Ltheir &+rth&+Rro&+rat&+L resulting in a &+Rmist &+Lof &+rblood&+L!&n", FALSE, ch, 0, victim, TO_CHAR);
	act("&+rOUCH! &n$n &+Lsuddenly appears behind you and makes a &+rslashing &+Lmotion at your &+rth&+Rro&+rat&+L!", FALSE, ch, 0, victim, TO_VICT);
	act("$N &+Lsuddenly stumbles, grasping at their &+rneck&+L as &n$n &+Lstands behind them grinning!", FALSE, ch, 0, victim, TO_NOTVICT);
...
void do_legsweep(P_char ch, char *arg, int cmd)
{
	...
	{
		act("Due to $S small size, you manage to take your own legs out from under you!", FALSE, ch, 0, vict, TO_CHAR);
		act("$n attempts to legsweep $N and winds up on $s ass instead!", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n attempts to legsweep you but loses $s &+Wbalance&n and lands on $s ass instead!", FALSE, ch, 0, vict, TO_VICT);
		CharWait(ch, (int)(PULSE_VIOLENCE * 0.5));
		SET_POS(ch, POS_PRONE + GET_STAT(ch));
		return;
	}
	...
	if (get_takedown_size(vict) > (get_takedown_size(ch) + 1))
	{
		act("You attempt to legsweep $M only to manage to hit $S massive shin and fall down yourself!", FALSE, ch, 0, vict, TO_CHAR);
		act("$n attempts to legsweep $N and winds up on $s ass instead!", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n attempts to legsweep you but loses $s &+Wbalance&n and lands on $s ass instead!", FALSE, ch, 0, vict, TO_VICT);
		CharWait(ch, (int)(PULSE_VIOLENCE * 0.5));
		SET_POS(ch, POS_PRONE + GET_STAT(ch));
	...
	if ((notch_skill(ch, SKILL_LEGSWEEP, get_property("skill.notch.offensive", 7)) || number(1, 100) <= percent_chance || IS_TRUSTED(ch)) && GET_POS(vict) == POS_STANDING)
	{
		if (!IS_TRUSTED(ch))
		{
			set_short_affected_by(ch, SKILL_BASH, (int)(3.5 * PULSE_VIOLENCE));
		...
		if (number(1, 100) <= 15)
		{
			act("You quickly maneuver alongside $N&n, drop low and &+RCOMPLETELY&n &+Wsweep&n $S legs out from under $M!", FALSE, ch, 0, vict, TO_CHAR);
			act("$n masterfully maneuvers next to $N and completely &+Wsweeps&n $S legs out from beneath $M!!!", FALSE, ch, 0, vict, TO_NOTVICT);
			act("Before you can react, $n maneuvers beside you and &+RCOMPLETELY&n &+Wkicks&n your legs out from beneath you!!!", FALSE, ch, 0, vict, TO_VICT);

			SET_POS(vict, POS_PRONE + GET_STAT(vict));
		}
		else
		{
			act("You swiftly maneuver next to $N&n, drop low and &+Wsweep&n $S legs out from under $M!", FALSE, ch, 0, vict, TO_CHAR);
			act("$n quickly maneuvers alongside $N and &+Wsweeps&n $S legs out from under $M!", FALSE, ch, 0, vict, TO_NOTVICT);
			act("$n quickly maneuvers beside you and &+Wkicks&n your legs out from under you!", FALSE, ch, 0, vict, TO_VICT);
	...
	{
		act("You attempt to legsweep $M but manage to land on your own ass instead!", FALSE, ch, 0, vict, TO_CHAR);
		act("$n attemps to legsweep $N but only manages to land on $s ass.", FALSE, ch, 0, vict, TO_NOTVICT);
		act("$n attemps to legsweep you, but only manages to land on $s ass.", FALSE, ch, 0, vict, TO_VICT);
...
bool check_crippling_strike(P_char ch)
{
	...
	if (IS_FIGHTING(ch))
	{
		for (kala = world[ch->in_room].people; kala; kala = kala2)
		{
			...
			if (IS_IMMOBILE(kala) || !IS_AWAKE(kala) || IS_STUNNED(kala) || !IS_HUMANOID(ch) || IS_ELITE(ch))
			{
				continue;
			}
			...
			if (skl && success > 0)
			{
				...
				if (success > 85)
				{
					act("You spin on your heel, slamming your elbow into $N's ear.", FALSE, kala, 0, ch, TO_CHAR);
					act("$n spins around, slamming $s elbow into your ear. \nYour vision blurs and your ears start ringing!", FALSE, kala, 0, ch, TO_VICT);
					act("$n spins around, slamming $s elbow into $N's ear.", FALSE, kala, 0, ch, TO_NOTVICT);
					...
					affect_to_char_with_messages(ch, &af, "&+WOooh, is that bird song you hear? How lovely.&n", "$n&n seems to be listening to the birds singing.");
				...
				{
					act("You stomp down hard on $N's knee, twisting it.", FALSE, kala, 0, ch, TO_CHAR);
					act("$n stomps down hard on your knee, twisting it painfully.", FALSE, kala, 0, ch, TO_VICT);
					act("$n stomps on $N's knee, twisting it.", FALSE, kala, 0, ch, TO_NOTVICT);
				...
				{
					act("You grab $N by the head and slam your knee in his face.", FALSE, kala, 0, ch, TO_CHAR);
					act("Blinding pain rushes over you as $n crushes your nose with his knee.", FALSE, kala, 0, ch, TO_VICT);
				...
				{
					act("As $N stands, you leap forward and slam the hilt of your weapon into $s back.", FALSE, kala, 0, ch, TO_CHAR);
					act("$n leaps forward and slams the hilt of his weapon into your back, oof!!", FALSE, kala, 0, ch, TO_VICT);
					act("As $N stands, $n leaps forward and slams the hilt of his weapon into $s back.", FALSE, kala, 0, ch, TO_NOTVICT);
...
void do_taunt(P_char ch, char *arg, int cmd)
{
	...
	if ((notch_skill(ch, SKILL_TAUNT, get_property("skill.notch.offensive", 7)) || number(1, 100) <= percent_chance || IS_TRUSTED(ch)))
	{
		...
		if (IS_TRUSTED(ch) || !NewSaves(vict, SAVING_FEAR, saveMod))
		{
			...
			else
			{
				...
				af.context  = (void *)ch;
				af.duration = duration;
				affect_to_char_with_messages(vict, &af, "You finally let the words go and can focus on other targets again.", "$n seems to have calmed down.");
