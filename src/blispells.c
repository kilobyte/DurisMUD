...
void spell_blight(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (GET_SPEC(ch, CLASS_BLIGHTER, SPEC_RUINER) || GET_SPEC(ch, CLASS_BLIGHTER, SPEC_SCOURGE))
	{
		dam = (dam * 112) / 100;
	}
...
void spell_sap_nature(int level, P_char ch, char *arg, int type, P_char victim, P_obj obj)
{
	...
	if (affected_by_spell(ch, SPELL_FALUZURES_VITALITY) && !GET_CLASS(victim, CLASS_BLIGHTER))
	{
