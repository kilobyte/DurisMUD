...
bool set_continent(int start_room, int continent)
{
	...
	while (!rooms_left.empty())
	{
		...
		for (int dir = 0; dir < NUM_EXITS; dir++)
		{
			...
			if (!TOROOM(room, dir))
				continue;
			if (world[TOROOM(room, dir)].continent)
				continue;
			if (TOROOM(room, dir) == NOWHERE)
				continue;
			if (!world[TOROOM(room, dir)].dir_option[rev_dir[dir]])
				continue;
			if (TOROOM(TOROOM(room, dir), rev_dir[dir]) != room)
				continue;
			if (TO_OCEAN(room, dir))
				continue;
			if (!SAME_ZONE(room, dir))
				continue;
...
void calculate_map_coordinates()
{
	...
	for (int room = 0; room <= top_of_world; room++)
	{
		...
		{
			int curr_room = room_stack[rs_pointer1];
			if (world[curr_room].x_coord < min_x)
				min_x = world[curr_room].x_coord;
			if (world[curr_room].y_coord < min_y)
				min_y = world[curr_room].y_coord;
			if (world[curr_room].z_coord < min_z)
				min_z = world[curr_room].z_coord;
		}
		for (unsigned rs_pointer1 = rs_start; rs_pointer1 < rs_pointer; rs_pointer1++)
		{
			int curr_room = room_stack[rs_pointer1];
