...
const char *get_event_name(P_event e) { return event_names[(int)e->type]; }
...
void clear_char_nevents(P_char ch, int type, void *func)
{
	...
	for (e1 = ch->events; e1; e1 = next_event)
	{
		...
		if ((e1->type != type && type != -1) || (type == EVENT_CHAR_EXECUTE && (void *)(e1->target.t_func) != func) || (type == EVENT_SPECIAL && (void *)(e1->actor.a_func) != func))
		{
			prev_event = e1;
			continue;
		}
		...
		case EVENT_SWIMMING:
			if (e1->actor.a_ch)
			{
				...
				else
				{
					...
					if (!e2)
					{
						logit(LOG_EXIT, "event for %s not found in ch->events list", e1->actor.a_ch->player.name);
						//  raise(SIGSEGV);
						break;
					}
					e2->next = e1->next;
